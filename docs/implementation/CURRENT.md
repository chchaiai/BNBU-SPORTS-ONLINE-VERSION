...
状态：实施中，不能作为功能完成或测试通过的交接。

## 最新状态（2026-09-07）

- 最新目标已取消“测试失败只记录”的限制，恢复修复并复验。V81-TEST-005已通过仅在有值时传递swimDelayReason修复，Docker后端typecheck退出0；原始失败证据保留。此结论不覆盖Prisma生成或数据库执行。
- 当前协议153操作；新增教师本人班级settlement-check只读检查，名单确认、原始体测和OCR来源尚未实现时明确返回UNAVAILABLE，不能结算。后续历史段落中的禁止修复及类型失败描述仅代表当时状态。

- 本文下方保留实施过程，早期“未测试/待接入”等描述是当时状态；当前验证以 VALIDATION.md、问题以 ISSUES.md、完整范围以 ACCEPTANCE.md 为准。
- 后端Draft152操作，新增迁移0021至0026。人工审核优先、排除学校统一日历两项用户决定持续有效。
- Docker领域6/6通过；最新后端类型检查因TS2379失败（V81-TEST-005）。Prisma P1012、历史迁移字节校验失败保持OPEN，SQL未执行。PostgreSQL18.4探测容器已停止，卷保留。
- 认证批准与撤销已接认可分配/重算；打卡提交投影已与读取投影统一，类型检查复验通过。真实数据库闭环仍未验证。
- 学生服务、Portal账号安全/认证/审核服务部分接入；教师退回补证事件刚接入。网页Docker镜像构建成功，原有typecheck在Phase 5B SHA校验失败，tsc未执行，记录V81-TEST-003；npm安装脚本警告V81-ENV-002未处理。未改JSX布局或CSS。
- 分管理员身份核验来源仍待答复；创建受理相关步骤保持待定，其余实施可继续。
- 尚无本地存档提交；必须完成范围审核及交接后按精确路径提交，禁止把当前工作声称为三端全量可用。
- 审核专项17项中16通过、1失败（V81-TEST-004旧提示断言）；新增3项待处理/认可投影用例通过。游泳时间专项最新7/7通过；当前共5项测试失败、2项环境警告，失败对象未修改。
- 教师有效、无效及无效改有效已使用V81流程接口，终结纠正带说明且保持幂等请求；处理阶段/固定原因映射已补。新增人工模式状态GET（SUPER、NORMAL/MAINTENANCE、组织范围、版本与更新时间），后端Docker类型检查通过；管理员现有UI没有相应操作入口，服务方法已备齐，尚未完成网页全流程。
- 最新课程发布使用domain/course-plan.ts计算实际开始日期容量：学校时区、学期/课程日期交集、当天剩余开始窗口、常规截止当天和排除日、每天1次及跨分类共享周名额；不假定认证、不要求开始窗口持续满运动门槛。专项Docker6/6与后端类型检查通过。全局模板发布选择、结算安排/归档联动、教师指定补练仍待实施。
- 材料只读核对：现有mediaEvidence无游泳前后阶段、首批受理或同批续传事实；提交仍要求素材已AVAILABLE，无法完成15分钟内先受理锁定再30分钟续传。需追加明确阶段/冻结批次/延期说明与处理队列的DB和接口，不能仅靠已有上传地址或客户端照片时间声称满足。普通包允许至少1项有效媒体（可仅视频），不能误加至少1图规则。
- 后续已新增0025游泳首批受理及文件清单表、同事务冻结/不可变约束和Prisma模型；POST swim-intake按服务器结束事实接受15分钟内批次或24小时内带说明的离线批次，记录30分钟续传截止、声明哈希及BEFORE/AFTER/OTHER阶段。阶段是材料声明，不自动证明真实拍摄时刻；教师复核仍必要。
- 首次游泳送审需与锁定批次ID/hash完全一致；补证需保留原批次前后照片，不能以新素材冒充原运动前证据。OFFLINE_DELAYED受理的有效材料包强制进入教师队列。学生仅新增acceptSwimIntake服务方法，页面调用尚未接线。
- 本批Docker时间边界3/3、后端typecheck通过（docker-swim-intake-*-20260907.txt）；未执行0025数据库迁移/约束。续传到期受理、传输完成时间与故障暂停、批次GET回读、关闭后旧提交scope及学生原页面阶段映射仍需完成，当前不能宣称游泳闭环。
- 最新续传批次：新增readSwimTransfer与GET swim-intake，读取服务器confirmedAt、材料处理状态及原/实际续传截止；30分钟按故障区间并集顺延，已真实超时不因之后故障复活。上传初始化在已锁定批次后拒绝新文件，确认/绑定只接收原批次ID。30分钟后、结束24小时内完成的原批次送审需显式swimDelayReason，追加到既有SUBMITTED事件并转教师队列；补证仍走其独立期限。
- 时间专项7/7通过，但最新后端typecheck退出2 TS2379（显式undefined可选属性），V81-TEST-005已登记，禁止修复该传参/类型/编译设置。当前共5项测试失败、2项环境警告；前面的类型通过仅是旧镜像证据。
- 独立继续实施：旧打卡提交scope允许CLOSED班级中关闭前已开始的合法会话继续，其余身份/期限检查仍执行。此改动尚未测试；游泳跨原课程截止/学期日期的已有受理窗口、初次受理前的平台故障、学生页面及传输URL续期仍待补齐。不得把当前局部流程声称为游泳全量验收。
- 新增GET /student/proof-todos：学生本人范围、签名游标、repeatable-read快照、补证剩余/暂停/截止、NORMAL和MAINTENANCE安全读取。学生listOwnProofTodos改为实际API分页；“提交补证”事件接原记录上下文、原媒体和新素材上传、workflow版本及固定幂等意图，成功后回读workspace，未改HTML模板/布局。
- 学生补证客户端Docker专项3/3通过（仅请求层模拟，非真实服务）；后端typecheck仍只报告原TS2379（V81-TEST-005），未修复。证据docker-student-proof-*及docker-proof-todos-*。
- 本轮核对发现：现有学生UI没有游泳BEFORE/AFTER阶段或延迟说明输入，不能按照片排序代填；泳批次API已有但实际页面接线仍缺来源。补证目前保留原包全部媒体，满额度时需要原素材选择机制；不得自动删除原证据。页面旧“仅展示/1.2.0不可补证”提示仍保留，需在遵守UI限制下明确后续处理。
- 新增0026 final-grade不可变版本表/连续版本/责任教师与组织触发器及Prisma模型；V81FinalGradesService按成员关系读历史、追加草稿或内部发布版本。整型为PostgreSQL INT范围，无0—100限制，无备注字段；只TEACHER且班级本人所有，不向学生发送通知。归档后普通入口拒绝，结算更正流程尚未接入。
- Portal提供listFinalGradeRevisions/appendFinalGrade服务。现有成绩表单实际输入耐力跑分钟/秒，不能擅自把它当成最终成绩；未将新接口错误接到旧换算按钮。UI-CONSTRAINTS.md集中记录这些输入缺口。
- Docker成绩协议专项3/3通过；后端类型检查仍只有既有TS2379，保持V81-TEST-005未修复。0026 SQL/事务/并发/权限运行与结算后历史更正版尚未验证。

## 已完成的准备

- 核对现行四份业务文档及 Backend 远端 `d1db18fae3870c3ad17e99c3e98ccbfa4d928947`，复用后端和相关协议/工具。
- 恢复 Web 原本依赖的 `contracts/openapi.yaml` 与 `contract-metadata.json`，来源为当前 HEAD 的父提交，只作旧网页编译输入。
- 安装 Backend 锁定依赖（`npm ci --ignore-scripts --no-audit --no-fund`）；尚未构建或测试。
- 新增 V8.1 精确计入选择、补证期限中断合并、公开原因校验，尚未测试。
- 新增 0021 SQL、V81Service/Controller/Module、课程规则和人工模式及审核草稿接口；生成本地 Draft 协议。
- 旧运动创建不再因已达标拒绝；改为检查已发布 V8.1 规则，真实时长不再两小时封顶；提交不占每日名额，也不默认有效。
- 没有修改任何网页 HTML、CSS 或 TSX 布局。
- 已追加不可变初次/补证材料包、同事务材料冻结约束、补证上传窗口、补证受理、逾期 worker、公开原因站内通知和终结事实纠错入口；旧有效/无效审核已转入 V8.1 链，旧重开入口拒绝 V8.1 记录。
- 人工模式批量入教师队列已有逐条事件；学校日历辅助代码已移除。
- 0021 新表已手写同步 Prisma 模型与关系，尚未生成客户端或验证 schema；检查约束、部分索引、不可变触发器仍由 SQL 定义。
- 规则发布后锁定已覆盖旧课程 PATCH 的时间字段及数据库日程/排除日期入口；开始窗口不再错误要求持续满单次门槛。
- 系统模式新增切换、历史和公告接口草稿，组织锁内追加维护区间、审计和站内通知；GET 不再整体绕过维护。新增学生补证计时专用安全读取入口。尚未测试。
- 网页仅补学生 api.js 和 Portal v81-data.ts 服务方法，页面事件仍待接线；不能据此宣称三端闭环。
- 新增 domain 单元测试文件，尚未执行。
- 后续已补账号安全草稿：`AccountSecurityService`、本人改密与安全状态接口、首次改密门禁、0022 账号安全/不可变登录账号表。教师仅 NORMAL 改密，管理员可在维护改密；当前会话保留，其他会话/刷新令牌撤销。缺少安全状态的旧教师/管理员按待首次改密处理，不能直接进入业务。
- 密码找回已调整为个人密码非空、已证明身份的停用账号返回停用错误、成功重置清除门禁并保持全部旧会话失效；教师维护时禁止请求和完成找回。新密码/确认密码字段加入日志脱敏。
- 分管理员登录支持独立唯一账号字段；总管理员和教师继续学校邮箱登录。管理员建立与权限分配尚未实施，因此不能宣称账号建立—登录已闭环。
- Portal `api-client.ts` 仅增加30行安全状态/本人改密服务方法，已撤回格式化造成的无关差异。未增加或修改布局，真实页面门禁与改密调用仍待核对。新代码未测试，不继承此前6项领域测试结论。

## 下一步必须完成（实施审阅发现，尚非测试问题）

1. V81 审核、补证、逾期与通知已实施，仍须完整读取投影、并发顺序及结算纠错联动。v81-credit-store.ts 使用新 certification 表，认证审核尚未接入。
2. 课程可完成性目前只做名额检查，需按开始窗口语义、实际日期/分类联合规则完善；发布后锁定旧 class-section PATCH 的日程入口。
3. 学校日历要求已排除；保留教师轮次事实，不引入固定 48 小时。
4. 检查新 Prisma 模型、迁移权限和 migration-safety 规则；每次改 SQL 都重新生成 manifest SHA。当前新模型尚未验证，生成客户端尚未刷新。
5. 账号：首次临时密码门禁、本人改密、总/分管理员权限与初始化、账号批量建立/删除与身份保留、学校邮箱规则及恢复流程。
6. 人工模式已逐条留痕且采用 SUPER 权限检查；管理员身份/权限初始化与首次改密门禁必须落实。维护专用操作不能扩大普通管理员能力。
7. 材料版本/补证及大小/hash校验已实施；游泳前后证据、15分钟受理、同批30分钟续传、24小时延迟仍待完成。旧视频解析已有音轨检查，网页 WebM/QuickTime 与 MP4 业务规则衔接待完成。
8. 正式进度、认证认可、学校名单/OCR 草稿、教师确认、内部 INT 成绩、不泄露学生分数、结算阻塞与更正版。
9. 管理员实际 mutations 尚多数走前端 Mock mutate()，必须逐项新增真实 API 分支；所有业务功能按页面逐一核对。
10. SystemModeGuard 已按两模式收紧 GET，维护区间切换已实施；仍须管理员安全操作逐一 allowlist、首次改密、密码找回角色检查及登录/刷新并发核对。恢复时间不是自动恢复指令。公开公告不得泄露治理原因；历史当前限100条，需分页。公告长度与通知存储边界待处理。
11. V8.1 新接口当前响应 schema 较粗，需要补全严格响应及网页接线。旧 docs/backend-contracts 发布记录不能当新 Draft 发布证据。
12. 创建独立本地 Docker compose，迁移/seed/API/三端/MinIO/Mailpit 同项目；最后执行测试。测试失败只登记 ISSUES.md，不修复。
13. 最终提供验证和依赖、更新运行文档，精确路径本地存档。

## 当前运行证据

- Docker Client/Server 可用；探查时无运行中容器。
- Backend npm ci 完成：637 packages，15 秒。
- 最近一次 `generate-openapi.mjs` 已生成 139 operations 的本地 Draft。0022 增加登录账号字段后已再次刷新 manifest SHA；账号新增字段及严格响应更新后须刷新协议生成物，001至0021历史验证范围不因此扩展。
- Docker 独立规则镜像构建成功；6项领域测试全部通过。
- Docker Prisma 校验失败 P1012，V81Event 唯一约束 map 名超过63 bytes；记录 V81-TEST-001，按用户指令不修复。镜像另有 OpenSSL 检测警告 V81-ENV-001，未处理。
- 尚未执行迁移、类型检查、API构建或三端 E2E；不能宣称整体可运行。

用户授权先人工审核闭环；不需要再次询问。测试失败不得自动修复。保持目标 active，当前不能宣称三端已完成。

## 最近实现变更的位置

- `backend/src/modules/v8/*`、`backend/src/modules/system-mode/*`、旧 auth/records/reviews/media/class-sections 服务及 `common/policy/system-mode-policy.decorator.ts`。
- `backend/prisma/migrations/0021_v81_business_foundation/migration.sql` 与 `backend/prisma/schema.prisma`。
- `.local/add-v81-schema.cjs` 只是已执行的一次性辅助脚本，已被 ignore，不存档；不能重复运行。
- 读取工具的路径错误不属于业务测试失败；Docker 第一批的真实验证结果见 VALIDATION.md 与 ISSUES.md。后续不能自动修复 V81-TEST-001 或把它当普通实施审阅问题处理。
- 本人改密仍需并发会话状态复核、密码证明限流和账号安全审计链核对；密码找回仍要求 organizationCode，文档无需组织代码的入口需完成。旧前端与协议登录字段、首次改密投影尚未统一。
- 后续新增 `V81AdminPermissionGuard`：按操作检查8项权限/SUPER，旧管理员课程创建、学生修改等未分配操作默认拒绝；共同账号/通知读取使用 ANY，但首次改密门禁仍先执行。新增接口必须同步此明确映射，不能通过未知路由自动放行。
- 新增分管理员分页列表、版本控制启停用 API，停用要求职责移交确认、撤销会话和刷新令牌，追加审计与事件。Portal `subadmin-api.ts` 提供相应真实服务函数；原页面按钮和列表尚未调用，未改 JSX 布局。
- 分管理员创建前的“人员已完成身份核验”来源已向用户提问：线下核验声明或学校身份系统。回答未到之前不实现依赖该决定的创建受理；其余功能继续推进。
- 当前协议已生成141 operations；上述管理员代码尚未测试。原模型校验失败保持未修复。
- 本人改密已加入认证限流，并在组织/用户锁内复核当前会话状态、所有者、期限及 tokenVersion；这些是本轮实施完善，未执行新测试，不能据此关闭既有失败。
- 新增 `v81-progress.ts`，提供现有学生网页已调用的 `/student-progress` 游标分页与班级 `/progress-target`，读取有效计入和认证认可，按分类封顶；不包含成绩字段。认证处理链尚未写入认可表，所以不能宣称认可业务已完成。
- 旧记录列表/详情已通过 `v81-record-projection.ts` 返回当前V81状态及实际计入分钟，待审核信用为0。写接口即时响应、筛选最新审核状态及一致快照仍需统一。
- 学生旧成绩列表、成绩详情及换算表读取已移除学生角色；成绩投影额外拒绝学生。学生 API 不再请求旧分数接口，进度映射优先读取包含认可的 effectiveSeconds。仅调整服务数据和状态说明，未改页面布局。
- 学生通知列表和已读回读使用已审核事件类型白名单，历史成绩通知保留但不返回；后续业务通知必须显式加入对应安全事件类型。旧缓存的专项验证仍待完成。
- 协议已生成143 operations，进度响应具有明确分类/总量结构；新增进度和隐私代码尚未运行验证。新增 ACCEPTANCE.md 记录完整范围，不能用局部通过替代全量验收。
- 认证批准已在旧 `ExemptionApplicationsService.review` 中接入 `approveCertificationCredit`：只有校队/社团对应 EXERCISE_CHECK_IN 批准接受两类整数分钟，校验同成员其他有效认可后的分类余额、写认可投影/事件并重算打卡进度。其他申请/非批准动作拒绝认可字段。仍缺撤销、认证专用列表/分配历史接口及教师网页调用参数。
- 新增0023申请受理材料历史表/不可变和3图校验触发器，Prisma模型已手写追加。申请媒体服务校验 IMAGE、JPEG/PNG/WebP、单张10MiB，正式受理还核对 AVAILABLE/真实大小/hash；补充保留已受理素材，累计最多3张。当前未测试，历史存量回填、图片缩略图和申请上传配额还需核对。
- 申请创建新增课程未关闭且学期 CURRENT 检查；创建/编辑/提交都在组织锁内复核 NORMAL。关闭前已存在申请可以继续，不因班级关闭自动截断。归档后的受理/审核边界仍需统一。
- 0023 manifest 与当前协议生成物须刷新；本轮不修复已记录P1012，未增加测试通过声明。
- 0023 manifest和生成物已刷新。后续新增0024认证撤销状态/事件/决策及状态机约束；新增 `v81-certifications.ts` 的认证分页列表、认可分配历史和责任教师撤销接口，已注册模块和协议，共146 operations。
- 撤销仅接受已批准且存在有效认可投影的认证：同事务取消认可、追加决策/申请事件/不可变分配事件、重算进度、审计和站内通知；归档后拒绝普通撤销，待接正式结算更正。旧存量未有认可投影的认证仍需迁移策略，不能假定额度。
- 申请审核的批准/驳回/补充结果已生成学生站内通知；新申请不再允许缺失明确子类型。Portal v81-data.ts 提供明确分钟参数的批准和撤销服务方法，教师页面事件调用仍待接入。
- 学生已有认证列表和分配历史查询路径现在有服务端实现，输出仅安全业务字段；没有新增UI布局。这批接口和0024尚未运行验证，原P1012保持未修复。
- Docker SQL探测镜像构建成功，PostgreSQL18.4健康启动，但0001迁移原始字节SHA不匹配manifest，SQL未执行，public表为0；V81-TEST-002仅记录。只读诊断0001至0020 CRLF文件的LF计算结果匹配，未转换文件或绕过检查；0021至0024原始SHA匹配。
- Docker npm run typecheck通过，证据docker-typecheck-20260907.txt；使用镜像中现有生成客户端，不代表P1012关闭或新客户端生成成功。
- 新增可存档初始化脚本tools/local-integration/initialize-v81-sql-probe.mjs，生成被忽略的本地随机密码且不覆盖已有文件。当前compose重现命令需带.local/v81-sql.env。
- 类型检查后继续实施：打卡提交成功响应使用事务内V81投影，返回当前审核阶段和实际计入分钟。此变更尚未复验；当前历史审核筛选、列表一致快照、其他写路径仍待完善。
- 提交响应投影已在新Docker镜像复验类型检查通过，manifest sha256:6f3c9ed1298a5dcc03d09980ab9b58eccdd56b09b0789e5c78e245eaaa1b8dc1，证据docker-submit-projection-build/typecheck-20260907.txt。SQL数据库已停止，卷保留。
- 教师现有退回补证确认处理函数已接入fetchRecordWorkflow/decideRecord：固定原因、24/72窗口、流程版本、同步锁，成功后真实回读；演示模式不发送请求。仅修改import和事件处理代码，没有改JSX布局。旧页面提示仍引用历史协议，需在最终交接中列明其与已接入行为的文案差异。
- 新增Dockerfile.v81-portal-test及portal-tests服务，准备执行网页原有npm run typecheck。前端正式1.2.0编译协议与后端V81 Draft尚未完整统一；不能以局部按钮接入声称人工审核闭环完成。
- 教师有效、无效按钮已改用V81流程版本与固定原因编码；无效改有效走/corrections，纠正请求保留相同参数和幂等键供重试，不再走旧审核两步链。所有写入成功后回读现有数据，未改JSX布局。
- 教师数据类型/映射已接待审核、待补证、技术处理中及六类公开原因和系统逾期原因，汇总仅有效记录累加计入分钟，其他处理状态计pendingCount。真实分钟按整分钟向下取整。这批新增代码尚未验证；不关闭V81-TEST-003，也不绕过原网页门禁。

2026-09-07 最新数据库复验：V81-TEST-001已修复（显式短约束名，SQL与Prisma一致），Docker db:validate退出0；V81-TEST-002已修复（0001–0020仅恢复LF，逐份匹配既有manifest哈希，不改原清单），Docker PostgreSQL 26批SQL全部执行，90表/15张V81表。生成迁移清单补入0025/0026。原始失败段落保留历史；当前尚未完成Prisma客户端重新生成及应用业务验收。数据库探测卷保留，已含表，不能再次直接运行要求空库的probe。

最新构建已通过：Docker backend-build使用当前schema生成Prisma客户端，并完整执行npm run build退出0。OpenSSL依赖已加入测试镜像，生成阶段下载放在镜像构建期间，正式构建测试断网执行。应用启动/业务闭环仍待验证。

最新启动：真实Nest应用live200，ready503，尚未就绪。SQL probe库无Prisma迁移历史；需独立运行库正式迁移。新增backend-startup容器与run-v81-startup-probe.mjs负责启动、HTTP检查、终止子进程，当前失败证据已保存。

最新：v81_runtime_test独立库正式Prisma迁移26批通过；真实应用live/ready均200。上一批ready503已由正式迁移运行库解决。运行库与SQL探测库均保留于本地sql_probe_data卷。下一步测试账号、人工模式、教师审核与学生回读。

账号真实API探测已实现：每次独立合成组织、管理员和教师首次改密、SUPER开启人工模式并回读。首次种子因小写组织代码23514失败，已修正测试后缀为大写，尚待复验；本次未执行登录和审核。原始日志docker-account-probe-20260907.txt。

最新真实API证据：管理员/教师登录、首次改密、重新登录及安全状态回读通过；SUPER人工模式开启并回读通过。docker-account-retest2-20260907.txt退出0。下一步在该真实应用测试中加入学生材料提交和责任教师审核、回读、权限/幂等验证。

最新依赖：MinIO本地真实对象往返通过，后端材料worker已在探测环境启用，账号和人工模式回归通过。下一步添加学生上传初始化、PUT、确认、绑定、材料处理及打卡送审。测试MinIO根身份仅用于本地探测，生产COS最小权限仍单独验收。

新增学生真实上传探测（初始化/PUT/确认/绑定/worker哈希与字节回读），运动时长由合成已结束会话提供。首跑学生学号格式23514阻止上传；已修正测试后缀大写，待Docker复验。证据docker-upload-probe-20260907.txt。

学生学号修正后密码登录401，属测试路径与业务规则不符。已接入Mailpit和学生邮箱验证码真实登录测试，尚待Docker复验，再继续上传。

学生验证码邮件未送达已定位并修复适配选择：显式SMTP配置优先于test内存发信器。待Docker复验，未验证学生登录和上传。

最新：SMTP学生验证码登录已执行成功；上传初始化422，错误辅助函数此前读取错误嵌套位置导致未记录code。已修正测试诊断并添加学生登录通过标记，需复验具体上传错误。

上传422来源限制已定位，文件选择器是否放行待业务答复；服务/DB尚未放宽。探测可按captureSource分别执行，默认相机来源继续验证上传链，素材与已结束会话均明确为合成前置数据。学生SMTP验证码登录已显式PASS。

用户业务确认（2026-09-07）：运动打卡材料只允许相机来源。保留EXERCISE_RECORD的IN_APP_CAMERA接口/数据库约束；FILE_PICKER拒绝符合确认规则。免测申请来源规则不受本次确认改变。

最新：用户确认运动仅相机来源；该来源真实HTTP上传/确认/绑定/处理/哈希大小回读已Docker通过。错误PNG被完整性校验拒绝，改正确合成PNG复验通过。下一步课程规则发布、记录送审、责任教师审核和学生结果回读。

最新进展：真实HTTP人工审核正向闭环通过（规则发布、材料上传/确认/绑定、学生送审0计入、教师VALID60分钟、学生回读及数据库核对）。docker-review-closure-20260907.txt退出0。当前依然未完成全部业务；下一步负向权限、重复请求/幂等、补证闭环及其余验收范围。

最新审核专项通过：学生审核403且状态不变，教师同幂等key重试返回一致且不追加判定事件/审核记录。Docker人工正向闭环回归通过。下一步补证与其他教师/跨组织隔离。

最新：目标记录workflow读取和审核写入的同组织非责任教师/跨组织教师隔离通过（四次404），人工闭环和幂等回归通过。接续补证及其他完整验收范围。

最新补证真实HTTP通过：24小时退回、学生待办/时钟、第二版原媒体引用、再次退回422、最终有效60分钟。仍需补证新上传及到期/暂停验证。

补证新上传真实HTTP已通过：首次清单保留、第二版原+新ID，最终有效计入及隔离/幂等回归通过。仍需期限/维护暂停和剩余整体业务实现验收。

维护补证测试首跑切换500，定位审计permissionId格式不合数据库约束，已修正SYSTEM-MODE，未放宽约束。恢复请求测试版本字段修正为policyVersion。待复验暂停/恢复。

最新：维护补证暂停/写入503/恢复顺延及后续审核Docker通过。认证撤销与分管理员审计permissionId同类格式已修正，待业务验证。验收表已纠正旧仅记录限制及旧DB失败状态。

最新补证超时真实worker通过：合成过去期限触发自动无效、零计入、清待办、拒绝迟交409，复扫不重复事件。提交/过期并发及72小时等仍可扩展；下一步继续认证/账号治理等未完成范围。

最新内部成绩API实测通过：整数追加/发布历史、幂等、学生管理员隔离、非责任/跨组织隔离、拒绝备注及不改变运动计入。结算和归档更正仍未完成。

新增认证批准/撤销真实测试，首跑被上传资源解析404阻止；已定位策略只认sessionId。后续需修协议resolver兼容enrollment并保持本人范围，补齐申请材料UPLOADED到可处理状态。认证批准/撤销仍未实测。

认证上传策略已新增MEDIA_TARGET_FROM_REQUEST：按businessPurpose分支解析学生所属enrollment或session，拒绝混合/缺失目标；协议生成同步153操作。认证附件进入处理的绑定阶段仍待修复，当前改动尚未Docker复验。

最新认证流程通过：实际图片上传可用、学生提交、教师批准课程120分钟、撤销REVOKED/active=false；运动审核/成绩回归通过。累计材料限制、进度回读等仍待覆盖。

最新学生进度回读通过：批准总180分钟，撤销回到运动60分钟，内部成绩未出现在进度字段中。认证操作与学生回读已连接；分类封顶、多记录重算和剩余业务继续。

下步格式缺口：认证WebP需要DTO、协议、声明校验及实际容器解析一致实现。已有新声明测试尚未运行；当前WebP不受理，不能计为完整认证验收。

WebP初步实现及Docker材料11/11通过；当前仅用途声明与既有媒体回归覆盖，需补真实WebP格式样本/异常容器测试和HTTP上传。运动仍不允许WebP。

### WebP 文件验证进度（2026-09-07）
新增有损/无损合成 WebP 样本及截断测试，Docker 媒体测试 15/15 通过。后续继续完整解码安全校验和认证材料真实上传链路；全量目标及本地 Git 存档尚未完成。

### WebP 认证接口联调通过（2026-09-07）
V81_CERTIFICATION_WEBP=1 的 Docker 后端构建及接口联调退出码 0。认证 WebP 实际上传校验、提交、批准、进度读取及撤销回退通过。详细证据见 VALIDATION.md 和 docker-webp-api-20260907.txt。完整图像解码、云 COS 及三端全量验收仍待完成。

### 图片完整解码接入（2026-09-07）
sharp 0.35.4 已接入 PNG/JPEG/WebP 实际像素解码。Docker 媒体 16/16 和真实 API 运动/认证审核闭环通过。扩大到完整单元测试发现 6 项失败（184/190 通过），证据及问题编号 V81-TEST-015 已记录，下一步核对失败与现行规则。全量验收及最终 Git 存档尚未完成。

### 全量单元测试进度
现行规则断言迁移后，Docker 189/190 通过。下一步处理 contract parity 输出中的 DTO/OpenAPI 差异；完整三端业务和最终存档仍未完成。

### 后端单元测试全量通过
DTO/协议差异已修正，Docker 单元测试 190/190、类型检查通过。后续继续真实业务未完成项及网页接口接线，全目标与最终 Git 存档尚未完成。

### 真实计时接口验证
Docker 验证服务器真实开始、暂停、继续、结束、重放及数据库一致性通过，实际 ACTIVE 2 秒、PAUSED 1 秒，伪造客户端年份未改变计时。详细范围和未覆盖项见 VALIDATION.md、docker-live-timer-20260907.txt。全量业务与最终存档仍未完成。

### 当前待处理 V81-TEST-016
真实计时后的首次 reconcile 返回 500 SYSTEM_DATA_INTEGRITY_ERROR，已保留 HTTP 与数据库日志；数据库日志无对应 SQL 错误，下一步检查应用层错误及事件登记。同步验收未通过，全目标保持进行中。

### 会话同步 V81-TEST-016 已修正验证
同步审计字段错误已修正，Docker 真实计时/同步去重/伪造事件拒绝通过。三端全业务、设备断网恢复与最终存档仍待完成。

### Portal 静态门禁通过
恢复历史协议原始 LF 字节后，Docker 完整 Portal typecheck（含协议、Worker、student TS）通过。页面全部业务接线、运行测试及最终存档仍待完成。

### Portal 构建/测试最新进度
Docker 全量构建通过；补齐测试依赖和新增审核提示翻译后 125/127 测试通过。剩余两项静态断言待核对，见 V81-TEST-017。真实全业务接线与最终存档仍待完成。

### Portal 构建及测试通过
Docker 127/127 测试通过；两项旧断言已按现行业务及 HEAD 原始控件更新。后续聚焦尚未接通的管理/结算业务与三端实际运行，最终存档仍未完成。

### 反馈管理开始实施
0027_v81_feedback_history 已在本地 Docker 迁移通过，补齐待技术团队状态和不可变处理记录字段。下一步实现管理员查询/处理接口、协议及真实业务测试，再接原页面。全目标和最终 Git 存档未完成。

### 反馈服务实现进度
V81FeedbackService 事务处理与历史读取已编写，Docker 类型检查通过。尚未挂 HTTP 路由或注册模块，协议/权限路由映射/真实业务测试/原网页接线是下一步；勿将类型检查视为反馈上线验收。

### 当前反馈阻碍
V81-TEST-018：旧 reject_feedback_mutation 触发器拒绝管理员更新。需新增迁移以现行业务约束替换旧占位禁写保护。路由/协议已注册，真实业务首轮失败记录保留；全目标继续。

### 反馈核心闭环已通过
V81-TEST-018由0028迁移修正，Docker真实反馈创建/处理/重新跟进/历史/重放/学生回读通过。下一步补齐列表搜索、人员信息、权限负面测试及原页面接线。全目标及最终存档尚未完成。

### 反馈列表接口已编写
156操作协议与路由一致性通过，Docker类型检查通过。新增每页6条的管理员反馈搜索/筛选/汇总/提交人字段。下一步真实HTTP验证和网页接线；详情人员信息、严格响应schema与学生历史仍需补齐。

### 反馈列表 API 已验证
Docker反馈处理及列表分页/组合筛选/编号邮箱搜索/概况/空结果通过；修正非法UUID搜索。证据 docker-feedback-list-api-20260907.txt。完整反馈页面接线、权限组合和响应schema仍待完成。

### 反馈详情与协议补齐
提交人资料及历史处理人姓名在Docker真实API验证通过，反馈三种响应schema已明确；156操作静态parity通过。下一步仍为权限/并发负面测试、原页面服务接线及学生历史读取。全目标和最终存档未完成。

### 反馈并发验证通过
同版本并发仅一条成功，另一条409；非法初始状态、空白回复及内部备注字段422，历史无额外写入。Docker证据 docker-feedback-concurrency-20260907.txt。下一步继续权限和原页面接线，全目标未完成。

### 原页面反馈接线准备
已新增 feedback-api.ts 真实请求与原类型适配，Docker完整typecheck通过。原admin-support仍需调用适配器；当前保留未接线状态，下一步继续事件/数据入口调整。

### 反馈数据库保护验证通过／页面决定待答复
原文篡改、历史篡改、无历史状态更新均被数据库拒绝且回读不变。证据 docker-feedback-db-guards-20260907.txt。页面真实/演示两套界面，复用完整原表单是否符合UI边界已发异步问题；等待答复期间继续后端独立工作，不标整个目标阻塞。

### 反馈权限验证通过
同token分管理员权限读取随实际授权/撤销生效，无权限写入403且不改变版本。Docker证据 docker-feedback-permissions-20260907.txt；权限修改是夹具而非授权接口验收。页面边界问题仍待答复，后续可继续学生历史和其他业务。

### 分管理员反馈读写验证通过
仅反馈权限可实际保存，学生回读回复；撤销后同token写入403且历史不变。Docker证据 docker-feedback-subadmin-write-20260907.txt。页面边界仍待答复，学生历史读取和其他后端业务继续。

### 反馈通知缺口已接入
处理同事务追加本人站内通知，Docker5次处理/重放后恰5条通知通过。下一步通知读取和反馈历史学生访问等；页面边界待答复，全目标未完成。

### 反馈通知HTTP验证通过
本人通知查询、已读、重放和未读过滤通过，反馈版本不受已读影响。证据 docker-feedback-notification-read-20260907.txt。继续学生历史与其他业务；UI决定待答复。

### 学生反馈历史已接API
本人公开回复历史读取Docker通过，不暴露管理员身份字段。157操作协议已生成。下一步隔离负面测试及页面接线，完整目标未完成。

### 反馈历史隔离通过
本人可读，他人/不存在反馈404，教师管理员访问学生路由403。测试夹具顺序问题修正后完整Docker反馈回归通过，原失败保留。证据 docker-feedback-history-isolation-final-20260907.txt。页面UI问题仍待答复，全目标未完成。

### 反馈模块完整单元回归通过
Docker190/190通过；当前157操作、28迁移。ACCEPTANCE.md已更新局部证据与未完成范围。页面复用决定待用户回复，仍可推进帮助中心等独立后端业务。

### 帮助中心开始补齐
规范化和发布状态规则已编写，Docker2/2通过。旧单语言数据结构不足，下一步双语文章存储及不可变修订、服务/协议和真实API；原UI不变。反馈页面问题仍待答复。

### 帮助中心数据库基础已建立
0029新增组织隔离的双语不可变文章修订，Docker迁移与构建通过。下一步服务/协议/学生公开投影及真实发布下线测试；当前仅存储基础，未完成帮助业务。页面UI边界确认仍待回复。

### 帮助事务服务已编写
V81HelpService创建/更新/详情与权限、规则、修订、审计事务逻辑已编写，类型检查通过。尚未注册服务/HTTP或补协议，下一步接口与真实API测试；原UI未改。

### 帮助中心 HTTP 发布生命周期（2026-09-07）
新增管理员文章创建、详情和保存 3 个接口，接入 HELP_CENTER 权限与维护模式；协议/处理器一致性 160/160。真实 Docker HTTP 已验证双语标题裁剪、不完整草稿、负小数排序权重、发布完整性、发布→下线→重新上线、幂等重放、版本冲突及教师/学生禁止写入。数据库保存同一文章 4 个不可变修订。
证据：docker-help-api-retest-20260907.txt。首次运行的测试脚本遗漏 baseUrl，原始失败保留于 docker-help-api-20260907.txt，修正测试脚本后通过。29 个迁移均已在持久化本地 PostgreSQL 中应用。运行依赖已停止，数据卷保留。
仍需帮助列表/筛选/全量概况、学生当前语言公开投影及页面接线，尚不能宣称帮助中心或三端业务全部完成。最终交接与精确本地提交尚待整体完成。

帮助 HTTP 接入后的当前镜像完整单元回归 192/192 通过，日志 docker-help-full-unit-current-20260907.txt。下一步帮助列表与学生公开读取；三端总目标保持进行中。

### 帮助列表与学生公开接口完成本地验证（2026-09-07）
新增管理员列表、学生列表和学生详情接口；协议处理器 163/163，29 迁移。管理员每页 5 条，权重降序/更新时间降序，支持双语标题和关键词搜索、状态分类筛选，全量概况不受筛选影响。学生只取得所属组织当前已发布文章的单语言投影，草稿/下线详情 404；重新上线保留首次发布时间。
Docker 验证：docker-help-publication-20260907.txt；学生网页现有 API 客户端切到 /student/help-articles 后，以真实验证码登录会话执行 HTTP 读取通过，证据 docker-help-web-client-20260907.txt。测试只替代浏览器存储与相对 URL 解析，未伪造业务响应，也不是浏览器渲染验收。原帮助页面布局未变（support.js 仅更新注释）。
仍需管理员完整编辑界面接线、跨组织与分管理员 HELP_CENTER 权限边界补测、浏览器三端验证、其他全目标缺口及最终精确本地提交。当前管理员 real 模式帮助页面仍为旧只读界面，不能声称管理员网页发布闭环完成。

### 帮助权限测试通过
当前 Docker docker-help-access-20260907.txt 验证分管理员仅 HELP_CENTER 可读写、撤权对同token即时生效，其他组织总管理员不可读写本组织文章。其他帮助回归继续通过。ACCEPTANCE.md 已刷新163操作/29迁移/192单元证据，并明确帮助后端与管理员页面未接线的边界。

### 耐力跑换算规则后端基础（2026-09-07）
新增 domain/endurance-table.ts：四套默认表各101行，闭区间唯一匹配、整数用时与分数、等级对应、相邻连续、分数随用时不得增加、空表/重复行标识拒绝。默认表按权威文档生成，不来源于客户端本地状态；无法命中时报错。
Docker docker-endurance-domain-20260907.txt 2/2通过，遍历四表0..600全部2404个整数用时并独立校对分数，验证首尾区间与越界未命中；负面编辑和中间删行破坏连续性被拒绝。尚未加入存储和HTTP管理接口，不能视为全局规则业务完成。
下一步组织隔离的换算表不可变版本与管理员增改删事务，之后真实API/网页接线和教师转换结果历史绑定。

### 耐力跑规则不可变数据库版本（2026-09-07）
0030_v81_endurance_tables 已在本地 PostgreSQL 应用，证据 docker-endurance-migration-20260907.txt，构建和原帮助链路回归通过。数据模型按组织/性别/年级组唯一，项目组合固定；每次修订完整保留整表 JSON 与操作人、请求标识和时间，版本连续并加行锁。规则表和修订禁止更新删除，插入触发器验证非空、字段类型、整数范围、等级匹配、连续性、分数顺序、规则ID唯一。
目前尚未编写管理员实际增改删服务，存储基础不等于业务接口完成。后续事务服务须记录成功/失败审计，并绑定历史转换结果的表版本。

0030存储专项 Docker 已通过，证据 docker-endurance-storage-20260907.txt。下一步管理员四表初始化、读取与单行增改删事务/接口/协议。

### 换算表事务服务已编写（2026-09-07）
V81EnduranceService 已编写四表只读默认投影和单行 create/update/delete 事务：GLOBAL_RULES权限、维护门禁、组织锁、版本比较、整表校验、不可变修订与审计。首次写入审计保留全部初始规则快照；默认读取不产生数据库写入，默认规则ID按组织/表稳定生成。尚未注册服务、HTTP处理器或协议，不能作为可用管理功能验收。下一步DTO/接口注册、严格协议和真实HTTP增改删回归。

### 耐力跑换算管理 HTTP 已接入（2026-09-07）
V81EnduranceController 注册四表读取与单行新增/修改/删除接口，GLOBAL_RULES权限映射、DTO约束、严格响应协议已生成；167操作与167处理器一致性通过。输入不能修改已有行的表归属：按指定表查找原行，其他表行不可命中。默认读取返回稳定版本0，不写库；首次成功修改追加版本1，审计保留完整初始表。后续版本递增，失败不提交。
当前进行Docker真实HTTP测试。尚需分管理员授权与跨组织负面测试、管理员原页面接线、教师正式换算结果与历史绑定、失败审计覆盖；总体目标未完成。

耐力跑管理员真实HTTP增改删Docker通过，证据docker-endurance-api-20260907.txt。下一步权限/并发和页面接线、教师换算结果绑定；当前167操作、30迁移。

### 换算表并发与权限通过
证据 docker-endurance-concurrency-20260907.txt。下一步可直接接原 admin-rules.tsx 已有完整布局：其读取state.enduranceRules、调用admin-service.saveEnduranceRule/deleteEnduranceRule；目前admin-store.createRealAdminState为空规则，real run/服务写入门禁仍需接入。该页面无需新增布局，可在数据层接线；帮助/反馈real只读页面的复用问题仍待决定。

### 管理员换算表原页面数据接线（2026-09-07）
新增portal app/endurance-api.ts，real模式按account-security权限加载四张表，将表版本随规则保存到内存投影；保存/删除通过真实接口，返回整表替换当前表。admin-store初始化及刷新加载真实规则；admin-service按real/demo走对应写入路径。admin-rules.tsx仅两处回调传入当前state，JSX布局未改。
Docker类型/原合同绑定检查通过（docker-endurance-portal-typecheck-20260907.txt），Portal完整127/127通过（docker-endurance-portal-tests-20260907.txt）。这些测试尚未证明新增适配器真实HTTP业务调用，下一步运行实际网页客户端与浏览器验证。还需保存失败/不确定响应重试语义专项；原后端幂等能力不能直接替代网页重试验收。

### 换算表网页不确定响应重试（2026-09-07）
endurance-api.ts增加同一已加载state、路由与请求体对应的待完成幂等键，成功后移除；请求失败保留键供原操作重试。更换表版本/输入不复用旧意图。此逻辑是页面存续期间内存级重试，尚未承诺关闭或刷新整个网页后继续原意图。
Docker正在直接加载Portal的API适配器，使用真实管理员会话读写；测试注入仅丢弃成功响应并提供浏览器存储/同源URL解析，不伪造服务端结果。

Portal真实HTTP与响应丢失重试Docker已通过，证据docker-endurance-portal-http-20260907.txt。下一步完整Portal类型/回归后浏览器操作、管理员其他模块、教师换算与结算等目标缺口。

### 换算表接线并发边界已验证
旧编辑表单随刷新误用新版本的接线风险已处理，真实HTTP409且不覆盖新内容通过，证据docker-endurance-stale-editor-20260907.txt。ACCEPTANCE刷新167操作/30迁移和全局规则当前证据。仍需浏览器验收与其他目标缺口；未完成最终交接/提交。

### 原始体测记录存储与年级依赖（2026-09-07）
已阅读教师流程11.1–11.3：教师确认本人学生项目、原始用时和测试日期，修正追加历史；换算按当前年级组自动选择，学生不取得分数/等级。当前数据库gradeYear为入学年份，异常学籍的当前年级来源未明确，已向用户询问按课程学年差值推导及异常核对方案。未收到答复前不编造年级分组。
0031_v81_physical_results已在Docker应用，包含项目、整数原始秒数、测试日期、责任教师、请求及版本。数据库强制责任教师/组织范围、连续版本、不可改写删除。docker-physical-migration-20260907.txt构建与旧换算表回归通过；尚未接正式体测写接口、OCR流程、学生投影或转换历史。

原始体测存储专项通过：docker-physical-storage-20260907.txt。当前31迁移。下一步可继续原始结果教师/学生API、性别项目与免测边界；当前年级来源问题待答复，不阻塞其他业务。

### 原始体测读取接口已接入（2026-09-07）
新增教师本人课程原始体测历史分页与学生本人当前原始结果读取两个接口，协议169/169通过。教师历史按版本降序翻页；学生仅投影version/runType/elapsedSeconds/testedOn，不含换算分、等级、其他语言源或教师身份；无记录返回NOT_RECORDED，已批准PHYSICAL_TEST免测优先返回EXEMPT且result为空。读取不依赖尚待确认的年级推导。
当前正在Docker验证基础读取及身份范围；写入仍为测试直接SQL，正式教师确认/OCR/学生页面尚未接完。

原始体测读取及角色范围Docker已通过，证据docker-physical-read-api-20260907.txt。年级问题仍待回复，可继续原始结果写入/免测限制等独立工作。

### 责任教师原始体测写入已接入（2026-09-07）
新增appendV81PhysicalResult，170操作parity通过。只接受项目、整数秒、有效测试日期、预期版本；责任教师/组织/本人课程范围、性别项目匹配、已批准免测禁止录入、维护/归档门禁、版本冲突与幂等事务已实现。追加原始历史及审计，不写换算分或最终成绩。未知OTHER性别不能猜项目。
年级判定问题仍待答复，转换结果关联暂未实现；OCR选中批次核对、学生通知和原页面写入接线仍需完成。

教师确认原始用时HTTP与回读/重试/负面测试通过，证据docker-physical-write-api-20260907.txt。后续免测限制与学生通知、页面/OCR、转换/结算等继续。

### 原始体测通知已加入事务
确认写入同事务创建RAW_ENDURANCE_RESULT本人通知，只含项目、原始秒数和测试日期，按用户偏好提供中文/英文。测试脚本数组形状误用已纠正，正在Docker复验。年级、OCR、正式换算与页面仍待完成。

原始体测通知复测通过，证据docker-physical-notification-retest-20260907.txt。下一步免测/原页面/年级与转换等业务。

### 体测免测闭环专项启动（2026-09-07）
在原始用时确认测试后增加真实PNG签名上传、确认、媒体校验可用、PHYSICAL_TEST申请提交和责任教师批准，验证学生原始结果接口切换EXEMPT、继续普通用时写入409、旧历史与原通知保留。此轮覆盖人工审批流程，未使用AI或学校真实医疗资料。正在Docker运行，结果以日志为准。

免测真实上传/提交/批准与原始结果衔接Docker通过，证据docker-physical-exemption-20260907.txt。后续学生/教师页面、年级转换和结算等目标继续，尚未最终提交。

### 学生原始体测数据接线（2026-09-07）
学生api.js增加getOwnPhysicalResult和无分数的mapPhysicalResult；工作区按当前enrollment真实读取，替换固定not_recorded/null占位，接口失败不伪造无记录。原services.js体测卡片绑定服务器项目/秒数/日期，grades.js已有时间显示兼容合法0秒；仅数据绑定与现有hint内容调整，未增删HTML结构或CSS布局。
Docker正在直接调用学生API与既有renderEnduranceScoring，分别核对245秒/日期和批准免测后不显示旧用时。该验证不等于浏览器页面交互，也尚未覆盖完整工作区刷新所有接口。

学生原始体测/免测数据接线及原渲染函数Docker通过，证据docker-physical-student-client-20260907.txt。下一步教师页面、原始结果/OCR与年级换算等；完整目标未完成。

### 教师原始体测数据投影接线（2026-09-07）
teacher-data.loadTeacherGrades增加每名学生原始历史读取和已批准免测状态合并，保留physicalResultVersion/测试日期/项目；原有分钟秒数列获取服务器用时，免测隐藏旧用时，无记录映射NotRecorded。physicalScore不再复用旧综合成绩，正式换算依赖年级确认与历史绑定。
本轮未改教师JSX布局，也尚未把教师旧saveGrade的重新计算操作替换为原始测试确认：原弹窗缺测试日期等必要输入，需继续核对已有入口。Docker类型与原127测试通过（docker-physical-teacher-projection-20260907.txt），新增投影测试正在运行。

教师原始体测投影专项5/5通过。旧文本格式化的undefined分/免测分数已去除，尚待最后回归。教师写入原始测试日期的现有入口仍需核查；不宣称整教师页面业务完成。

### 当前体测接线回归通过
当前Docker：后端194/194；Portal类型+127/127；V8.1专项5/5。证据docker-physical-backend-full-unit-20260907.txt与docker-teacher-physical-final-regression-20260907.txt。教师日期输入和年级来源待用户决定，可继续其他后台模块。31迁移、170操作；全目标未完成，未最终提交。

### 体测导入草稿校验基础（2026-09-07）
新增domain/physical-import.ts：按学号精确且唯一匹配，不凭姓名找人；姓名、项目、日期不符与全部重复行标疑点，保留原始行；无疑点也confirmed=false。支持整数秒（含s/秒单位）、分:秒、中文分秒，4.30/4,30/越界秒/不明确形式拒绝；日期仅严格YYYY-MM-DD并验证真实日历日期。正式原始结果写入复用日期函数。
Docker docker-physical-import-domain-20260907.txt 2/2通过（解析/日期与匹配/重复/不自动确认）。这只是草稿领域基础，尚无文件接收、CSV/XLSX转换、OCR服务、持久化草稿、选中批次确认或网页导入接线。教师日期入口和年级来源问题仍待答复。

### 体测CSV草稿读取基础（2026-09-07）
新增domain/physical-csv.ts，复用现有csv-parse依赖，严格UTF-8与五列中英文标题映射；保留字符串学号前导零、原用时、带引号逗号单元格。调用方提供maxBytes/maxRows技术资源限制，禁止空文件、重复/未知/缺失列、损坏CSV与无效编码。输出仅PhysicalImportRow，后续交inspectPhysicalImport标疑点，不生成正式体测结果。
尚未实现上传接口、草稿持久化、XLSX/OCR服务及教师选择确认，不能宣称成绩单导入可用。

CSV与草稿联合Docker4/4通过，证据docker-physical-csv-20260907.txt。下一步草稿存储及责任教师导入/修改/选中确认接口，原待确认事项仍保留。

### 体测导入草稿存储（2026-09-07）
0032_v81_physical_imports已在Docker应用，原体测/免测回归通过（docker-physical-import-migration-20260907.txt）。新增批次原始行及来源摘要、逐行不可变修订、逐行正式结果关联；数据库校验责任教师课程范围、行号、连续版本、最新版本确认和正式结果课程范围，确认后禁止修订。
正在执行直接SQL存储专项验证。该存储尚不等于完整导入：行内容语义、疑点重新检查和选中批次原子写入须由后续服务实现；文件/OCR原件保留与访问还需接对象存储。

0032草稿部分确认存储专项Docker通过，证据docker-physical-import-storage-20260907.txt。下一步责任教师导入创建/读取/修订/选择确认事务与接口；全目标未完成。

### 体测导入创建/读取事务服务（2026-09-07）
新增V81PhysicalImportsService：CSV上限1MiB/1000行（本地实现技术资源上限），责任教师本人课程、维护与归档门禁；幂等创建原始批次、摘要、逐行版本1和审计，批量SQL插入；读取在RepeatableRead中取最新行并用当前ACTIVE成员按学号重新标疑点，保留原始行与最新行、已确认版本及pendingCount。
此服务尚未注册HTTP/DTO/协议或运行真实调用，类型检查不等于导入功能可用。下一步创建/详情接口与真实CSV草稿验证，之后逐行修订和批量确认。原件文件留存/XLSX/OCR与界面仍待补齐。

### 体测CSV导入草稿 HTTP 注册（2026-09-07）
新增createV81PhysicalImport和getV81PhysicalImport，责任教师角色、UUID/CSV长度DTO及严格响应字段已加入协议；172操作parity通过。接口接受JSON内UTF-8 CSV内容供客户端读取文件后上传，暂不是multipart/对象存储原件上传；该能力边界不隐去。
正在Docker验证三行CSV正常/含糊/重复/错误日期、幂等、详情回读、越权与无正式结果。修订/批量确认、XLSX/OCR、源文件留存和界面尚未完成。
...
### 体测导入选中确认实现（2026-09-07）
新增 confirmV81PhysicalImport，174 操作 parity 通过。在幂等事务和批次锁内重新校验当前草稿及选中版本，拒绝重复选择/已确认/未解决疑点；调用抽出的 appendInTransaction 共用原始体测、免测、版本、通知和审计规则。任意行失败则正式结果、确认关联和通知全部回滚。未选中行继续待处理。
本轮首次构建字段错误及随后审计版本冲突均已记录在 ISSUES，原日志保留。当前正在 docker-physical-import-confirm-event-retest-20260907.txt 复验。批量规模、完整单条体测回归、真实文件留存/XLSX/OCR和页面仍需验证或实现。
...
### 学生邮箱换绑并发验证（2026-09-07）
业务依据10-student-flow.md 5.3：旧邮箱和新邮箱都需验证；学生现有API客户端已调用/me/email-verification-challenges及verify。新增V81_EMAIL_SWAP：真实Mailpit两封验证码、并发提交同一挑战、赢家幂等回放、脱敏/me和数据库邮箱/版本检查。
首次服务返回200/401，测试误用primaryEmail而失败，已在ISSUES保留证据；复验docker-email-rebind-concurrency-retest-20260907.txt正在进行。未修改邮箱服务锁或业务；该单次竞争测试不能排除所有不同挑战、不同会话和唯一邮箱竞争组合。
...
### 教师批量账号 CSV 校验基础与邮箱决定（2026-09-07）
用户答复：教师邮箱自定义，不强制固定域名。因此教师批量校验仅要求有效邮箱格式、批内唯一及现有账号无冲突，不再要求学校域名白名单；不把此前试验域名规则当成业务要求。
新增domain/teacher-import.ts：UTF-8 CSV，必填employee_id/name/email、可选college；拒绝初始密码列和未知/重复列，保留工号前导零；逐行校验数据库长度、格式、重复及现有冲突。任一行出错canCreate=false，初始密码规则独立校验至少8位且含大小写与数字，预览行不含密码。
旧版本docker-teacher-import-domain-20260907.txt 3/3通过但含已被用户否定的域名限制，仅留过程证据；当前规则正在docker-teacher-import-custom-email-20260907.txt复验。
仍需实现真实数据库预览/确认事务、管理员权限、初始密码哈希、首次改密和现有网页接口接线。此前身份核验等其他待确认事项未由本条邮箱答复解决。
...
### 教师批量预览真实接口（2026-09-07）
新增 POST /admin/teacher-imports/preview，仅 USER_ACCOUNTS 权限管理员。RepeatableRead 中检查本组织教师工号及所有用户邮箱冲突，返回逐行错误、canCreate；有效批次返回服务器HMAC凭证，绑定组织/操作者/精确CSV，供后续确认接口校验。预览不接受或输出密码，不创建账号；错误批次previewToken=null。
CSV技术限制1MiB/1000行，自定义邮箱不限制域名。179操作parity通过，正在docker-teacher-import-preview-20260907.txt验证数据库冲突、自定义邮箱、重复、教师越权及预览不写账号。
确认接口、原子建号、密码哈希、首次改密及网页importUsers真实接线尚未完成；凭证尚未接确认消费，不能宣称批量建立可用。
...
### 教师批量确认接口实现（2026-09-07）
新增 POST /admin/teacher-imports/confirm：USER_ACCOUNTS 权限、绑定操作者/组织/精确 CSV 的 HMAC 预览凭证、统一初始密码至少8位含大小写和数字、NORMAL 模式、组织锁及 Serializable 幂等事务中重新校验全部工号/邮箱冲突。整批写入 users、teacher_profiles、v81_account_security 和一条含数量/用户ID的批次事件，响应不含密码；账号凭据使用 Argon2id 哈希，密码幂等摘要使用服务端 HMAC，不保存密码明文。初始共享密码采用每批一次随机盐哈希，个人改密生成新的个人哈希。
不把管理员录入邮箱当作邮箱验证码验证：新账号 emailVerifiedAt 保持空，需按既有邮箱验证流程另行验证。180 操作协议静态 parity 通过、Docker 编译/33迁移/启动通过。真实确认流程的最终脚本复验正在 docker-teacher-import-confirm-final-20260907.txt 进行。
网页现有 admin-users.tsx 的预览仍读取本地状态、确认仅允许 demo 模式；下一步需接真实预览与确认并刷新列表，保留现有布局。其余全业务、最终交接及本地 Git 存档仍待完成。
...
### 学生本人补练窗口与站内通知（2026-09-08）
新增 GET /enrollments/{enrollmentId}/makeup-windows，仅允许本组织本人学生读取；包含分页、已撤销和过期历史。windowState只表示时间与撤销状态，不能替代会话启动时对成员、课程、学期及系统模式的检查。教师授权/撤销在同一幂等事务写入站内通知，采用用户语言和组织时区；学生通知读取白名单接入 MAKEUP_WINDOW_GRANTED、MAKEUP_WINDOW_REVOKED。此次仅后端、协议与测试变更。
验证：docker-makeup-student-notifications-20260908.txt退出0，覆盖验证码登录、本人分页、他人成员404、教师/管理员403、无效limit422、撤销历史、授权/撤销重放与并发后的通知入库数量。继续完成通知读取接线后，docker-makeup-student-inbox-20260908.txt退出0，重新覆盖前述断言并验证学生实际GET通知、标记已读、同键重放及未读列表排除。41迁移无待应用、Nest构建通过；本地Draft216操作/216处理器，query42/body103静态parity及迁移manifest检查通过。测试使用合成学生、未来补练安排和本地Mailpit，不等于真实学校数据或三端浏览器验收。
此前docker-makeup-session-20260908.txt的取消请求422未修复，原失败与合成进行中会话保留。本轮未验证完整运动结束/材料提交、日周上限、已结算状态拦截，也未完成补练网页数据接线与全三端验收。Docker服务已停止，持久卷保留。正式结算、学期切换归档等其余业务、最终交接与精确本地Git存档仍待完成。

2026-09-08 学期切换前检查实现并发现403：GET /admin/semesters/{id}/switch-check读取目标状态/组织业务日期、旧学期及分页课程阻塞，正式结算来源未实现时保持UNAVAILABLE。共享课程检查读取已提取，原教师接口仍自行验证责任范围。新增本地Draft217操作，query43/body103静态parity通过。
Docker docker-semester-switch-check-20260908.txt退出1：Nest构建/41迁移/已有学期管理回归通过，新增接口首个管理员请求403。管理员权限映射遗漏getV81SemesterSwitchCheck，按测试问题只记录要求保留，不补映射或改权限策略；新增接口行为断言尚未执行。正式结算、真正原子切换归档、三端全部业务与最终Git存档均未完成。详见ISSUES.md本日条目。
2026-09-08 共享结算读取提取后独立回归：docker-settlement-reader-regression-20260908.txt退出0，原教师结算检查继续跟随当前电子名单版本并报告未确认/待办阻塞，名单注册、管理员汇总、教师综合名单及确认原流程通过。该回归不绕过或修复新增管理员接口403，也不补作其缺失断言。Docker服务已停止并保留持久卷。

### 已确认运动模板发布与课程绑定（2026-09-08）
新增0042_v81_rule_templates：同组织连续版本、固定已确认参数JSONB约束、SUPER/正常模式写入守卫与不可修改/删除触发器。模板只保存当前文档已确认范围：1200分钟、门槛30/45/60默认30、周2/3/4默认3、单次60、日1、整分钟、默认补证24/特殊总窗口72小时、收尾7天。24/72适用于教师选择补证窗口，72不是游泳专属；游泳提交时限继续由已有业务模块执行。
新增 GET/POST /rule-templates 和 GET /rule-templates/{id}。总管理员发布，教师/总管理员读取，分管理员GLOBAL_RULES不获得模板治理权限。组织锁、版本检查、幂等事务保存模板与审计，不接受任意公式或自定义规则参数。
课程规则请求新增templateId：草稿可不指定，新发布必须选择同组织已发布模板，模板ID随规则/审计保存。v81_course_rules新增外键与来源触发器；先前已发布规则保留原事实且template_id为空，不补造历史模板来源，不重算已发布规则。旧已发布规则的不可修改保护继续生效。
Docker docker-rule-templates-20260908.txt退出0：42迁移应用/Nest构建通过；实际HTTP验证SUPER发布、教师/分管理员拒绝、未知规则字段422、跨组织读取404、幂等重放、并发201/409、版本分页、新课程无模板422/未知模板404、有效模板发布与重放、再发模板不改旧课程、旧规则更换模板409，以及SQL修改/删除模板被拒绝。审计仅两条对应版本1/2。220操作/220处理器静态parity通过，query44/body104，迁移manifest检查通过。
验证边界：合成账号及课程计划，本地服务；新模板读写端到端不等于三端浏览器验收。现有课程发布客户端和直接插入课程规则的旧探测夹具尚未迁移templateId，新发布请求需要该字段；未据此声称旧完整测试仍通过。模板选择网页接线、旧无来源规则的交接处理、更多日期/维护/异常边界仍待完成。学期检查403及其他原测试失败保持未修复。Docker服务停止、持久卷保留；全范围交接及精确Git存档未完成。

### 期末截止提醒处理（2026-09-08）
新增V81CourseRemindersService/Worker与0043通知唯一索引。当前学期已发布课程进入常规截止前14天且尚未截止时，读取当前ACTIVE成员有效计入/认证后的两类剩余目标及待处理记录/申请；目标未完成或仍有待办则生成本人站内提醒。责任教师已有课程结算检查中真实BLOCKED类别形成提醒，不把UNAVAILABLE来源伪装为数量。通知不含分数/等级/排名，使用用户语言与组织时区，服务器当前时刻写入通知及系统审计。
课程发布事务调用即时检查；后台每5秒扫描一个符合条件的课程，每批最多100名成员，游标遍历后重新扫描；进程重启可重扫，数据库按组织/接收者/目标去重，不依赖已读状态。组织/课程锁及NORMAL重检保护处理；单课程错误记录脱敏日志并继续其他课程，后续轮次重试。补证/课程规则/成绩不因提醒改变。
Docker docker-course-reminders-20260908.txt退出0：43迁移/Nest构建通过，模板发布及课程绑定回归通过；测试时钟覆盖14天前1毫秒不提醒、恰好14天进入提醒、发布未发生不提醒、不足14天判断为即时、到截止不再新发；真实数据库验证维护模式不发、恢复后两路并发与重复处理只生成学生/教师各1条通知、学生剩余1200分钟事实、教师HTTP通知可读取、系统审计与测试时钟一致。220操作静态parity及43迁移manifest检查通过。
验证边界：学生/课程与时钟为合成夹具；直接驱动处理服务，不证明后台真实计时扫描、重启恢复或大批量扫描延迟达标。发布时不足14天的完整HTTP场景未验证，仍必须通过原有可完成性检查，不为提醒放宽1200分钟和可完成性规则。已完成且无待办、跨类别认证抵扣、多页成员、维护跨过截止、已关闭/归档等更广运行边界待验证。前端本轮未改；原测试失败保留，Docker已停止且数据卷保留。正式结算/归档、三端完整验收和精确本地Git存档仍待完成。

### 期末提醒真实应用定时扫描（2026-09-08）
新增tools/local-integration/v81-reminder-live-worker-probe.mjs，V81_REMINDER_LIVE_WORKER=1驱动。使用真实Nest进程、SystemClock与应用自身5秒定时器，不调用提醒处理服务代替调度；总管理员HTTP发布模板、合成学生Mailpit验证码登录后，将明确标注的临近截止规则直接写入测试数据库，以隔离后台扫描。该安排绕过发布可完成性检查，仅作为定时任务夹具，不能证明该短计划可合法通过课程发布。
Docker docker-reminder-live-worker-20260908.txt退出0：43迁移/Nest构建/账号首次改密通过；维护12秒期间0提醒，恢复NORMAL后教师/学生各收到1条，首条在恢复后1062毫秒生成，时间不回填；两个角色真实GET通知且目标范围正确，学生已读和同键重放通过，再等待12秒重复扫描仍共2条，系统审计仅2条且实际服务器时间一致。该延迟仅为本次单一场景观测，不是性能保证。
尚需真正进程重启、多页成员/多课程负载、完整临近截止发布、已完成/认证变化等边界。此前学期检查403、补练取消422与其他失败仍保留。本轮仅测试与文档更新；验收清单新增模板/补练/提醒当前证据并明确学期阻塞。Docker已停止并保留卷；三端完整业务、正式结算/归档及最终精确Git存档尚未完成。

### 教师内部最终成绩真实HTTP专项（2026-09-08）
新增独立V81_FINAL_GRADE_HTTP探测，使用合成学生和本地Mailpit验证码登录，不改变原完整人工链路中保留的失败测试。docker-final-grade-http-20260908.txt退出0：43迁移/Nest构建/真实账号首次改密通过；教师写入INT32下界草稿、上界内部发布和负数发布，123/456并发201/409且同键重放不增版本；历史两页按4/3、2/1返回，历次值和发布状态保留。
INT越界、小数、字符串、非布尔发布值及note/remark/reason额外字段422；学生和管理员GET/POST403，其他教师和跨组织教师404。学生通知列表发布前后不变，内部成绩历史响应只包含既定5字段；审计四个连续版本含责任教师标识，不将最终分数写入通用事件facts。数据库修改/删除历史及非责任教师直接追加被拒绝；拒绝后历史保持不变。
本轮只增加测试及更新验收记录；未修改生产业务代码或UI/UX。测试不涉及正式结算快照、归档后的原因更正、成绩相关所有其他接口泄漏面或完整网页操作，不能据此宣称结算完成。原测试问题保留，Docker停止保留数据卷；剩余业务、全范围交接和精确Git存档待完成。

### 综合名单内部成绩来源（2026-09-08）
在准备结算快照时发现综合名单缺少独立内部成绩来源，本轮先补齐该依赖。v81-composite-roster.ts在同一RepeatableRead快照读取每名成员最新编辑版本与最近published版本；后续草稿不替代最近已发布版本。返回finalGrade.latestRevision/publishedRevision（均可为空），未匹配身份行finalGrade为空；运动/认证目标、体测及待办计算保持独立。教师XLSX增加内部最新成绩/版本与内部已发布成绩/版本四列，不新增备注。
Docker docker-composite-final-grades-20260908.txt退出0：43迁移/Nest构建及原内部成绩专项通过；HTTP发布模板/规则、真实CSV两行上传及名单确认、教师综合预览，验证已发布版本4与后续未发布草稿5分开，未注册行不猜成绩，草稿变更后运动/体测投影一致。实际XLSX解码验证最新777/version5与原已发布值/version4；学生及管理员读取和导出均403。220操作静态parity通过，新增字段进入本地协议。测试为合成账号/课程/成绩。
本轮未完成教师最终确认和不可变结算快照，isSettlementSnapshot仍false；未注册或未录体测的结算例外继续待用户业务答复。旧测试问题保留，前端UI/UX未改，变化在后端API和教师导出文件。Docker停止并保留卷；正式结算、归档更正、三端完整验收与最终Git存档仍未完成。

### 统一只读审计查询（2026-09-08）
新增 GET /admin/audit-events 和 GET /admin/audit-events/{source}/{eventId}，AUDIT_QUERY权限、组织范围内合并audit_logs与v81_events。组合筛选source/action/outcome/requestId/actorUserId/targetType/targetId/startDate/endDate在分页前执行，日期按组织时区包含起止整日，每批最多50；签名游标绑定管理员、组织、全部条件、顺序和批量大小，以时间/ID/来源稳定排序。详情按明确来源定位，避免两表同ID混淆。
FOUNDATION使用已有动作专属安全元数据白名单；V81只投影版本号，不读取或返回facts中的原始业务快照、内部成绩或个人资料。V81现有表未保存角色快照、结果、原因代码，响应保持null，不从现用户资料或事件名推断成功/失败；此数据来源缺口仍待写入链完善，含无actor事件的系统主体标记亦未在本轮补造。原始事件只读；授权读取本身追加既有AUDIT_LOG_READ安全审计。
Docker docker-unified-audit-events-20260908.txt退出0：43迁移/Nest构建/登录通过；合成V81未知操作验证上海时区开始日00:00与结束日23:59:59.999包含、外侧1毫秒排除、分页2+1、组合筛选、结果未知不误匹配SUCCEEDED、详情字段相同、FOUNDATION原结果和角色保持。模拟密码/内部成绩/原始快照仅在测试原始facts里，HTTP只返回版本；跨组织详情404、教师403、子管理员权限授予/撤销、limit51及无效日期422、游标跨筛选422。查询后原6条合成业务事件保留，222操作静态parity通过（query45/body104）。
运行日志ZIP是独立的服务器诊断归档，尚未实现；现有exports路由仍拒绝请求，不能把审计列表下载当作ZIP完成。后续需日志采集/组织归属/强制脱敏、异步归档和短时下载授权，以及网页接线。原测试失败未修复，前端UI/UX未改；Docker停止并保留卷，最终全范围交接与Git存档待完成。

### 运行日志来源与强制字段投影（2026-09-08）
新增common/logging/runtime-log-source.ts，LoggingModule导出RuntimeLogSource；可选RUNTIME_LOG_DIRECTORY指向服务器采集器提供的绝对只读目录。读取受限.ndjson普通文件，拒绝符号链接/路径越界、畸形或不完整数据，限制1000文件/64MiB源/10万条记录。按日志自带organizationId和组织时区起止日期筛选；字段白名单只保留HTTP时间、请求标识、已知操作、服务器协议推导路由模板、方法、状态、耗时、原结果及错误代码。原始路径、actorUserId、User-Agent、原始payload及其他未知字段均不进入结果；无组织的启动/全局日志不混入组织数据。源摘要/字节统计仅为内部读取证据，不是公开下载地址或完整留存保证。
Docker测试采集真实Nest子进程stdout到容器私有目录，配置LOG_LEVEL=info，保留正式结构化日志流程；未把审计事件列表转作运行日志。docker-runtime-log-source-20260908.txt退出0：43迁移/Nest构建/账号登录通过；发出本组织与跨组织真实HTTP请求，来源读取仅包含本组织、日期匹配记录，模拟敏感query不进入投影；另验证恶意原始路径/身份/密码/Authorization/User-Agent/payload丢弃、路由由已知协议重建、非HTTP日志跳过、未配置来源和非法日期拒绝。222操作parity通过。
覆盖标签固定AVAILABLE_SCOPED_HTTP_RECORDS_ONLY，不能宣称所有服务器/后台/网关日志或完整日期留存已经覆盖。生产CVM采集器、只读挂载、轮转/保留与完整性监测仍需配置；目录未配置时来源不可用。后台无组织日志的安全归属、异步ZIP任务、服务器短时下载授权和网页接线仍待实现。前端UI/UX未改，原失败保留，Docker停止保留数据库/媒体卷；测试原始stdout目录随临时容器结束，不保存为对外归档。完整任务及最终Git存档未完成。
