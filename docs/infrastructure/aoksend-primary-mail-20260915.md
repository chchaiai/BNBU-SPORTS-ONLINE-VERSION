...
### 2026-09-15 14:27 腾讯云正式模板通过，但发送限额阻止验收

- 刷新控制台确认模板 `60326` 审核通过；详情中 `code`、`minutes` 与三张原图 URL 均符合正式模板。
- 香港短生命周期探测容器模拟主通道 HTTP 503，适配器实际调用腾讯云 `60326`，返回 `FailedOperation.ExceedSendLimit`。本次未获接受，不能声称备用当前可用。
- 诊断 deliveryId：`mail-probe-17942801-ad0b-4505-8176-b5f6409d484e`；首次腾讯云 requestId：`644edcdb-d508-47c4-8e87-d421b66e7369`。三次适配器尝试均为相同限额错误，停止继续试发。
- 控制台概览显示最高域名信誉等级 1、单域名单日最高 500 封；该页面未提供本次具体耗尽计数，因此确切限额范围仍需提供商确认。
- 生产主通道仍为 AoKSend 正式模板 `E_154198061747`；备用仍为 `56852`，尚未将新模板启用生产。历史 SES 成功证据不证明本次限额期间仍可接管。
- 待限额恢复后验证 `60326` 收件并将 `TENCENT_SES_TEMPLATE_EXPIRY_MINUTES_VARIABLE=minutes` 与新 ID 一同启用。另仍需未入班测试学校邮箱完成真实绑定。

### 2026-09-15 14:24 正式主模板启用（以下历史条目以本节为准）

- 用户误改内置模板导致重新审核；现场检查发现正式模板 `E_154198061747` 已显示“使用中”，预览图片及 `code`、`minutes` 变量正常。
- 香港当前生产镜像独立探测：`mail-probe-493cf88a-0559-4802-86c4-09c563df868b`，AoKSend HTTP 200 / code 200，124ms，未调用备用。用户确认邮件收到、图片、验证码及“10 分钟”均正常。
- 已通过 `tools/production/activate-approved-mail-template.py` 将当前生产模板设为 `E_154198061747`。本次沿用现场最新发布 `student-email-domain-20260915` 与镜像 `sha256:f67f7a5d177424d0bbd3dde8aac15bea90452e5c90e949b7c98a3a124e5a1e4c`，避免回退其他任务的学生邮箱修复。
- 当前发布目录使用独立 `mail-approved-20260915.env`；生产环境变量复核、容器健康及公开 readiness 均通过。仅更新邮件配置并重建 backend 容器，无镜像替换、数据库迁移。
- 腾讯云正式模板 ID 已确认 `60326`，仍审核中；备用继续使用审核通过的 `56852`。
- 内置模板固定“30 分钟”的问题已随主模板替换消除。首次入班真实绑定回归仍待未入班测试邮箱，腾讯云正式模板替换仍待审核。
- 配置备份位于当前发布目录的 `compose.before-approved-mail-20260915.yml`。该备份会恢复已重新审核的旧 AoKSend 模板，仅能作为配置恢复依据；旧模板恢复可用前不得据此宣称主通道恢复。
...
| 验收项 | 当前证据与结论 |
| --- | --- |
| 正式 HTML 主通道发送、收件、显示 | 14:24 已通过，香港主通道接受，用户确认显示正常；已启用生产 |
