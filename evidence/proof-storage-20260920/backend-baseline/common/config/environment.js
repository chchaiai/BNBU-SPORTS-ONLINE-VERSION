import { ocrConfiguration } from '../../modules/v8/tencent-ocr-provider.js';
import { aiReviewConfiguration } from '../../modules/v8/tencent-ai-review-provider.js';
export const APP_ENVIRONMENTS = ['local', 'test', 'development', 'staging', 'production'];
...
function integer(raw, name, options) {
...
function tencentDbCaFile(raw, appEnvironment) {
...
function allowlist(raw, appEnvironment) {
    const values = required(raw, 'CORS_ALLOWLIST')
        .split(',')
        .map((value) => value.trim())
        .filter((value) => value.length > 0);
    if (values.length === 0)
        throw new Error('CORS_ALLOWLIST must contain at least one origin');
...
function optionalBoolean(raw, name, defaultValue) {
    const value = raw[name];
...
function objectStorage(raw, appEnvironment) {
    const baseNames = [
    ...
    const requiredForEnvironment = appEnvironment === 'production' || optionalBoolean(raw, 'OBJECT_STORAGE_REQUIRED', false);
    const basePresentCount = baseNames.filter((name) => {
        const value = raw[name];
        return typeof value === 'string' && value.trim().length > 0;
    }).length;
    if (basePresentCount === 0 && !requiredForEnvironment)
        return null;
    ...
    if (provider === 'STATIC') {
        if (appEnvironment === 'staging' || appEnvironment === 'production') {
            throw new Error('STORAGE_CREDENTIAL_PROVIDER must be TENCENT_CVM_ROLE for staging and production');
        }
        ...
        if (optionalText(raw.OBJECT_STORAGE_ACCESS_KEY) !== null ||
            optionalText(raw.OBJECT_STORAGE_SECRET_KEY) !== null) {
            throw new Error('Static object storage credentials must be omitted when using TENCENT_CVM_ROLE');
        }
...
function mediaConfiguration(raw, appEnvironment) {
    const storageNames = [
    ...
    const requiredForEnvironment = appEnvironment === 'production' || optionalBoolean(raw, 'MEDIA_STORAGE_REQUIRED', false);
    const names = [...storageNames, ...settingNames];
    const allNames = [...names, 'MEDIA_STORAGE_ACCESS_KEY', 'MEDIA_STORAGE_SECRET_KEY', 'MEDIA_STORAGE_PUBLIC_ENDPOINT'];
    ...
    if (!anyPresent && !requiredForEnvironment)
        return null;
    ...
    if (storage === null)
        throw new Error('Media object storage configuration is required');
    const publicEndpointText = optionalText(raw.MEDIA_STORAGE_PUBLIC_ENDPOINT);
    const publicEndpoint = publicEndpointText === null ? null : objectStorage({ ...storageRaw,
        OBJECT_STORAGE_ENDPOINT: publicEndpointText }, appEnvironment).endpoint;
    ...
    return {
        ...
        scannerMode,
        ...(scannerMode === 'EXTERNAL_REQUIRED' ? {
            scannerHost: required(raw, 'MEDIA_SCANNER_HOST'),
            scannerPort: raw.MEDIA_SCANNER_PORT === undefined ? 3310 : integer(raw, 'MEDIA_SCANNER_PORT', { minimum: 1, maximum: 65535 }),
        } : {}),
...
function pushConfiguration(raw) {
    const names = ['PUSH_TOKEN_ENCRYPTION_KEY', 'PUSH_TOKEN_ENCRYPTION_KEY_VERSION'];
...
function emailDeliveryConfiguration(raw, appEnvironment) {
    const configuredProvider = optionalText(raw.EMAIL_DELIVERY_PROVIDER);
    const coreNames = ['SMTP_HOST', 'SMTP_PORT', 'SMTP_FROM_ADDRESS'];
    const requiredForEnvironment = appEnvironment === 'staging' ||
    ...
    const username = typeof rawUsername === 'string' && rawUsername.trim().length > 0
...
function tencentSesEmailDeliveryConfiguration(raw) {
    ...
    const configuredVariableNames = Object.values(templateVariables).filter((value) => value !== null);
...
function templateVariable(raw, name, isRequired) {
...
export function validateEnvironment(raw) {
    ...
    if (accessTokenTtlSeconds >= refreshTokenIdleTtlSeconds ||
    ...
    const publicOrganizationCode = typeof raw.PUBLIC_ORGANIZATION_CODE === 'string' ? raw.PUBLIC_ORGANIZATION_CODE.trim() : undefined;
    if (publicOrganizationCode !== undefined && !/^[A-Za-z0-9_-]{1,32}$/.test(publicOrganizationCode))
        throw new Error('PUBLIC_ORGANIZATION_CODE must be a nonempty organization code of at most 32 characters');
    ...
    const runtimeConfig = {
        ...
        corsAllowlist: allowlist(raw, appEnvironment),
        ...
        objectStorage: objectStorage(raw, appEnvironment),
        media: mediaConfiguration(raw, appEnvironment),
        push: pushConfiguration(raw),
        emailDelivery: emailDeliveryConfiguration(raw, appEnvironment),
        tencentDbCaFile: tencentDbCaFile(raw, appEnvironment),
    };
