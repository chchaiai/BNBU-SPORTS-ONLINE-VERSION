...
import { validateProofFile, validateApplicationProofFile } from "./proofs.js";
...
export function subscribeNetworkFailure(listener) {
...
export function networkFailureMessage() {
  return globalThis.navigator?.onLine === false
    ? tx("当前无网络连接，请检查网络后重试", "No internet connection. Check your network and try again.")
    : tx("网络连接不稳定，请稍后重试", "Your connection is unstable. Please try again later.");
}
...
export function currentApiUserId() { return readTokens()?.userId ?? null; }
...
export class ClientTransportError extends Error {
  constructor(cause, { method = null, route = null, requestId = uuid() } = {}) {
...
export const MAX_PROOF_VIDEO_SECONDS = 10;        // EXERCISE_RECORD video hard cap
...
function knownApiErrorMessage(error) {
  if (error?.name === 'ProofDraftStorageError') return error.storageReason === 'QuotaExceededError'
    ? tx('本机存储空间不足，凭证暂未保存。请保持页面打开，释放存储空间后重试。', 'Device storage is full and evidence is not saved. Keep this page open, free storage and retry.')
    : tx('浏览器暂时无法保存凭证，请保持页面打开后重试。若持续失败，请提供诊断编号。', 'The browser could not save evidence. Keep this page open and retry. If it continues, provide the diagnostic reference.');
  ...
  if (error.code === 'MEDIA_UPLOAD_FAILED') return tx('照片或视频尚未上传成功，申请或打卡尚未提交。已保留材料，请重试上传。', 'The photo or video was not uploaded, so the application or check-in has not been submitted. Your evidence is retained; retry the upload.');
  if (isUnsupported(error)) return tx("该功能暂未开放。", "This feature is not yet available.");
  if (error.code === "SYSTEM_SERVICE_UNAVAILABLE" && error.details?.currentState === "FEATURE_DEFERRED") {
    return tx("暂时功能无法实现，敬请期待", "This feature is not available yet. Please stay tuned.");
  }
  ...
  const known = {
    ...
    AUTH_VERIFICATION_CODE_INVALID: tx("验证码错误或已过期，请重新获取验证码。", "The verification code is incorrect or expired. Request a new code."),
    ...
    SESSION_OUTSIDE_TIME_WINDOW: tx("当前不在课程开放时段或本人获准的补练时段内。", "Outside the course check-in window or your authorized makeup window."),
    ...
    MEDIA_VERIFICATION_INCOMPLETE: tx("视频已上传，正在处理，可以继续填写其他内容。处理完成后请再次提交。", "Video uploaded and processing. You can continue editing and submit when ready."),
    MEDIA_TRANSITION_NOT_ALLOWED: tx("凭证当前状态不支持该操作。", "This action is not allowed for the proof's current state."),
    MEDIA_FAILURE_NOT_RETRYABLE: tx("视频处理失败，请删除该视频后重新上传。", "Video processing failed. Remove it and upload again.") + (error.details?.mediaId ? tx(` 诊断编号：VID-${error.details.mediaId}`, ` Diagnostic: VID-${error.details.mediaId}`) : ""),
...
const clientDiagnosticIds = new WeakMap();
function clientDiagnosticId(error) {
...
export function toUserFacingError(error, { log = true } = {}) {
  ...
  const storageCodes = {QuotaExceededError:'LOCAL_PROOF_STORAGE_FULL',InvalidStateError:'LOCAL_PROOF_STORAGE_CLOSED',UnknownError:'LOCAL_PROOF_STORAGE_UNAVAILABLE',NotReadableError:'LOCAL_PROOF_FILE_UNREADABLE',DataCloneError:'LOCAL_PROOF_STORAGE_SERIALIZATION'};
  const code = error?.name === 'ProofDraftStorageError' ? (storageCodes[error.storageReason] || 'LOCAL_PROOF_STORAGE_FAILED') : error instanceof ApiError && /^[A-Z][A-Z0-9_]{0,79}$/.test(error.code)
    ? error.code
    : category === "NETWORK"
      ? "NETWORK_UNAVAILABLE"
      : category === "TIMEOUT"
  ...
  const retryable = error?.name === "ProofDraftStorageError" || error?.code === "MEDIA_INTEGRITY_MISMATCH" || category === "NETWORK" || category === "TIMEOUT" ||
    category === "RATE_LIMIT" || category === "SERVER" || category === "CONFLICT";
  const title = accountDeletionTitle || {
    NETWORK: networkFailureMessage(),
    TIMEOUT: networkFailureMessage(),
  ...
  const action = (code === "MEDIA_INTEGRITY_MISMATCH" ? tx("请重试上传；如仍失败，请重新选择或拍摄凭证。", "Retry the upload. If it still fails, select or capture the evidence again.") : null) || accountDeletionAction || {
  ...
  const model = {
    code,
    title,
    message: ["NETWORK", "TIMEOUT"].includes(category) ? networkFailureMessage() : isTimeoutError(error)
  ...
  const swimReason = error instanceof ApiError ? error.details?.reason : null;
  if (error instanceof ApiError && error.details?.fieldErrors?.some(item => item?.code === 'MAJOR_CONFIRMATION_LOCKED')) {
    model.title = tx('专业已确认', 'Major already confirmed');
    model.message = tx('正式专业确认后不能再次自行修改。', 'A confirmed major cannot be changed again by the student.');
    model.action = tx('如填写错误，请联系管理员纠正。', 'Contact an administrator to correct an error.');
    model.retryable = false;
  }
  if (error instanceof ApiError && error.details?.fieldErrors?.some(item => item?.code === 'STUDENT_EMAIL_PREFIX_INVALID')) {
    model.title = tx('邮箱格式不正确', 'Invalid student email');
    model.message = tx('请输入 1 个小写字母 + 9 位数字的学生邮箱前缀。', 'Enter one lowercase letter followed by nine digits.');
    model.action = tx('检查邮箱前缀，并使用 @mail.bnbu.edu.cn 后缀后重试。', 'Check the prefix and use @mail.bnbu.edu.cn, then try again.');
    model.retryable = false;
  }
  const swimMessages = {
    SWIM_INTAKE_REQUIRED: tx('游泳材料尚未完成预受理，请返回凭证页确认运动前后照片后重新提交。', 'Swimming proof has not been accepted. Confirm before/after photos and submit again.'),
    SWIM_BEFORE_AFTER_REQUIRED: tx('游泳需要运动前、运动后照片各至少一张，请按真实拍摄阶段标记。', 'Swimming needs a before and an after photo. Label their actual capture stages.'),
    SWIM_ORIGINAL_BEFORE_AFTER_REQUIRED: tx('缺少本次游泳的运动前或运动后照片，不能用事后拍摄的照片冒充运动前凭证。', 'Original before/after swimming photos are required. A later photo cannot replace before-exercise proof.'),
    SWIM_LOCKED_BATCH_MISMATCH: tx('游泳材料批次已锁定，请使用原批次继续上传，不能替换凭证。', 'The swimming batch is locked. Resume the original files; replacements are not allowed.'),
    SWIM_LOCKED_CONTENT_MISMATCH: tx('当前文件与已受理的游泳材料不一致，请恢复原文件。', 'The file differs from the accepted swimming proof. Restore the original.'),
    SWIM_DELAY_REASON_REQUIRED: tx('游泳材料已超过正常受理或续传时限，请填写真实的延迟原因后提交教师审核。', 'The swimming intake or transfer window has elapsed. Enter the actual delay reason for teacher review.'),
    SWIM_DELAY_WINDOW_EXPIRED: tx('已超过游泳结束后24小时的延迟提交期限，请联系任课教师处理，重复提交不会延长期限。', 'The 24-hour delayed swimming submission window has expired. Contact your teacher.'),
    LOCKED_BATCH_CONTENT_HASH_REQUIRED: tx('无法确认游泳材料的文件完整性，请保留原件并重新准备凭证。', 'Swimming proof integrity could not be confirmed. Keep the originals and prepare the proof again.'),
  };
  if (Object.hasOwn(swimMessages, swimReason)) {
    model.title = tx('游泳凭证提交未完成', 'Swimming proof submission incomplete');
    model.message = swimMessages[swimReason];
    model.action = tx('保留现有凭证，按以上提示处理。', 'Keep your evidence and follow the guidance above.');
    model.retryable = false;
  }
  if (error instanceof ApiError && error.code === "AUTH_VERIFICATION_CODE_INVALID") {
    model.title = tx("验证码校验未通过", "Verification code not accepted");
    model.action = tx("请重新获取验证码后再试。", "Request a new code and try again.");
  }
...
export function apiErrorText(error) {
  ...
  return [
    model.message,
    model.action,
    tx(`错误码：${model.code}`, `Error code: ${model.code}`),
...
// Bound both connection and body reading; aborted writes are never replayed here.
async function fetchApiJson(url, options, context) {
  ...
  try {
    ...
    const error = new ClientTransportError(cause, context);
    for (const listener of networkFailureListeners) listener(error);
    throw error;
...
async function rawRequest(path, {
  ...
  const { response, parsed } = await fetchApiJson(`${apiBaseUrl()}${path}`, {
    method, headers: requestHeaders,
    body: body !== undefined ? JSON.stringify(body) : undefined,
  }, { method, route: path, requestId });
  if (!response.ok) {
...
async function refreshSession(expectedEpoch) {
  ...
  const data = await rawRequest("/auth/refresh", {
    ...
    if (error instanceof ApiError && error.code === 'SYSTEM_MAINTENANCE' &&
        currentApiSessionEpoch() === expectedEpoch && readRefreshIntent()?.idempotencyKey === intent.idempotencyKey) {
      writeRefreshIntent(null);
    }
    throw error;
  });
...
export async function request(path, options = {}) {
  ...
  const requestSessionId = stableOptions.auth === false ? null : readTokens()?.sessionId;
  try {
    return await rawRequest(path, stableOptions);
  } catch (error) {
    if (authSessionEpoch !== requestEpoch || (requestSessionId && requestSessionId !== readTokens()?.sessionId)) throw new Error("API_SESSION_EPOCH_CHANGED");
...
export async function listOwnProofTodos() {
  const items = [], seen = new Set();
  let cursor = null;
  for (let page = 0; page < CURSOR_PAGE_LIMIT; page++) {
    const result = await request(`/student/proof-todos?limit=100${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`);
    if (!Array.isArray(result?.items)) throw invalidSuccessResponse(result, "/student/proof-todos", "GET");
...
export async function getOwnCurrentCourseContract(classSectionId) {
...
// Retired student grade endpoint. Assessment progress comes from /student-progress.
export const listMyScores = () => Promise.resolve([]);
...
export function normalizeSystemModeProjection(projection) {
  const rawMode = typeof projection?.mode === "string"
    ...
    : "";
  if (!["NORMAL", "MAINTENANCE"].includes(rawMode)) {
    throw new ApiError(502, { code: "SYSTEM_MODE_INVALID_RESPONSE" });
  }
  return {
    mode: rawMode,
...
export async function getSystemModeStatus(onAnnouncement) {
  const epoch = currentApiSessionEpoch();
  const value = await request("/system-mode/announcement", { auth: false });
  const status = normalizeSystemModeProjection(value);
  const announcement = status.mode === "MAINTENANCE" ? value?.announcement : null;
  const english = apiLocale() === "en";
  const announcementStatus = { ...status,
    supplementTiming: { kind: status.mode === "MAINTENANCE" && hasApiSession() ? "unavailable" : "noActiveTask" },
    title: announcement ? (english ? announcement.titleEn : announcement.titleZh) : "",
    message: announcement ? (english ? announcement.bodyEn : announcement.bodyZh) : "",
    estimatedRecoveryTime: announcement?.estimatedRecoveryAt || null,
  };
  onAnnouncement?.(announcementStatus);
  let supplementTiming = { kind: "noActiveTask" };
  if (status.mode === "MAINTENANCE" && hasApiSession()) {
    supplementTiming = { kind: "unavailable" };
    try {
      const { items } = await listOwnProofTodos();
      if (isCurrentApiSessionEpoch(epoch)) {
        if (items.length === 0) supplementTiming = { kind: "noActiveTask" };
        else if (items.every(item => item.expired === true)) supplementTiming = { kind: "expiredBeforeMaintenance" };
        else {
          const paused = items.filter(item => item.paused === true && item.expired === false &&
            Number.isSafeInteger(item.remainingSeconds) && item.remainingSeconds >= 0);
          if (paused.length && paused.length === items.filter(item => item.expired !== true).length) supplementTiming = { kind: "paused", serverConfirmedRemainingSeconds: Math.min(...paused.map(item => item.remainingSeconds)) };
        }
      }
    } catch { /* Keep an explicit unavailable state instead of reusing an old countdown. */ }
  }
  return { ...status,
    supplementTiming,
    title: announcement ? (english ? announcement.titleEn : announcement.titleZh) : "",
...
export async function listHelpArticles() {
  ...
  const route = `/student/help-articles?locale=${encodeURIComponent(locale)}`;
...
export async function getOwnRosterStatus(enrollmentId) {
  const route = `/enrollments/${encodeURIComponent(enrollmentId)}/roster-status`;
  const value = await request(route);
  const statuses = ['MATCHED', 'PENDING_REGISTRATION', 'IDENTITY_CONFLICT', 'EXTRA_IN_PLATFORM'];
  const keys = ['enrollmentId', 'classSectionId', 'generatedAt', 'available', 'rosterVersion', 'status', 'registrationComplete'];
  const shape = value && typeof value === 'object' && !Array.isArray(value) &&
    Object.keys(value).length === keys.length && keys.every(key => Object.hasOwn(value, key)) &&
    value.enrollmentId === enrollmentId && typeof value.classSectionId === 'string' && value.classSectionId.length > 0 &&
    typeof value.generatedAt === 'string' && Number.isFinite(Date.parse(value.generatedAt));
  const state = value?.available === false
    ? value.rosterVersion === null && value.status === null && value.registrationComplete === false
    : value?.available === true && Number.isSafeInteger(value.rosterVersion) && value.rosterVersion > 0 &&
      statuses.includes(value.status) && value.registrationComplete === (value.status === 'MATCHED');
  if (!shape || !state) throw invalidSuccessResponse(value, route, 'GET');
  return value;
}
export async function getOwnPhysicalResult(enrollmentId) {
  const route = `/student/enrollments/${encodeURIComponent(enrollmentId)}/physical-result`;
  const value = await request(route);
  const result = value?.result;
  const recorded = value?.status === 'RECORDED' && result && ['800m', '1000m'].includes(result.runType) &&
    Number.isSafeInteger(result.elapsedSeconds) && result.elapsedSeconds >= 0 &&
    typeof result.testedOn === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(result.testedOn);
  if (!recorded && !(['EXEMPT', 'NOT_RECORDED'].includes(value?.status) && result === null))
    throw invalidSuccessResponse(value, route, 'GET');
  return value;
}
export function mapPhysicalResult(value) {
  return { enduranceRunStatus: value.status === 'RECORDED' ? 'recorded' : value.status === 'EXEMPT' ? 'exempt' : 'not_recorded',
    enduranceRunTimeSeconds: value.status === 'RECORDED' ? value.result.elapsedSeconds : null,
    enduranceRunType: value.status === 'RECORDED' ? value.result.runType : null,
    enduranceTestedOn: value.status === 'RECORDED' ? value.result.testedOn : null, enduranceRunScore: null };
}
...
// ── Exercise sessions ────────────────────────────────────────────
export const getServerSession = (sessionId) => request(`/exercise-sessions/${encodeURIComponent(sessionId)}`);
...
export const createRecordDraft = ({ sessionId, creditType, sportType, sportName, description }, idempotencyKey) =>
  request("/exercise-records", {
    ...
    body: {
      ...
      description,
      clientRequestId: idempotencyKey || uuid(),
    },
  });
export const submitRecord = (recordId, mediaIds, expectedVersion, swimDelayReason, idempotencyKey) =>
  request(`/exercise-records/${recordId}/submit`, {
    method: "POST", idempotent: true,
    ...(idempotencyKey ? { headers: { 'Idempotency-Key': idempotencyKey } } : {}),
    body: { mediaIds, expectedVersion, ...(swimDelayReason ? { swimDelayReason } : {}) },
  });
export const getRecordWorkflow = (recordId) =>
  request(`/exercise-records/${encodeURIComponent(recordId)}/workflow`);
export const getSwimIntake = (recordId) =>
  request(`/exercise-records/${encodeURIComponent(recordId)}/swim-intake`);
export const acceptSwimIntake = (recordId, items, expectedVersion, delayReason, idempotencyKey) =>
  request(`/exercise-records/${encodeURIComponent(recordId)}/swim-intake`, {
    method: "POST", idempotent: true,
    ...(idempotencyKey ? { headers: { "Idempotency-Key": idempotencyKey } } : {}),
    body: { items, expectedVersion, ...(delayReason ? { delayReason } : {}) },
  });
export const submitRecordSupplement = (recordId, mediaIds, expectedVersion, idempotencyKey) =>
  request(`/exercise-records/${encodeURIComponent(recordId)}/supplements`, {
    method: "POST", idempotent: true,
    ...(idempotencyKey ? { headers: { "Idempotency-Key": idempotencyKey } } : {}),
...
async function uploadMediaDraftWithProgress(serverSessionId, draft, blob, {prepareOnly, onProgress, onCheckpoint}) {
  ...
  try {
    if (!pending.objectUploaded) {
      const put = await uploadObject(proxyObjectUrl(initiated.uploadUrl), {
        onProgress,
...
export async function uploadExemptionApplicationMediaDraft(enrollmentId, draft, blob, {onProgress = () => {}} = {}) {
  try {
    return await uploadExemptionWithProgress(enrollmentId, draft, blob, onProgress);
  } catch (error) {
    onProgress({phase: error?.code === 'MEDIA_VERIFICATION_INCOMPLETE' ? 'PROCESSING' : 'FAILED'});
    throw error;
  }
...
async function uploadExemptionWithProgress(enrollmentId, draft, blob, onProgress) {
  onProgress({phase:'READING'});
  const verdict = validateApplicationProofFile(blob);
  if (!verdict.ok) {
  ...
  try {
    ...
    ].includes(error.code) || (error instanceof ApiError && error.code === "MEDIA_UPLOAD_FAILED" && error.status === 403)) {
      draft.pendingUpload = null;
      draft.initiateIdempotencyKey = uuid();
    }
    throw error;
 ...
 */
export async function loadServerRecordProofs(recordId, evidenceContext) {
  const context = evidenceContext ?? await getRecordEvidenceContext(recordId);
...
export const verifyJoinEmail = (challengeId, code, joinEmail, joinInviteToken) =>
  request('/auth/student-sign-in-codes/verify', { method: 'POST', auth: false, idempotent: true,
    body: { challengeId, code, joinEmail, joinInviteToken, deviceId: stableDeviceId() } });
...
export const getMyPreferences = () => request("/me/preferences");
export const updateMyPreferences = (input, idempotencyKey) => request("/me/preferences", {
...
export function mapServerRecord(record, { courseIdBySection = {} } = {}) {
  ...
  const reviewText = review
    ? review.result === "VALID"
      ? review.publicComment || (credited > 0 ? tx("记录有效，已计入运动时长。", "Record valid; hours credited.") : tx("记录有效，当前未计入考核进度。", "Record valid; currently not credited toward the target."))
      : review.result === "INVALID"
        ...
        : record.workflowStage === "AWAITING_SUPPLEMENT"
          ? tx("等待补充材料。", "Supplementary evidence required.")
  ...
  return {
    ...
    workflowStage: record.workflowStage || null,
    workflowVersion: record.workflowVersion || null,
    materialVersion: record.materialVersion || null,
    eligibleMinutes: record.eligibleMinutes ?? null,
    creditReason: record.creditReason || null,
...
export function mapServerStudent(me, profile, semester = null, studentStatus = "PENDING") {
  ...
  return {
    ...
    majorConfirmationLocked: profile.majorConfirmationLocked === true,
...
export function mapClassSectionCheckInTimeWindow(section) {
  ...
  return {
    ...
    semesterDeadline: section?.checkInEndDate || null,
  };
}
...
export function mapStudentProgressProjection(progress) {
  ...
  const courseSeconds = Math.max(0, Number(progress.courseRelated?.effectiveSeconds ?? progress.courseRelated?.validExerciseSeconds) || 0);
  const generalSeconds = Math.max(0, Number(progress.general?.effectiveSeconds ?? progress.general?.validExerciseSeconds) || 0);
  return {
...
export async function loadApiWorkspace(preloadedIdentity = null) {
  ...
  for (const section of sections) {
    ...
    if (section.teacherId && activeEnrollments.some((enrollment) => enrollment.classSectionId === section.id) &&
        !Object.hasOwn(teacherCache, section.teacherId)) {
      teacherCache[section.teacherId] = await optionalNotFound(getTeacherById(section.teacherId));
    }
  }

  const courseIdBySection = {};
  const rosterStatusByEnrollment = new Map(await Promise.all(activeEnrollments.map(async enrollment => {
    try {
      const status = await getOwnRosterStatus(enrollment.id);
      if (status.classSectionId !== enrollment.classSectionId) throw invalidSuccessResponse(status, `/enrollments/${enrollment.id}/roster-status`, 'GET');
      return [enrollment.id, status];
    } catch { return [enrollment.id, null]; }
  })));
  const courses = sections.map((section) => {
    ...
    return {
      ...
      name: section.displayName || course?.courseName || fromInvite?.courseName,
      semester: (isCurrent ? semesterDisplayName(semester) : "") || fromInvite?.semesterDisplayName || "",
      teacher: teacherCache[section.teacherId]?.fullName || section.teacherDisplayName || section.teacherName || fromInvite?.teacherDisplayName || "",
      ...
      isCurrent,
      deadline: section.checkInEndDate || "",
  ...
  const [proofTodoPage, contractCourse] = await Promise.all([
    ...
    optionalCapability(getOwnCurrentCourseContract(currentSection?.id)),
  ]);
  ...
  return {
    workspace: {
      ...
      grades: {
        ...
        ...mapPhysicalResult(physicalResult),
      },
      memberships,
