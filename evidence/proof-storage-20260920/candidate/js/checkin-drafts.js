...
const enqueue = work => {
  const result = writes.then(work).catch(cause => {
    const error = new Error('Local proof storage failed', {cause});
    error.name = 'ProofDraftStorageError';
    error.storageReason = ['QuotaExceededError', 'InvalidStateError', 'UnknownError', 'NotReadableError', 'DataCloneError'].includes(cause?.name) ? cause.name : 'Other';
    throw error;
  });
  writes = result.catch(() => {});
  return result;
...
function openDatabase() {
  if (!database) database = new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      db.onversionchange = () => { database = null; db.close(); };
      resolve(db);
    };
...
async function operation(mode, callback, retry = true) {
  ...
  try { return await new Promise((resolve, reject) => {
    ...
    if (!retry || !['InvalidStateError', 'UnknownError'].includes(error?.name)) throw error;
    database = null;
    db.close();
    return operation(mode, callback, false);
  }
}
export const saveProofDraft = (owner, scope, draft) => enqueue(async () => {
  // mediaId is assigned after AVAILABLE verification. Submission still checks
  // ownership and availability on the server; local caching is no longer required.
  if (owner && scope && draft.mediaId) return;
