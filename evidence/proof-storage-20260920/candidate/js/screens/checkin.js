...
import {
  ...
  currentApiSessionEpoch, isCurrentApiSessionEpoch, listMyRecordPage, mapSubmittedRecords,
...
export async function restoreCheckinContinuity(app) {
  ...
  if (app.isApiMode() && local?.phase === 'finished' && local.serverId === scope) {
    try {
      ...
      if (accountId(app) !== owner || app.ui.checkin !== ui || loadSession(owner)?.serverId !== scope) return;
