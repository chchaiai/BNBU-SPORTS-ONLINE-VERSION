...
import { networkFailureMessage } from "../api.js";
...
import { formatLocalRecoveryTime } from "../local-time.js";
...
export function renderMaintenancePage(app) {
  ...
  const localRecovery = formatLocalRecoveryTime(estimatedRecoveryTime, english ? 'en' : 'zh');
  const recovery = localRecovery
    ? tx(`预计恢复时间：${localRecovery}`, `Estimated recovery time: ${localRecovery}`)
  ...
  return `<div class="screen maintenance-page">
    <div class="maintenance-center col" style="gap:14px;align-items:center;text-align:center">
      ...
      <div class="headline-small text-on-surface">${esc(app.state.systemModeStatus.title) || tx("系统维护中", "System maintenance")}</div>
...
export function renderConnectionNotice(app, blocking = false) {
  const error = app.state.connectionError;
  const message = globalThis.navigator?.onLine === false || ["NETWORK", "TIMEOUT"].includes(error?.category)
    ? networkFailureMessage() : tx("服务暂时不可用，请稍后重试", "Service unavailable. Please try again later.");
  const body = `<div class="banner sync-error" role="alert" style="position:relative;z-index:20">
    ${icon("cloud-off", 24)}<div class="banner-text">${esc(message)}</div>
    <button class="text-btn pressable" data-action="root.retryConnection">${tx("重新尝试", "Try again")}</button></div>`;
  return blocking ? `<div class="screen splash-screen"><div class="splash-center">${body}</div></div>` : body;
}
