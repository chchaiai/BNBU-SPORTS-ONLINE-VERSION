...
test('all sports use the shared submission rules',()=>{
 assert.equal(isRealtimeSwim({details:{sportType:'SWIMMING'}}),false);
 assert.equal(isRealtimeSwim({details:{sportType:'swimming'}}),false);
...
test('finished swimming uses shared material controls',()=>{
 ...
 const html=renderCheckIn(app);assert.equal((html.match(/data-change="checkin.swimPhase"/g)||[]).length,0);assert.doesNotMatch(html,/checkin.swimDelay/);assert.doesNotMatch(html,/运动前后照片各至少/);
});
