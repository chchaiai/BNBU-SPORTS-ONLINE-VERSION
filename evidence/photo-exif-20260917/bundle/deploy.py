...
assert os.geteuid()==0
base=Path('/opt/bnbu-sports-production');work=Path('/home/ubuntu/bnbu-photo-exif-20260917')
previous=base/'releases/ai-review-contract-20260917';release=base/'releases/photo-exif-20260917'
...
assert set(gate)=={'js/photo-originals.js','js/screens/checkin.js'}
...
def switch(target):
 link=base/'current-photo-exif-tmp';assert not link.exists() and not link.is_symlink();link.symlink_to(target);os.replace(link,base/'current')
try:
 assert (base/'current').resolve()==previous and snapshot()==containers
 switch(release)
 cache={}
 for name,item in gate.items():
  with urllib.request.urlopen('https://www.student.bnbusports.cn/student/'+name+'?verify=photo-exif-20260917',timeout=25) as response:
