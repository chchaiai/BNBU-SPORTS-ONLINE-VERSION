...
export const localStore = {
  ...
  getHelpArticles(locale, userId) {
    if (typeof userId !== 'string' || !userId) return [];
    const normalized = String(locale || "").toLowerCase().startsWith("en") ? "en" : "zh-CN";
    const articles = read(`helpArticles.${userId}.${normalized}`, []);
    return Array.isArray(articles) ? articles : [];
  },
  setHelpArticles(locale, articles, userId) {
    if (typeof userId !== 'string' || !userId) return;
    const normalized = String(locale || "").toLowerCase().startsWith("en") ? "en" : "zh-CN";
    write(`helpArticles.${userId}.${normalized}`, Array.isArray(articles) ? articles : []);
  },
