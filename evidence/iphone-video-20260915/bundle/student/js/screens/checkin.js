  ...
  if (!preVerdict.ok) {
    if (preVerdict.error === "format" || preVerdict.error === "empty") {
      ...
      rejectWith(type === "image"
        ...
        : tx(`「${name}」超过视频 100MB 上限，请重新录制。`, `“${name}” exceeds the 100MB video limit. Record again.`));
    }
    return;
  }
    ...
    try {
      ...
      if (!intent || intent.recordId !== todo.recordId) {
        ...
        for (const draft of drafts) {
          ...
          mediaIds.push(draft.mediaId || (await uploadMediaDraft(context.sessionId, draft, blob)).mediaId);
        }
