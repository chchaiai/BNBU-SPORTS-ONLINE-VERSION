...
function renderRecordsTab(app) {
  ...
  return `<div class="col" style="gap:14px;padding-bottom:28px">
    ...
    ${app.state.workspace.recordNextCursor ? `<button class="outlined-btn" data-action="checkin.moreRecords">${tx("加载更早记录","Load earlier records")}</button>` : ""}
  </div>`;
}
...
export const checkinActions = {
  "checkin.moreRecords": async (app) => {
    const workspace=app.state.workspace,cursor=workspace.recordNextCursor;
    if(!cursor || checkinState(app).loadingRecords)return;
    checkinState(app).loadingRecords=true;
    try { const page=await listMyRecordPage(cursor);
      ...
      workspace.records.push(...mapSubmittedRecords(page.data,{courseIdBySection}).filter(r=>!ids.has(r.id)));
      workspace.recordNextCursor=page.meta?.pagination?.nextCursor ?? null;
    } catch(error){apiFailureDialog(app,error,tx('记录加载失败','Cannot load records'));}
    finally {checkinState(app).loadingRecords=false;app.render();}
  },
