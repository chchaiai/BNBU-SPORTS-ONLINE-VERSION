...
import { assertCreditableDuration, creditedDuration, normalizeRecordContent, } from '../domain/exercise-record.js';
import { projectExerciseRecord, projectExerciseRecordEvidenceContext, } from './exercise-record-projection.js';
...
let ExerciseRecordsService = class ExerciseRecordsService {
    ...
    ids;
    scores;
    constructor(prisma, idempotency, audit, outbox, cursors, digest, clock, ids, scores) {
        this.prisma = prisma;
    ...
    async list(principal, scope, input) {
        ...
        if (input.aiReview && principal.role !== 'TEACHER')
            this.scopeDenied();
        const filters = {
        ...
        const businessDatePosition = position === null ? null : new Date(`${position.value}T00:00:00.000Z`);
                        ...
                        : [
                            {
                                OR: [
                                    ...
                                    { sportName: { contains: filters.search, mode: 'insensitive' } },
                                    ...(principal.role === 'ADMIN' ? [{ student: { OR: [{ fullName: { contains: filters.search, mode: 'insensitive' } }, { studentNumber: { contains: filters.search, mode: 'insensitive' } }] } }] : []),
                                ],
                            },
                        ]),
        ...
        return pagedResult((await projectV81Records(this.prisma, items, principal.role !== 'STUDENT')).map((row, index) => ({ ...row, ...(principal.role === 'ADMIN' ? { studentName: items[index].student.fullName, studentNumber: items[index].student.studentNumber, className: items[index].classSection.displayName } : {}) })), {
    ...
    async getEvidenceContext(principal, context) {
        this.assertContext(principal, context);
    ...
    async create(principal, input, facts) {
        ...
        return this.idempotency.execute({
            ...
            try {
                ...
                if (existing !== null) {
                    return this.idempotency.failure(new ApplicationError('EXERCISE_RECORD_ALREADY_EXISTS_FOR_SESSION', 409));
                }
    ...
    async update(principal, context, input, facts) {
        ...
        return this.idempotency.execute({
            ...
            const content = normalizeRecordContent({
                creditType: input.creditType ?? current.creditType,
            ...
            return this.idempotency.success(projectExerciseRecord({
                ...updated,
    ...
    async submit(principal, context, input, facts) {
        this.assertStudentContext(principal, context);
        ...
        const result = await this.idempotency.execute({
            ...
            try {
                ...
                if (current.actualDurationSeconds !== current.session.actualDurationSeconds ||
                    current.pausedDurationSeconds !== current.session.pausedDurationSeconds ||
                    current.creditedDurationSeconds !== credit) {
                    return this.idempotency.failure(new ApplicationError('EXERCISE_RECORD_DURATION_NOT_CREDITABLE', 422));
                }
                ...
                if (media.length !== mediaIds.length ||
                    ...
                    return this.idempotency.failure(new ApplicationError('EXERCISE_RECORD_MEDIA_INCOMPLETE', 422));
                }
                let imageCount = 0;
                let videoCount = 0;
                ...
                return this.idempotency.success((await projectV81Records(transaction, [{
                ...
                if (this.isUniqueViolation(error)) {
                    await transaction.$executeRaw `ROLLBACK TO SAVEPOINT exercise_record_submit`;
                    return this.idempotency.failure(new ApplicationError('EXERCISE_RECORD_DUPLICATE_SUBMISSION', 409));
                }
                throw error;
            }
        });
    ...
    async discard(principal, context, input, facts) {
        this.assertStudentContext(principal, context);
    ...
    async withdraw(principal, context, input) {
        this.assertStudentContext(principal, context);
    ...
    async assertSubmissionScope(transaction, record, now) {
        ...
        const makeup = await transaction.$queryRaw `SELECT rules.closing_deadline,
    ...
    async appendEvent(transaction, record, principal, facts, input) {
    ...
    async appendEvidence(transaction, record, principal, facts, input) {
    ...
    async lock(transaction, table, id) {
        if (table === 'exercise_records') {
            await transaction.$queryRaw(Prisma.sql `SELECT id FROM exercise_records WHERE id = ${id}::uuid FOR UPDATE`);
            return;
        }
        if (table === 'exercise_sessions') {
            await transaction.$queryRaw(Prisma.sql `SELECT id FROM exercise_sessions WHERE id = ${id}::uuid FOR UPDATE`);
            return;
        }
        if (table === 'enrollments') {
            await transaction.$queryRaw(Prisma.sql `SELECT id FROM enrollments WHERE id = ${id}::uuid FOR UPDATE`);
            return;
        }
        if (table === 'media_evidence') {
            await transaction.$queryRaw(Prisma.sql `SELECT id FROM media_evidence WHERE id = ${id}::uuid FOR UPDATE`);
            return;
        }
        throw new Error('Unsupported lock table');
    }
    assertContext(principal, context) {
        if (context.organizationId !== principal.organizationId)
            this.scopeDenied();
    ...
    assertStudentContext(principal, context) {
        this.assertStudent(principal);
        this.assertContext(principal, context);
    }
...
ExerciseRecordsService = __decorate([
    Injectable(),
...
export { ExerciseRecordsService };
// Production baseline with scoped evidence-context recovery overlay.