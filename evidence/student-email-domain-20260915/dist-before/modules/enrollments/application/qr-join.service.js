...
import { projectJoinResult } from './enrollment-projection.js';
let QrJoinService = class QrJoinService {
    capabilities;
    invites;
    enrollments;
    identities;
    ...
    ids;
    config;
    constructor(capabilities, invites, enrollments, identities, auth, idempotency, audit, outbox, crypto, digest, clock, ids, config) {
    ...
    async join(context, facts) {
        ...
        const reference = await this.idempotency.execute({
            ...
            if (capability?.organizationId !== context.organizationId ||
            ...
            if (capability.status === 'CONSUMED') {
                if (capability.consumedIdempotencyKeyHash === keyHash &&
                    capability.resultCiphertext !== null &&
                    capability.resultReplayExpiresAt !== null &&
                    capability.resultReplayExpiresAt > now) {
                    return this.idempotency.success({ capabilityId: capability.id }, {
                ...
                return this.idempotency.failure(new ApplicationError('AUTH_JOIN_CAPABILITY_ALREADY_USED', 409));
            }
            if (capability.status === 'EXPIRED' || capability.expiresAt <= now) {
                return this.idempotency.failure(new ApplicationError('AUTH_JOIN_CAPABILITY_EXPIRED', 410));
            }
            if (capability.status !== 'ACTIVE') {
                return this.idempotency.failure(new ApplicationError('AUTH_JOIN_CAPABILITY_INVALID', 401));
            }
            const invite = await this.invites.findById(capability.courseInviteId, transaction);
            const section = await this.invites.lockClassSection(capability.organizationId, capability.classSectionId, transaction);
            if (!invite || !permitsInviteCompletion(invite, capability.issuedAt, now) ||
            ...
            const identity = this.crypto.decrypt('join-identity-snapshot', capability.id, capability.encryptedIdentitySnapshot);
            ...
            if (identity.verificationChallengeId && identity.verifiedEmail) {
                ...
                if (verification?.status !== 'CONSUMED' || verification.organizationId !== capability.organizationId)
            ...
            else if (!owner?.emailVerifiedAt || !owner.primaryEmailNormalized?.includes('bnbu')) {
                throw new ApplicationError('AUTH_REQUIRED', 401);
            }
            ...
            if (existingIdentity && existingIdentity.user.id !== identity.authenticatedUserId)
                throw new ApplicationError('AUTH_REQUIRED', 401);
            let resolved = await this.identities.resolveOrCreate(capability.organizationId, identity, now, transaction);
            ...
            const permanent = await this.enrollments.findForClassStudent(capability.classSectionId, resolved.profile.id, transaction);
            if (permanent !== null) {
                return this.idempotency.failure(permanent.status === 'ACTIVE'
                    ? new ApplicationError('ENROLLMENT_ALREADY_ACTIVE', 409)
                    : new ApplicationError('ENROLLMENT_REJOIN_DISABLED', 409));
            }
            const semesterActive = await this.enrollments.findActiveForSemesterStudent(capability.organizationId, section.semesterId, resolved.profile.id, transaction);
            if (semesterActive !== null) {
                return this.idempotency.failure(new ApplicationError('ENROLLMENT_SEMESTER_CONFLICT', 409));
            }
            ...
            const view = await this.enrollments.findViewById(capability.organizationId, enrollment.id, transaction);
            if (view === null) {
            ...
            const resultReplayExpiresAt = new Date(now.getTime() + this.config.qrJoinSecretReplaySeconds * 1_000);
            const consumed = {
                ...capability,
                status: 'CONSUMED',
                consumedAt: now,
    ...
    async replay(capabilityId, context, keyHash) {
        const capability = await this.capabilities.findById(capabilityId);
        if (capability?.id !== context.capabilityId ||
        ...
        return this.crypto.decrypt('join-result-replay', capability.id, capability.resultCiphertext);
    }
    keyReference(key) {
        return this.digest.digest('idempotency-key-reference', key);
    }
};
QrJoinService = __decorate([
    Injectable(),
...
export { QrJoinService };
//# sourceMappingURL=qr-join.service.js.map