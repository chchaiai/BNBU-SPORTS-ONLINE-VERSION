...
import { AuthCodeDeliveryPort, AuthCodeDeliveryUnavailableError, } from '../client-capabilities/auth-code-delivery.port.js';
const CODE_LENGTH = 6;
...
let EmailVerificationService = class EmailVerificationService {
    ...
    crypto;
    delivery;
    constructor(prisma, idempotency, audit, outbox, digest, clock, ids, crypto, delivery) {
        this.prisma = prisma;
    ...
    async requestChallenge(principal, input, facts) {
        ...
        const reservation = await this.reserveChallenge(principal, user, targetEmail, targetEmailNormalized, mode, input.locale, facts);
        if (reservation.kind === 'REPLAY')
            return reservation.value;
        return this.deliverChallenge(reservation, facts);
    }
    async verifyChallenge(principal, challengeId, input, facts) {
        await this.idempotency.execute({
            ...
            request: {
                challengeId,
                currentCodeDigest: input.currentEmailCode === undefined
                    ? null
            ...
            const currentAccepted = challenge.mode === 'FIRST_BIND'
                ? input.currentEmailCode === undefined
                : input.currentEmailCode !== undefined &&
                    challenge.currentEmailCodeDigest !== null &&
                    this.crypto.verifyCode(this.codeContext('CURRENT', challenge.id), input.currentEmailCode, challenge.currentEmailCodeDigest);
            const newAccepted = this.crypto.verifyCode(this.codeContext('NEW', challenge.id), input.newEmailCode, challenge.newEmailCodeDigest);
            if (!active || !currentAccepted || !newAccepted) {
                ...
                await transaction.emailVerificationChallenge.update({
                    ...
                    data: {
                        status: challenge.expiresAt <= now
                            ? 'EXPIRED'
            ...
            if (!['PENDING_CONTACT_BINDING', 'ACTIVE'].includes(challenge.user.status)) {
                return this.idempotency.failure(new ApplicationError('USER_STATUS_NOT_ACTIVE', 409, {
            ...
            if (occupied !== null) {
                return this.idempotency.failure(new ApplicationError('CONFLICT_RESOURCE_ALREADY_EXISTS', 409, {
                    resourceType: 'VERIFIED_EMAIL',
                }));
            }
            ...
            await this.audit.append(transaction, {
                ...
                requestId: facts.requestId,
                idempotencyKeyReference: this.digest.digest('idempotency-key-reference', facts.idempotencyKey ?? ''),
                outcome: 'SUCCEEDED',
                safeMetadata: {
    ...
    reserveChallenge(principal, user, targetEmail, targetEmailNormalized, mode, locale, facts) {
        ...
        return this.idempotency.reserveStage({
            ...
            const currentEmailCode = mode === 'REBIND' ? this.crypto.generateNumericCode(CODE_LENGTH) : null;
            ...
            await transaction.emailVerificationChallenge.create({
                data: {
                    ...
                    currentEmailCodeDigest: currentEmailCode === null
                        ? null
                        : this.crypto.digestCode(this.codeContext('CURRENT', challengeId), currentEmailCode),
                    newEmailCodeDigest: this.crypto.digestCode(this.codeContext('NEW', challengeId), newEmailCode),
    ...
    async deliverChallenge(owner, facts) {
        try {
    ...
    async failDelivery(owner, error) {
        return this.idempotency.completeStage(owner, async (transaction) => {
            await transaction.emailVerificationChallenge.update({
                where: { id: owner.value.challengeId },
            ...
            return this.idempotency.failure(new ApplicationError('SYSTEM_SERVICE_UNAVAILABLE', 503, {
                reason: error instanceof AuthCodeDeliveryUnavailableError
                    ? 'EMAIL_DELIVERY_NOT_CONFIGURED'
                    : 'EMAIL_DELIVERY_FAILED',
            }));
        });
    }
    codeContext(target, challengeId) {
