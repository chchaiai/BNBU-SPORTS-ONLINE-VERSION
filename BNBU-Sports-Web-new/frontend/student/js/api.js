...
export async function uploadMediaDraft(serverSessionId, draft, blob) {
  ...
  try {
    ...
    if (error instanceof ApiError && (["MEDIA_UPLOAD_SESSION_EXPIRED", "MEDIA_INTEGRITY_MISMATCH", "MEDIA_VIDEO_DURATION_EXCEEDED", "MEDIA_AUDIO_TRACK_REQUIRED", "MEDIA_LOCATION_METADATA_NOT_ALLOWED"].includes(error.code) || (error.code === "MEDIA_UPLOAD_FAILED" && error.status === 403))) {
      draft.pendingUpload = null;
      draft.initiateIdempotencyKey = uuid();
    }
    throw error;
