...
/** Browser identification is a compatibility policy, not an authentication boundary. */
export function supportsStudentBrowser(info = globalThis.navigator) {
  const ua = String(info?.userAgent || '');
  // Embedded apps and other Chromium products also advertise Chrome/Safari.
  if (/MicroMessenger|WeChat|QQBrowser|MQQBrowser|\bQQ\/|UCBrowser|UCWEB|SamsungBrowser|HuaweiBrowser|HUAWEI.*Browser|HeyTapBrowser|OppoBrowser|VivoBrowser|MiuiBrowser|Quark|baidubrowser|BaiduBoxApp|AlipayClient|DingTalk|Lark|Feishu|FBAN|FBAV|Instagram|Line\/|DuckDuckGo|YaBrowser|Vivaldi|OPR\/|Opera|; wv\)|\bwv\b/i.test(ua) || info?.brave) return false;
  if (/\b(?:Edg|EdgA|EdgiOS)\/\d+/i.test(ua)) return true;
  if (/\b(?:Firefox|FxiOS)\/\d+/i.test(ua)) return true;
  if (/Version\/\d+.*Safari\//i.test(ua) && !/Chrome|Chromium|CriOS/i.test(ua)) return true;
  const brands = info?.userAgentData?.brands;
  if (Array.isArray(brands) && brands.length) {
    return brands.some(item => item.brand === 'Google Chrome' || item.brand === 'Microsoft Edge');
  }
  return /\b(?:Chrome|CriOS)\/\d+/i.test(ua) && !/Version\/\d+.*Chrome\//i.test(ua);
}
