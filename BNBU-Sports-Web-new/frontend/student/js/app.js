...
export const app = {
  ...
  applySystemModeStatus(status) {
    ...
    const previousStatus = JSON.stringify(this.state.systemModeStatus);
    const previouslyChecked = this.state.systemModeChecked;
    if (!["NORMAL", "MAINTENANCE"].includes(status.mode)) return;
    const hadConnectionError = this.state.connectionError !== null;
    this.state.connectionError = null;
    this.state.systemMode = status.mode;
    this.state.systemModeChecked = true;
    this.state.systemModeStatus = {
      mode: this.state.systemMode,
      supplementTiming: status.supplementTiming ?? { kind: "unavailable" },
      title: status.title || "",
      message: status.message || "",
      estimatedRecoveryTime: status.estimatedRecoveryTime || null,
    ...
    // A healthy poll must not replace an active camera, file picker or form.
    if (hadConnectionError || !previouslyChecked || previousMode !== this.state.systemMode ||
        previousStatus !== JSON.stringify(this.state.systemModeStatus)) this.render();
  },
    ...
    this._systemModeRefresh = getSystemModeStatus((status) => this.applySystemModeStatus(status))
      ...
      .catch((error) => {
        if (error.code === "SYSTEM_MAINTENANCE") {
          ...
          this.state.connectionError = toUserFacingError(error, { log: false });
          this.render();
        }
      })
  ...
  _connectionRetry: null,
  async retryConnection() {
    if (this._connectionRetry) return this._connectionRetry;
    this._connectionRetry = (async () => {
      if (this._systemModeRefresh) await this._systemModeRefresh;
      await this.refreshSystemMode();
      if (!this.state.connectionError && this.state.systemMode === "NORMAL" &&
          this.state.authenticated && !this.state.isLoading) await this.reloadApiWorkspace();
    })().finally(() => { this._connectionRetry = null; });
    return this._connectionRetry;
  },
  showNetworkFailure(error) {
    this.state.connectionError = error ? toUserFacingError(error, { log: false }) :
      { category: "NETWORK", title: networkFailureMessage() };
    this.render();
  },
  hasActiveEnrollment() {
  ...
  async markAllNoticesRead() {
    ...
    for (const notice of unread) {
      if (!(await this.markNoticeRead(notice.id))) complete = false;
    }
    return complete;
  },
  ...
  async reloadApiWorkspace(preloadedIdentity = null) {
    ...
    const generation = this.workspaceGeneration = (this.workspaceGeneration || 0) + 1;
    const epoch = currentApiSessionEpoch();
    const current = () => this.workspaceGeneration === generation && isCurrentApiSessionEpoch(epoch);
    ...
    try {
      ...
      if (!current()) return false;
      const savedExercise = localStore.getExerciseSession(workspace.student.id);
      if (restoringExercise && ["active", "paused", "finished"].includes(savedExercise?.phase)) this.state.tab = "checkin";
      ...
      if (this.workspaceGeneration !== generation) return false;
  ...
  logout({ clearAccountData = false } = {}) {
    ...
    if (clearAccountData && accountId) {
      void clearProofDrafts(this.state.workspace.student.id).catch(() => {});
      localStore.clearAccountData(this.state.workspace.student.id);
  ...
  async setAppLanguage(lang) {
    if (!this.isApiMode()) { this.applyAppLanguage(lang); this.render(); return; }
    if (this.ui.preferenceSaving) return;
    const epoch = currentApiSessionEpoch();
    const locale = lang === "en" ? "en" : "zh-CN";
    const pending = this.ui.preferenceIntent;
    if (pending && pending.input.locale !== locale) {
      this.state.lastError = { code: "PREFERENCE_PENDING", category: "NETWORK", retryable: true,
        message: tx("上一笔语言设置结果未确认，请重试原语言。", "The previous language update is unconfirmed. Retry the original language."),
        action: tx("确认原请求后再选择其他语言。", "Confirm the original request before selecting another language."), requestId: null, fieldErrors: [] };
      this.ui.preferenceError = this.state.lastError;
      this.render(); return;
    }
    this.ui.preferenceSaving = true;
    try {
      const current = this.state.preferences || await getMyPreferences();
      if (!isCurrentApiSessionEpoch(epoch)) return;
      const intent = pending || { input: { locale, pushEnabled: current.pushEnabled,
        emailEnabled: current.emailEnabled, expectedVersion: current.version }, key: crypto.randomUUID() };
      this.ui.preferenceIntent = intent;
      const saved = await updateMyPreferences(intent.input, intent.key);
      if (!isCurrentApiSessionEpoch(epoch)) return;
      this.state.preferences = saved;
      this.ui.preferenceIntent = null;
      this.ui.preferenceError = null;
      this.state.lastError = null;
      this.applyAppLanguage(saved.locale === "en" ? "en" : "zh");
    } catch (error) {
      if (!isCurrentApiSessionEpoch(epoch)) return;
      if (error?.code === "CONFLICT_VERSION_MISMATCH" || error?.code === "VALIDATION_FAILED") {
        this.ui.preferenceIntent = null;
        this.state.preferences = null;
      }
      this.state.lastError = toUserFacingError(error);
      ...
      if (isCurrentApiSessionEpoch(epoch)) { this.ui.preferenceSaving = false; this.render(); }
    }
  },
  ...
  renderAuthContent() {
    ...
    if (s.authenticated) {
      ...
      if(this.isApiMode() && s.workspace?.student?.profileQualityStatus==='REQUIRES_PROFILE_UPDATE') return renderRequiredProfile(this);
  ...
  render() {
    ...
    } else if (!s.systemModeChecked && s.connectionError) {
      content = renderConnectionNotice(this, true);
    } else {
    ...
    if (s.connectionError && s.systemModeChecked) content = `<div class="system-frame">${renderConnectionNotice(this)}<div class="system-frame-body">${content}</div></div>`;
  ...
  start(rootElement) {
    ...
    subscribeProfileCorrection(() => {if(this.state.authenticated && !this.state.isLoading) void this.reloadApiWorkspace();});
    subscribeNetworkFailure((error) => this.showNetworkFailure(error));
    globalThis.addEventListener("offline", () => this.showNetworkFailure());
    globalThis.addEventListener("online", () => { void this.retryConnection(); });
    subscribeSystemMaintenance(() => {
      this.applySystemModeStatus({ ...this.state.systemModeStatus, mode: "MAINTENANCE" });
    ...
    if (remoteSystemModeEnabled) {
      ...
      globalThis.document?.addEventListener?.("visibilitychange", () => {
        if (globalThis.document.visibilityState === "visible") {
          void this.refreshSystemMode();
          void resumeCheckinContinuity(this);
        }
      });
    }
  },
