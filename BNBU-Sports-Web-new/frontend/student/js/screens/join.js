import { isStudentSchoolEmail, isNewStudentSchoolEmail } from "../student-school-email.js";
...
import { requestStudentSignInCode, verifyJoinEmail, previewInvite, previewCourseInvitation, joinWithInvite, storeJoinContext, ApiError, toUserFacingError } from "../api.js";
...
const inviteApiLimitHint = () =>
  tx("邀请有效期由教师自定义，默认 30 分钟。到期前已登记的同一次流程可有 10 分钟宽限，且不得刷新续期。", "Invitation duration is set by the teacher (default 30 minutes). A registration already started before expiry may use one 10-minute grace period that cannot be refreshed.");
...
function loadQrDecoder() {
  ...
  decoderLoading ??= new Promise((resolve, reject) => {
    ...
    script.onerror = () => { decoderLoading = null; reject(new Error('QR_DECODER_UNAVAILABLE')); };
...
async function startCamera(app) {
  ...
  if (!video) return;
  if (mediaStream) { video.srcObject = mediaStream; void video.play().catch(() => {}); return; }
  ...
  try {
    ...
    ui.permission = "granted";
    {
      let detector = null;
      try {
        if ('BarcodeDetector' in globalThis) detector = new globalThis.BarcodeDetector({formats:['qr_code']});
      } catch { /* Use the bundled decoder when the platform lacks QR support. */ }
      ...
      detectTimer = setInterval(async () => {
        const video = app._viewport?.querySelector('.scan-video');
        if (ui.resolving || busy || !video?.videoWidth || generation !== cameraGeneration) return;
        busy = true;
        try {
          let value;
          if (detector) {
            try { value = (await detector.detect(video))[0]?.rawValue; }
            catch { detector = null; }
          }
          if (!value && context) {
            ...
            value = decode(context.getImageData(0,0,canvas.width,canvas.height).data,canvas.width,canvas.height)?.data;
          }
          if (value && value !== ui.lastScannedValue) {
            ui.lastScannedValue = value;
            resolveQrValue(app, value);
          }
        } catch { /* frame not ready */ } finally { busy = false; }
      }, 400);
    }
  } catch {
    stopCamera();
...
function joinConfirmState(app, params) {
  if (!app.ui.joinConfirm) {
    ...
    app.ui.joinConfirm = {
      ...
      email: request ? request.email : (app.state.workspace?.student?.email || ""),
      emailVerified: Boolean(app.state.authenticated && app.state.workspace?.student?.emailVerified),
      emailCode: "", challengeId: null, joinEmailProof: null, verifyingEmail: false,
...
export function renderCourseJoinConfirm(app, params) {
  ...
  const emailStep = !params.correctionRequest && params.course?.real && !ui.emailVerified;
  const emailContent = `<div class="col" style="gap:16px">
    <p>${tx('先验证学校邮箱，再填写或确认学生信息，最后正式入班。', 'Verify your school email, confirm your student details, then join the course.')}</p>
    <label>${tx('学校邮箱', 'School email')}<input class="text-field" type="email" autocomplete="email" value="${esc(ui.email)}" data-input="joinConfirm.field" data-field="email" ${ui.verifyingEmail ? 'disabled' : ''} placeholder="y123456789@mail.bnbu.edu.cn"></label>
    <button class="tonal-btn" data-action="joinConfirm.sendEmail" ${ui.verifyingEmail ? 'disabled' : ''}>${tx('发送验证码', 'Send code')}</button>
    ${ui.challengeId ? `<label>${tx('验证码', 'Verification code')}<input class="text-field" inputmode="numeric" autocomplete="one-time-code" value="${esc(ui.emailCode)}" data-input="joinConfirm.field" data-field="emailCode"></label>
    <button class="primary-btn" data-action="joinConfirm.verifyEmail" ${ui.verifyingEmail ? 'disabled' : ''}>${tx('验证邮箱并继续', 'Verify email and continue')}</button>` : ''}
    ${ui.error ? scanMessage(ui.error) : ''}
  </div>`;
  const identityContent = ui.submitted
    ...
    : `<div class="col" style="gap:14px">
        ...
        ${joinField({ id: "studentNumber", label: tx("学号（必填）", "Student ID (required)"), value: ui.studentNumber, supporting: `${ui.studentNumber.length} / ${MAX_STUDENT_NUMBER}`, disabled: ui.submitting || !writeEnabled, maxlength: MAX_STUDENT_NUMBER, error: ui.invalidField === "studentNumber" ? tx("学号须为 2 开头的 10 位数字。", "Student ID must contain 10 digits and start with 2.") : null })}
        ${course && course.real
        ...
        + (allowsCustomMajor(ui.collegeName) ? joinField({id:'majorName',label:tx('专业（大写字母或未分流）','Major (uppercase letters or 未分流)'),value:ui.majorName,disabled:!formEnabled,maxlength:200}) + `<button type="button" data-action="joinConfirm.undeclaredMajor" ${!formEnabled?'disabled':''}>${tx('选择未分流','Choose undeclared')}</button>`
        : joinSelect({id:'majorName',label:tx('专业','Major'),value:ui.majorName,options:academicMajorOptions(ui.collegeName).map(value=>({value,label:value==='未分流'?tx('未分流','Undeclared'):value})),disabled:!formEnabled || !ui.collegeName}))
        + ['dateOfBirth'].map((field) => joinField({
          id: field, label: tx('出生年月日','Date of birth'),
        ...
        })).join('') + joinSelect({ id: 'regionCode', label: tx('生源地','Region'), value: ui.regionCode,
          options: studentRegionOptions(), disabled: !formEnabled,
          error: ui.invalidField === 'regionCode' ? tx('请选择生源地。','Select your region.') : null }) + (ui.regionCode === 'OTHER' ? joinField({id:'otherRegionName',label:tx('国家或地区名称','Country or region name'),value:ui.otherRegionName || '',disabled:!formEnabled,maxlength:100,error:ui.invalidField === 'otherRegionName' ? tx('请填写具体国家或地区。','Enter your country or region.') : null}) : '') : ''}
    ...
    <div class="screen-scroll" data-scroll-key="join-confirm">
      <div class="col" style="gap:16px;padding:${params.preLogin ? "24px" : "0 4px 24px"}">
        ...
        <div class="swiss-panel"><div class="col" style="gap:14px">
          ...
          <div class="body-medium text-muted">${tx("验证学校邮箱后填写或确认学生信息，再正式入班，无需教师审批。", "Verify your school email, enter or confirm your student details, then join. Teacher approval is not required.")}</div>
        </div></div>
        ${sectionTitle(emailStep ? tx("验证学校邮箱", "Verify school email") : tx("填写/确认学生信息", "Enter / confirm student details"))}
        <div class="swiss-panel">${emailStep ? emailContent : identityContent}</div>
      </div>
    </div>
  </div>`;
}
...
export const joinActions = {
  "joinConfirm.undeclaredMajor": (app) => { const ui=app.ui.joinConfirm;if(ui&&!ui.submitting&&app.isWriteAllowed()){ui.majorName='未分流';app.render();} },
  // — Scan —
  "scan.back": (app) => {
    stopCamera();
    app.ui.scan = null;
  ...
  "joinConfirm.sendEmail": async (app) => {
    ...
    if (!isNewStudentSchoolEmail(ui.email)) {
      ui.error = tx('邮箱格式不正确，请输入 1 个小写字母 + 9 位数字的学生邮箱前缀，并使用 @mail.bnbu.edu.cn。', 'Use one lowercase letter and nine digits before @mail.bnbu.edu.cn.'); app.render(); return;
    }
    ...
    try { const result = await requestStudentSignInCode(ui.email.trim(), invite); ui.challengeId = result.challengeId; ui.challengeEmail = ui.email.trim(); }
    catch (error) { ui.error = toUserFacingError(error); }
    finally { ui.verifyingEmail = false; app.render(); }
  },
  "joinConfirm.verifyEmail": async (app) => {
    const ui = app.ui.joinConfirm;
    if (!ui || ui.verifyingEmail || !ui.challengeId || !app.isWriteAllowed()) return;
    const invite = app.state.authenticated ? app.state.subParams?.inviteCode : app.state.pendingInvite?.code;
    ui.verifyingEmail = true; ui.error = null; app.render();
    try {
      const result = await verifyJoinEmail(ui.challengeId, ui.emailCode.trim(), ui.email.trim(), invite);
      if (!result?.joinEmailProof) throw new Error('JOIN_EMAIL_PROOF_MISSING');
      ui.joinEmailProof = result.joinEmailProof; ui.emailVerified = true;
      if (result.profile) {
        const profile = result.profile;
        ui.name = profile.fullName; ui.studentNumber = profile.studentNumber;
        for (const key of ['gender','gradeYear','collegeName','majorName','dateOfBirth','regionCode','otherRegionName']) ui[key] = String(profile[key] ?? '');
      }
    } catch (error) { ui.error = toUserFacingError(error); }
    finally { ui.verifyingEmail = false; app.render(); }
  },
  ...
  "joinConfirm.select": (app, el) => {
    ...
    if (el.dataset.field === 'collegeName') { ui.majorName = ''; app.render(); }
  ...
  "joinConfirm.submit": (app) => {
    ...
    if (realJoin && !ui.emailVerified) { app.render(); return; }
    ui.invalidField = !name || name.length > MAX_NAME
      ? "name"
      : !/^2[0-9]{9}$/.test(studentNumber)
        ? "studentNumber"
        : realJoin && !ui.gender
          ? "gender"
          : realJoin && !ui.gradeYear
            ...
            : !realJoin && email && !isStudentSchoolEmail(email)
              ...
              : null;
    ui.error = null;
    if (realJoin && !ui.invalidField && !STUDENT_COLLEGES.some(item=>item.code===ui.collegeName)) ui.invalidField='collegeName';
    if (realJoin && !ui.invalidField && !validAcademicDetails(ui.collegeName,ui.majorName.trim())) { ui.invalidField='majorName'; ui.error=tx('请选择对应学院的专业；自行填写的专业只能包含大写英文字母。','Select a major for your college; custom majors require uppercase letters only.'); }
    ...
    if (realJoin) {
      ...
      joinWithInvite(params.inviteCode, {
        ...(ui.joinEmailProof ? { joinEmailProof: ui.joinEmailProof } : {}),
        fullName: name,
        studentNumber,
        gender: ui.gender,
