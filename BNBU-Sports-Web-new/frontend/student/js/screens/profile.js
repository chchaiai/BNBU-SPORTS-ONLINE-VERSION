...
const profileFields = [
  ...
  ['regionCode', 'regionCode', () => tx('生源地','Region')],
];
...
function profileCompletionNotice(app) {
  ...
  if (app.isApiMode() && !missing.length && student.major === '未分流' && student.majorConfirmationLocked !== true) {
    return `<div class="swiss-panel" role="status"><p>${tx('当前专业为未分流。确定正式专业后，可在此确认一次。','Your major is undeclared. You can confirm your formal major here once.')}</p><button class="primary-btn pressable" data-action="profile.editDetails">${tx('确认专业','Confirm major')}</button></div>`;
  }
...
export function profileCompletionForm(app) {
  ...
  if (!state) return '';
  const majorLocked = app.state.workspace.student.majorConfirmationLocked === true;
  ...
  return `<form class="swiss-panel col" style="gap:16px" data-profile-details-form data-submit="profile.saveDetails">
    ...
    <label class="col">${tx('学院','College')}<select class="text-field" required data-input="profile.detailsField" data-field="collegeName" ${state.busy||majorLocked?'disabled':''}><option value="">${tx('请选择','Select')}</option>${STUDENT_COLLEGES.map(item=>`<option value="${item.code}" ${state.collegeName===item.code?'selected':''}>${esc(item.code+' '+item.name)}</option>`).join('')}</select></label>
    ${majorLocked ? `<p>${esc(state.majorName)} · ${tx('专业已确认，如需纠正请联系管理员。','Your major is confirmed. Contact an administrator for corrections.')}</p>` : allowsCustomMajor(state.collegeName) ? field('majorName',tx('专业（大写字母或未分流）','Major (uppercase letters or 未分流)')) + `<button type="button" data-action="profile.undeclaredMajor">${tx('选择未分流','Choose undeclared')}</button>` : `<label class="col">${tx('专业','Major')}<select class="text-field" required data-input="profile.detailsField" data-field="majorName" ${state.busy?'disabled':''}><option value="">${tx('请选择','Select')}</option>${academicMajorOptions(state.collegeName).map(value=>`<option value="${value}" ${state.majorName===value?'selected':''}>${value==='未分流'?tx('未分流','Undeclared'):value}</option>`).join('')}</select></label>`}
    ${field('dateOfBirth',tx('出生年月日','Date of birth'),'date')}
    <label class="col">${tx('生源地','Region')}<select class="text-field" required data-input="profile.detailsField" data-field="regionCode" ${state.busy?'disabled':''}><option value="">${tx('请选择','Select')}</option>${studentRegionOptions().map(option=>`<option value="${option.value}" ${state.regionCode===option.value?'selected':''}>${esc(option.label)}</option>`).join('')}</select></label>
    ${state.regionCode==='OTHER'?field('otherRegionName',tx('国家或地区名称','Country or region name'),'text',100):''}
    ${state.error?(typeof state.error==='object'?userFacingErrorPanel(state.error,{compact:true}):`<p role="alert" class="text-error">${esc(state.error)}</p>`):''}
    ...
    <div class="screen-scroll" data-scroll-key="account-details">
      <div class="col" style="gap:16px">
        ...
        <div class="swiss-panel"><div class="col" style="gap:14px">
          ...
          ${accountDetailRow(tx("生源地", "Region"), (student.regionCode === "OTHER" ? student.otherRegionName || studentRegionLabel(student.regionCode) : studentRegionLabel(student.regionCode)))}
...
export const profileActions = {
  "profile.undeclaredMajor": (app) => { if(app.ui.profileDetails && !app.ui.profileDetails.busy && !app.state.workspace.student.majorConfirmationLocked){app.ui.profileDetails.majorName='未分流';app.render();} },
  ...
  "profile.detailsField": (app,el) => {
    ...
    if (app.state.workspace.student.majorConfirmationLocked && ['collegeName','majorName'].includes(el.dataset.field)) { app.render(); return; }
  ...
  "profile.saveDetails": async (app) => {
    ...
    try {
      ...
      app.ui.profileDetails=null;
    } catch(error) {state.error=toUserFacingError(error); if(error.status===409) {await app.reloadApiWorkspace(); state.expectedVersion=app.state.workspace.student.profileVersion;} app.ui.profileDetails=state;}
