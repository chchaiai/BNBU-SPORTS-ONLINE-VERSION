...
test('short and exactly ten-second clips pass while actual over-limit and unknown durations fail',()=>{
 ...
 for(const durationSeconds of [10.01,12,null,NaN,Infinity])assert.equal(validateVideoDraftDuration(file,durationSeconds,true).error,'duration');
 assert.equal(validateVideoDraftDuration(file,15,false).ok,true);
 assert.equal(validateProofFile(file,'video',{durationSeconds:15}).ok,true);
});
