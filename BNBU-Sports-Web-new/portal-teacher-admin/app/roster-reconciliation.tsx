...
function importErrorMessage(error: unknown) {
  ...
  const messages: Record<string, string> = {
    UNSUPPORTED_FILE_TYPE: currentApiRequestMode() === "demo" ? "文件格式错误，请上传 .xlsx、.xls 或 .csv 文件。" : "文件格式错误，请上传 .xlsx 或 .csv 文件；旧版 .xls 请先另存为 .xlsx。",
  ...
  if (!bundle.currentRoster) {
    return (
      <section className="roster-reconciliation-page">
        ...
        {importOpen && <RosterImportDialog course={course} currentBundle={bundle} onClose={() => { setImportOpen(false); if (!isDemo) void load(); }} onImported={(next) => { setBundle(next); setImportOpen(false); showToast(isDemo ? "导入成功并完成对齐；数据仅保存在当前前端演示会话。" : "后端已创建并验证新的官方名单版本；请点击“运行核对”生成结果。"); }} />}
      </section>
    );
  }
  ...
  return (
    <section className="roster-reconciliation-page">
      ...
      <section className="roster-stat-grid" aria-label="名单对齐概览">
        <RosterStatCard label="官方名单总人数" value={stats.officialTotal ?? "核对中"} selected={statusFilter === "ALL"} onClick={() => selectStatus("ALL")} />
              ...
              <tbody>{pageRows.map((result) => {
                ...
                return <tr key={result.id} className={result.status === RosterReconciliationStatus.MATCHED ? "is-matched-row" : ""}>
                  ...
                  <td className="action-column"><TableActionMenu iconOnly label={`处理 ${student.name}`}>
                    <TableActionMenuItem disabled={reconciling} onClick={() => setDetail(result)}>查看差异详情</TableActionMenuItem>
                  </TableActionMenu></td>
                </tr>;
              })}</tbody>
            </DataTable>
      ...
      {importOpen && <RosterImportDialog course={course} currentBundle={bundle} onClose={() => { setImportOpen(false); if (!isDemo) void load(); }} onImported={(next) => { setBundle(next); setImportOpen(false); showToast(isDemo ? "导入成功并完成对齐；数据仅保存在当前前端演示会话。" : "后端已创建并验证新的官方名单版本；请运行核对生成最新结果。"); }} />}
    </section>
  );
}
...
function RosterImportDialog({ course, currentBundle, onClose, onImported }: { course: RosterCourseReference; currentBundle: RosterReconciliationBundle; onClose: () => void; onImported: (bundle: RosterReconciliationBundle) => void }) {
  ...
  const chooseFile = async (file: File | undefined) => {
    ...
    setValidation(null);
    try {
      if (!isDemo && !/\.(xlsx|csv)$/i.test(file.name)) throw new Error("UNSUPPORTED_FILE_TYPE");
  ...
  const confirmImport = async () => {
    if (!parsed || !mapping || !validation || !canSubmitRoster(validation, isDemo)) return;
    setImporting(true);
    setError("");
    try {
  ...
  return createPortal(<div className="modal-backdrop roster-import-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !importing && onClose()}>
    <section className="modal teacher-dialog teacher-dialog-wide roster-import-dialog" role="dialog" aria-modal="true" aria-labelledby="roster-import-title">
      ...
      <div className="teacher-dialog-body">
        {!isDemo && <OcrImportPanel courseId={course.id} purpose="ROSTER" />}
        ...
        {validation && parsed && <>
          ...
          <aside className="roster-mock-notice"><AlertTriangle size={18} aria-hidden="true" /><p>{isDemo ? "当前使用前端 Mock Service。确认后会在本次浏览器会话中保存名单并自动重新对齐，不会修改学校服务器或真实学生成员关系。" : "确认后会把原始文件和字段映射提交到后端，完成校验和名单确认；导入不会直接增删课程成员。"}</p></aside>
        </>}
        {error && <p className="form-error roster-import-error" role="alert">{error}</p>}
      </div>
      <div className="modal-footer"><button className="secondary-button" type="button" disabled={importing} onClick={onClose}>取消导入</button>{parsed && !validation && <button className="primary-button" type="button" onClick={validate}>本地预检</button>}{validation && <button className="primary-button" type="button" disabled={importing || !canSubmitRoster(validation, isDemo)} onClick={() => void confirmImport()}>{importing ? isDemo ? "正在导入并对齐" : "正在提交后端" : "确认创建新版本"}</button>}</div>
    </section>
  </div>, document.body);
}
