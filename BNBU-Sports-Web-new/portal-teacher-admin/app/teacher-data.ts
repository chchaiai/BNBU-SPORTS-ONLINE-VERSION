...
async function fetchTeacherSemesters():Promise<Semester[]> {
  const result:Semester[]=[],seen=new Set<string>();let after:string|null=null;
  do{const page:{items:Semester[];nextCursor:string|null}=await request('/teacher/semesters?limit=100'+(after?'&after='+encodeURIComponent(after):''));
    result.push(...page.items);after=page.nextCursor;if(after&&seen.has(after))throw new Error('SEMESTER_PAGINATION_INVALID');if(after)seen.add(after);
  }while(after);return result;
}
...
export async function reviewExemptionApplication(
  ...
  body: ReviewExemptionApplicationBody & {courseMinutes?:number;generalMinutes?:number},
  idempotencyKey?: string,
...
export type TeacherGradeView = {
  finalGrade?: number;
  physicalResultVersion?: number;
  physicalTestedOn?: string;
  physicalRunType?: '800m' | '1000m';
  id: string;
  studentId: string;
  courseId: string;
  enrollmentId: string;
...
export type TeacherExemptionView = {
  ...
  mediaIds: string[];
  submittedAt: string;
  status: "pending" | "supplement_required" | "approved" | "rejected" | "revoked";
  reviewComment?: string;
  version: number;
};
...
export function mapClassSectionToCourse(
  ...
  return {
    ...
    courseTarget: progressTarget ? progressTarget.courseTargetSeconds / 3600 : 0,
    otherTarget: progressTarget ? progressTarget.generalTargetSeconds / 3600 : 0,
    progressConfigured: progressTarget !== null,
...
export async function loadTeacherCourses(): Promise<{
  ...
  const [sections, catalog, semester, ownSemesters] = await Promise.all([
    fetchClassSections(),
    fetchCourses(),
  ...
  const semestersById=new Map(ownSemesters.map(item=>[item.id,item]));
  ...
  for (const section of sections) {
    ...
    const progressTarget = await fetchClassProgressTarget(section.id).catch((error: unknown) => {
      // A new teaching section has no published target until its rules are published.
      if (error instanceof ApiError && error.status === 404) return null;
      throw error;
    });
    const ownSemester=semestersById.get(section.semesterId);
    if(!ownSemester)throw new Error('COURSE_SEMESTER_PROJECTION_MISSING');
    courses.push(mapClassSectionToCourse(ownSemester.status==='ARCHIVED'?{...section,status:'ARCHIVED'}:section, course, semesterDisplayName(ownSemester,semesterLabel), progressTarget));
  }
...
function reviewToAuditStatus(record: ExerciseRecord): AuditStatus {
  if (record.workflowStage === "AWAITING_SUPPLEMENT") return "supplement";
  if (record.workflowStage === "TECHNICAL") return "technical";
  if (record.workflowStage === "PENDING_AI" || record.workflowStage === "PENDING_TEACHER") return "pending";
...
export function mapExerciseRecordToCheckin(
  ...
  const durationMinutes = Math.floor((record.actualDurationSeconds || 0) / 60);
  const creditedMinutes = Math.floor(
  ...
  return {
    ...
    // Submission is an instant, not the business date used for daily rules.
    // Preserve its timezone until the view formats it exactly once.
    submittedAt: record.submittedAt || "",
    status: auditStatus === "valid" ? "有效"
      : auditStatus === "invalid" ? "已调整"
      : auditStatus === "supplement" ? "待补证"
      : auditStatus === "technical" ? "技术处理中" : "待审核",
    risk: null,
    confidence: null,
    ...
    auditRemark: record.currentReview?.publicComment ?? undefined,
...
export type PhysicalResultRevision = { version: number; runType: '800m' | '1000m'; elapsedSeconds: number; testedOn: string; createdAt: string };
export async function fetchPhysicalResultHistory(enrollmentId: string, beforeVersion?: number) {
  return request<{ items: PhysicalResultRevision[]; nextBeforeVersion: number | null }>(
    `/enrollments/${encodeURIComponent(enrollmentId)}/physical-results?limit=100${beforeVersion === undefined ? '' : `&beforeVersion=${beforeVersion}`}`);
}
export function withPhysicalResult(grade: TeacherGradeView, latest: PhysicalResultRevision | undefined, exempt: boolean): TeacherGradeView {
  return { ...grade, enduranceStatus: exempt ? 'Exempt' : latest ? 'Recorded' : 'NotRecorded',
    minutes: !exempt && latest ? Math.floor(latest.elapsedSeconds / 60) : undefined,
    seconds: !exempt && latest ? latest.elapsedSeconds % 60 : undefined,
    physicalResultVersion: latest?.version ?? 0,
    physicalTestedOn: !exempt ? latest?.testedOn : undefined,
...
export async function loadTeacherGrades(
  ...
  return Promise.all(selected.map(async student => {
    ...
    const latest = grades.items[0];
    const accounting = progressByEnrollment.get(student.enrollmentId);
    const grade: TeacherGradeView = {
      id: 'final:' + student.enrollmentId,
      ...
      validCourseDurationSeconds: accounting?.courseRelated.effectiveSeconds,
      validGeneralDurationSeconds: accounting?.general.effectiveSeconds,
      totalValidDurationSeconds: accounting?.totalEffectiveSeconds,
      qualificationStatus: accounting ? (accounting.status === 'COMPLETED' ? 'QUALIFIED' : 'NOT_QUALIFIED') : undefined,
    };
...
export type RecognitionRevision={id:string;applicationId:string;revisionNumber:number;courseSeconds:number;generalSeconds:number;reason:string;active:boolean;createdAt:string};
export function fetchRecognitionRevisions(applicationId:string):Promise<RecognitionRevision[]>{
  return fetchAllPages<RecognitionRevision>(`/activity-certification-applications/${encodeURIComponent(applicationId)}/recognition-allocation-revisions`,new URLSearchParams());
}
