...
export async function saveEnduranceRule(input: EnduranceRuleInput, currentState?: AdminState) {
  if (adminDataMode === 'real') {
    if (!currentState) throw new AdminServiceError('VALIDATION', 'ENDURANCE_RULE_NOT_FOUND');
    const { writeRealEnduranceRule } = await import('./endurance-api');
    return writeRealEnduranceRule(currentState, input);
  }
...
export async function deleteEnduranceRule(id: string, currentState?: AdminState, selectedVersion?: number) {
  if (adminDataMode === 'real') {
    if (!currentState) throw new AdminServiceError('VALIDATION', 'ENDURANCE_RULE_NOT_FOUND');
    const { writeRealEnduranceRule } = await import('./endurance-api');
    return writeRealEnduranceRule(currentState, id, selectedVersion);
  }
  return mutate("admin.config.write", (state) => {
...
export { listTeacherAccounts as listAssociatedTeacherProfiles } from './teacher-import-api';
...
type UnifiedAuditEvent = Omit<AuditLogProjection, 'outcome'> & {source:'FOUNDATION'|'V81';outcome:string|null};
const auditEventProjection = (event:UnifiedAuditEvent):AuditLogProjection => ({...event,
...
export async function listAuditLogProjections(
  ...
  Object.entries(filters).forEach(([name, value]) => {
    if (value) query.set(name==='q'?'requestId':name, value);
  });
  const response = await request<{items:UnifiedAuditEvent[];nextCursor:string|null;timezone:string}>(
    `/admin/audit-events?${query.toString()}`,
  );
  return {
    items: response.items.map(auditEventProjection),
...
export async function getAuditLogProjection(auditLogId: string) {
  const [source,id] = auditLogId.split(':');
  if(!['FOUNDATION','V81'].includes(source)||!id)throw new Error('INVALID_AUDIT_REFERENCE');
  return auditEventProjection(await request<UnifiedAuditEvent>(
    `/admin/audit-events/${source}/${encodeURIComponent(id)}`));
}

export type RuntimeLogArchiveJob = {id:string;status:string;version:number;startDate:string;endDate:string;
  byteLength:number|null;sha256:string|null;failureCode:string|null;coverage:string};
export type RuntimeLogArchiveDownload = {downloadUrl:string;expiresAt:string};

export function requestRuntimeLogArchive(filters: AuditLogQueryFilters, key: string) {
  return request<RuntimeLogArchiveJob>("/admin/runtime-archives", {
    method: "POST",
...
export function getRuntimeLogArchiveJob(exportId: string) {
  return request<RuntimeLogArchiveJob>(
    `/admin/runtime-archives/${encodeURIComponent(exportId)}`,
  );
}

export function createRuntimeLogArchiveDownload(exportId: string, expectedVersion:number) {
  return request<RuntimeLogArchiveDownload>(
    `/admin/runtime-archives/${encodeURIComponent(exportId)}/download-url`,
    {
      method: "POST",
...
export async function fetchRuntimeArchive(job:RuntimeLogArchiveJob,download:RuntimeLogArchiveDownload) {
  const prefix=`/api/v1/admin/runtime-archives/${job.id}/content?capability=`;
  if(!download.downloadUrl.startsWith(prefix))throw new Error('INVALID_ARCHIVE_DOWNLOAD_PATH');
  const blob=await requestRuntimeArchiveZip(download.downloadUrl.slice('/api/v1'.length));
  if(blob.size!==job.byteLength)throw new Error('ARCHIVE_SIZE_MISMATCH');
  ...
  if(hash!==job.sha256)throw new Error('ARCHIVE_HASH_MISMATCH');
  return blob;
}
