...
const SAFE_USER_FIELD_NAMES = new Set([
  ...
  "checkInStartDate", "checkInEndDate", "regularDeadline", "closingDeadline", "courseTarget", "generalTarget",
]);
...
function safeFieldErrors(error: unknown, locale: "zh" | "en"): UserFacingFieldError[] {
  ...
  return error.details.fieldErrors.slice(0, 20).map((item) => {
    ...
    const messages: Record<string, [string,string]> = {
      CLASS_SECTION_DATE_RANGE_INVALID: ['打卡起止日期须按先后填写，并位于本学期内。','Set the check-in dates in order and within the semester.'],
      CLASS_SECTION_TIME_RANGE_INVALID: ['每日结束时间必须晚于开始时间。','The daily end time must follow the start time.'],
      CLASS_SECTION_EXCLUDED_DATE_INVALID: ['排除日期必须位于本学期的打卡日期范围内。','Excluded dates must fall within the semester check-in window.'],
      COURSE_DEADLINES_INVALID: ['常规提交截止后须保留完整 7 天收尾期，且收尾结束不得超过学期结束日期。请提前常规截止日期。','Allow seven full closing days after the regular deadline, within the semester. Move the regular deadline earlier.'],
      COURSE_PLAN_NOT_COMPLETABLE: ['当前可运动日期、每日时段或每周次数不足以完成目标，请增加可用时段或降低目标后再发布。','The available dates, daily windows or weekly limit cannot cover the targets. Increase availability or reduce the targets.'],
    };
    const known = typeof value.code === 'string' ? messages[value.code] : undefined;
    return {
      ...
      message: known ? known[locale === 'en' ? 1 : 0] : locale === "en" ? "Check this field and try again." : "请检查此字段后重试。",
    };
  });
}
...
// Non-secret account scope for pending UI requests. Never persist tokens with the request.
export const apiSessionUserId = (): string | null => readTokens()?.userId ?? null;
...
async function refreshSession(expectedEpoch: number): Promise<void> {
  ...
  const session = await rawRequest<AuthSessionData>("/auth/refresh", {
    ...
    // A definitive maintenance rejection did not rotate the token. A later
    // recovery must create a new intent instead of replaying the saved denial.
    if (error instanceof ApiError && error.code === 'SYSTEM_MAINTENANCE' &&
        authSessionEpoch === expectedEpoch && readRefreshIntent()?.idempotencyKey === intent.idempotencyKey) {
      writeRefreshIntent(null);
    }
    throw error;
  });
...
async function requestWithRefresh<T>(
  ...
  try {
    ...
    const canRefresh =
      ...
      readTokens() !== null;
    const rejectedProof = options.credentialProof && error instanceof ApiError &&
      error.status === 401 && error.code === "AUTH_CREDENTIAL_INVALID";
    if (canRefresh && isTerminalAccessTokenFailure(error) && !rejectedProof) {
      clearApiSession();
      throw error;
    }
...
/** Authenticated private archive download; capabilities never enter diagnostic routes. */
export function requestRuntimeArchiveZip(path: string): Promise<Blob> {
  if(!/^\/admin\/runtime-archives\/[0-9a-f-]{36}\/content\?capability=[A-Za-z0-9_-]{43}$/i.test(path))
    return Promise.reject(new Error('INVALID_ARCHIVE_DOWNLOAD_PATH'));
  const route=path.split('?')[0],epoch=authSessionEpoch;
  return requestWithRefresh(async()=>{
    if(apiRequestMode==='demo')throw new LocalReviewApiBlockedError('GET',route);
    if(authSessionEpoch!==epoch)throw new AuthSessionSupersededError();
    const tokens=readTokens();if(!tokens)throw new ApiError(401,{code:'AUTH_REQUIRED'});
    let response:Response;
    try { response=await fetch(`${apiBaseUrl()}${path}`,{headers:{Authorization:`Bearer ${tokens.accessToken}`},redirect:'error',cache:'no-store'}); }
    catch(error){throw new ClientTransportError(error,{method:'GET',route});}
    if(!response.ok)throw new ApiError(response.status,await response.json().catch(()=>null),{method:'GET',route});
    if(response.headers.get('content-type')?.split(';')[0]!=='application/zip')throw new ApiError(502,{code:'SYSTEM_INVALID_RESPONSE'});
...
export function getAccountSecurity(): Promise<AccountSecurityData> {
  return request<AccountSecurityData>("/auth/account-security");
}
...
export function changeOwnPassword(
  ...
  Pick<AccountSecurityData, "userId" | "version" | "mustChangePassword">
> {
  return request("/auth/own-password", {
    credentialProof: true,
    method: "POST",
    body: input,
