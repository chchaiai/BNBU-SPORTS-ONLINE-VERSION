...
type TargetProps={locale:AdminLocale;notificationTarget?:NotificationTarget|null};
...
function TicketDialog({
  ...
  const submit = async () => {
    if (onSave) {
      ...
      if (!reply.trim() || reply.trim().length > 2000 || status === "pending") {
        setSaveError(locale === "zh" ? "请填写 1–2000 字的公开回复。" : "Enter a public reply of 1–2000 characters."); return;
      }
  ...
  return (
    <AdminDialog locale={locale} title={`${ticket.id} · ${adminLabel(locale, "ticketCategory", ticket.category)}`} description={formatAdminDate(locale, ticket.submittedAt, true)} close={close} dirty={Boolean(reply)} wide footer={<>
      ...
      <button className="primary-button" type="button" disabled={saving || busyKey === key} onClick={() => void submit()}>{saving || busyKey === key ? adminCopy(locale, "processing") : adminCopy(locale, "save_ticket")}</button>
    </>}>
      ...
      {error?.userFacingError
        ...
        : <AdminInlineError message={saveError || error?.message} />}
    </AdminDialog>
  );
}
...
export function AdminSupport({ locale,notificationTarget }: TargetProps) {
  const { mode } = useAdminStore();
  return mode === "real" ? (
    <RealFeedbackSupport locale={locale} notificationTarget={notificationTarget} />
  ) : (
    <DemoAdminSupport locale={locale} />
  );
}

function RealFeedbackSupport({ locale,notificationTarget }: TargetProps) {
  const [items, setItems] = useState<(FeedbackItem & {publicReply?: string | null})[]>([]);
  const [selected, setSelected] = useState<FeedbackDetail | null>(null);
  const intent = useRef<{id: string; status: Exclude<TicketStatus, "pending">; reply: string; version: number; signature: string; key: string} | null>(null);
  const listItems = async () => {
    const all: FeedbackItem[] = [];
    for (let page = 1; ; page++) {
      const result = await listAdminFeedback({page}); all.push(...result.items);
  ...
  const open = async (id: string) => {
    ...
    try {
      if (intent.current) {
        ...
        await handleAdminFeedback(pending.id, pending.status, pending.reply, pending.version, pending.key);
        intent.current = null;
      }
      setSelected(await getAdminFeedback(id));
    }
    catch (failure) { setLoadError(adminApiErrorText(failure, locale)); }
  };
  useEffect(()=>{
    if(notificationTarget?.targetType!=='FEEDBACK')return;
    let cancelled=false;const epoch=currentApiSessionEpoch();setSelected(null);setLoadError('');
    void getAdminFeedback(notificationTarget.targetId).then(detail=>{
      if(!cancelled&&epoch===currentApiSessionEpoch())setSelected(detail);
    }).catch(failure=>{if(!cancelled&&epoch===currentApiSessionEpoch())setLoadError(adminApiErrorText(failure,locale));});
    return()=>{cancelled=true;};
  },[notificationTarget,locale]);

  const save = async (status: Exclude<TicketStatus, "pending">, reply: string) => {
    if (!selected) return;
    const signature = JSON.stringify([selected.id, selected.version, status, reply]);
    if (intent.current && intent.current.signature !== signature)
      throw new Error(locale === "zh" ? "上次提交结果尚未确认，请先用原状态和回复重试。" : "Retry the original status and reply to confirm the previous submission.");
    intent.current ??= {id: selected.id, status, reply, version: selected.version, signature, key: crypto.randomUUID()};
    try {
      const result = await handleAdminFeedback(selected.id, status, reply, selected.version, intent.current.key);
      setItems(current => current.map(item => item.id === selected.id ? {...item, ...result} : item));
      intent.current = null;
    } catch (failure) {
      if (failure instanceof ApiError && failure.status >= 400 && failure.status < 500 && failure.status !== 408) {
        intent.current = null;
        if (failure.status === 409) setSelected(await getAdminFeedback(selected.id));
      }
      throw failure;
    }
  };
