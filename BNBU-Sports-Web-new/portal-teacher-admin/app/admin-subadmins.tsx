...
export function AdminSubadmins({ locale, mode }: { locale: AdminLocale; mode: WorkspaceMode }) {
  ...
  const deleteRequest = useRef<{id:string;version:number;key:string}|null>(null);
  const [statusCandidate, setStatusCandidate] = useState<{ account: StoredSubAdmin; key: string } | null>(null);
  const [identity, setIdentity] = useState<IdentityChallenge | null>(null);
  const [identityCode, setIdentityCode] = useState('');
  const identityRequest = useRef<{ email: string; key: string } | null>(null);
  const verifyRequest = useRef<{ code: string; version: number; key: string } | null>(null);
  const createRequest = useRef<string | null>(null);
  const updateRequest = useRef<{ fingerprint: string; key: string } | null>(null);

  useEffect(() => {
    let cancelled = false;
    const frame = window.requestAnimationFrame(() => {
      if (mode === "demo") setAccounts(readStoredAccounts());
      else {
        setAccounts([]);
        void listSubadminAccounts().then(rows => {
          if (!cancelled) setAccounts(rows.map(row => ({ ...row, department: row.department ?? '',
            permissions: toSubadminRoutes(row.permissions) })));
        }).catch(() => {
          if (!cancelled) setError(locale === 'en' ? 'Unable to load administrator accounts. Please reload.' : '无法加载分管理员账号，请刷新重试。');
        });
      }
    });
  ...
  const openCreate = () => {
    identityRequest.current = null; verifyRequest.current = null; createRequest.current = null;
    setIdentity(null); setIdentityCode('');
    setError("");
  ...
  const openEdit = (account: StoredSubAdmin) => {
    identityRequest.current = null; verifyRequest.current = null; createRequest.current = null; updateRequest.current = null;
    setIdentity(null); setIdentityCode('');
    setError("");
    setShowPassword(false);
    setEditor({
  ...
  const save = async () => {
    ...
    if (editor.mode === 'create' && editor.permissions.length === 0) {
    ...
    setBusy(true);
    try {
      const original = editor.mode === 'update' ? accounts.find(item => item.id === editor.id) : undefined;
      if (mode === 'real' && editor.mode === 'create') {
        createRequest.current ??= window.crypto.randomUUID();
        const result = await createSubadminAccount({ email, identityVerifiedByAdmin: true,
          account, name, department, permissions: editor.permissions,
          initialPassword: editor.initialPassword, confirmPassword: editor.confirmInitialPassword }, createRequest.current);
        setAccounts(current => [...current.filter(item => item.id !== result.id), { ...result,
          department: result.department ?? '', permissions: toSubadminRoutes(result.permissions) }]);
        setEditor(null); setNotice(locale === 'en' ? 'Created' : '创建成功');
        return;
      }
      if (mode === 'real' && original?.email.toLowerCase() !== email.toLowerCase()) {
        if (identityRequest.current?.email !== email) identityRequest.current = { email, key: window.crypto.randomUUID() };
        const challenge = await requestSubadminIdentity(email, locale, identityRequest.current.key);
        setIdentity(challenge); setIdentityCode(''); setError('');
        return;
      }
      if (mode === 'real' && editor.mode === 'update') {
        await saveRealProfile(editor); return;
      }
  ...
  const saveRealProfile = async (input: UpdateEditorState, identityChallengeId?: string) => {
    const original = accounts.find(item => item.id === input.id);
    if (!original?.version) throw new Error('SUBADMIN_VERSION_REQUIRED');
    const body = { expectedVersion: original.version, name: input.name.trim(), department: input.department.trim(),
      email: input.email.trim(), permissions: input.permissions, ...(identityChallengeId ? { identityChallengeId } : {}) };
    const fingerprint = JSON.stringify({ id: input.id, ...body });
    if (updateRequest.current?.fingerprint !== fingerprint) updateRequest.current = { fingerprint, key: window.crypto.randomUUID() };
    const result = await updateSubadminAccount(input.id, body, updateRequest.current.key);
    setAccounts(current => current.map(item => item.id === result.id ? { ...result, department: result.department ?? '',
      permissions: toSubadminRoutes(result.permissions) } : item));
    setIdentity(null); setIdentityCode(''); setEditor(null);
    setNotice(locale === 'en' ? 'Updated' : '更新成功');
  };

  const verifyAndCreate = async () => {
    if (!identity || !editor || busy) return;
    if (identity.status !== 'VERIFIED' && !/^\d{6}$/.test(identityCode)) {
      setError(locale === 'en' ? 'Enter the six-digit email code.' : '请输入邮箱收到的六位验证码。'); return;
    }
    setBusy(true); setError('');
    try {
      let verified = identity;
      if (identity.status !== 'VERIFIED') {
        if (verifyRequest.current?.code !== identityCode || verifyRequest.current.version !== identity.version)
          verifyRequest.current = { code: identityCode, version: identity.version, key: window.crypto.randomUUID() };
        verified = await verifySubadminIdentity(identity, identityCode, verifyRequest.current.key);
        setIdentity(verified);
      }
      if (editor.mode === 'update') { await saveRealProfile(editor, verified.id); return; }
      createRequest.current ??= window.crypto.randomUUID();
      const account = await createSubadminAccount({ identityChallengeId: verified.id,
        account: editor.account.trim(), name: editor.name.trim(), department: editor.department.trim(),
        permissions: editor.permissions, initialPassword: editor.initialPassword,
        confirmPassword: editor.confirmInitialPassword }, createRequest.current);
      setAccounts(current => [...current.filter(item => item.id !== account.id), { ...account,
        department: account.department ?? '', permissions: toSubadminRoutes(account.permissions) }]);
      setIdentity(null); setIdentityCode(''); setEditor(null);
      setNotice(locale === 'en' ? 'Created' : '创建成功');
    } catch (cause) {
      if (cause instanceof ApiError && typeof cause.details.actualVersion === 'number') {
  ...
  const toggleStatus = (id: string) => {
    ...
    if (mode !== "demo") {
      ...
      if (account) { setError(''); setStatusCandidate({ account, key: window.crypto.randomUUID() }); }
      return;
    }
    const now = new Date().toISOString();
  ...
  const confirmStatus = async () => {
    ...
    const { account, key } = statusCandidate;
    if (!account.version) return;
    setBusy(true);
    setError('');
    try {
      const result = await setSubadminStatus({ id: account.id, expectedVersion: account.version,
        status: account.status === 'ACTIVE' ? 'DISABLED' : 'ACTIVE',
        ...(account.status === 'ACTIVE' ? { handoverCompleted: true } : {}) }, key);
      setAccounts(current => current.map(item => item.id === result.id ? { ...item, ...result } : item));
      setStatusCandidate(null);
      setNotice(locale === 'en' ? 'Status updated' : '状态已更新');
    } catch {
      setError(locale === 'en' ? 'Status update could not be confirmed. Retry or reload to check the latest status.' : '暂时无法确认状态更新结果，请重试或刷新核对最新状态。');
    } finally { setBusy(false); }
  };
  ...
  const deleteAccount = async () => {
    ...
    if (mode === 'real') {
      if (!deleteCandidate.version) return;
      if (deleteRequest.current?.id !== deleteCandidate.id || deleteRequest.current.version !== deleteCandidate.version)
        deleteRequest.current = {id:deleteCandidate.id,version:deleteCandidate.version,key:window.crypto.randomUUID()};
      setBusy(true); setError('');
      try {
        const result = await deleteSubadminAccount(deleteCandidate.id,deleteCandidate.version,deleteRequest.current.key);
        setAccounts(current=>current.filter(item=>item.id!==result.id));
        setDeleteCandidate(null); setNotice(locale==='en'?'Deleted':'删除成功');
      } catch { setError(locale==='en'?'Deletion could not be confirmed. Retry or reload to check.':'暂时无法确认删除结果，请重试或刷新核对。'); }
      finally { setBusy(false); }
      return;
    }
    ...
    <div className="admin-page-stack admin-subadmins-page">
      {mode === "real" && (
        <aside className="admin-readonly-banner" role="note">
          ...
          <b>{locale === "en"
            ? "The super administrator confirms identity when creating an account. Email changes require a verification code."
            : "创建时由总管理员确认人员身份；修改邮箱仍需验证码核验。"}</b>
        </aside>
      )}
        ...
        <AdminSectionHeading
          ...
          action={
            <div className="admin-subadmin-heading-actions">
              ...
              <button className="primary-button" type="button" disabled={busy} onClick={openCreate}>
      ...
      {deleteCandidate && (
        <AdminConfirm
          ...
          description={locale === "en" ? "Confirm the exact account and that all responsibilities have been handed over." : "请核对准确账号，并确认已完成全部职责移交。"}
          close={() => { if (!busy) setDeleteCandidate(null); }}
          confirm={() => { void deleteAccount(); }}
          busy={busy}
        ...
        >
          <div className="admin-subadmin-delete-confirmation">
            ...
            <p>
              {locale === "en"
                ? mode === 'real' ? "This deletes the login account and current credentials. Completed business and audit history remain available." : "This removes the account from the current Mock preview."
                : mode === 'real' ? "删除将清除登录账号和当前凭据，已完成的业务记录和审计历史继续保留。" : "删除后，该账号会从当前 Mock 预览中移除。"}
            </p>
      ...
      {statusCandidate && (
        <AdminConfirm locale={locale} busy={busy}
          title={statusCandidate.account.status === 'ACTIVE' ? (locale === 'en' ? 'Disable sub-administrator' : '停用分管理员') : (locale === 'en' ? 'Enable sub-administrator' : '启用分管理员')}
          description={statusCandidate.account.status === 'ACTIVE'
            ? (locale === 'en' ? 'Confirm that responsibility handover is complete. Disabling revokes active sessions.' : '请确认业务职责已完成交接。停用后，现有登录会话将失效。')
            : (locale === 'en' ? 'Enable this account with its existing permissions.' : '按现有权限重新启用该账号。')}
          close={() => { if (!busy) setStatusCandidate(null); }} confirm={() => { void confirmStatus(); }}>
          <div className="admin-confirm-object"><b>{statusCandidate.account.name}</b><code>{statusCandidate.account.account}</code><span>{statusCandidate.account.email}</span></div>
          {error ? <p className="admin-inline-error" role="alert">{error}</p> : null}
        </AdminConfirm>
      )}
      ...
      {identity && editor && (
        <AdminConfirm locale={locale} busy={busy}
          title={locale === 'en' ? 'Verify administrator email' : '核验管理员邮箱'}
          description={identity.email}
          close={() => { if (!busy) { setIdentity(null); setIdentityCode(''); identityRequest.current = null; verifyRequest.current = null; createRequest.current = null; } }}
          confirm={() => { void verifyAndCreate(); }}
          confirmLabel={editor.mode === 'create' ? (locale === 'en' ? 'Verify and create' : '核验并创建') : (locale === 'en' ? 'Verify and save' : '核验并保存')}>
          <AdminField locale={locale} label={locale === 'en' ? 'Email verification code' : '邮箱验证码'} required>
            <input inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={identityCode}
              disabled={busy || identity.status === 'VERIFIED'} onChange={event => setIdentityCode(event.target.value)} />
          </AdminField>
          {error ? <p className="admin-inline-error" role="alert">{error}</p> : null}
        </AdminConfirm>
      )}
      ...
      {editor && !identity && (
        <AdminDialog
          ...
          description={
            editor.mode === "create"
              ? locale === "en"
                ? "Before creating, confirm that you have verified this person's identity and reviewed all assigned permissions. Creation does not send an email code."
                : "创建前请确认已完成人员身份核验，并核对全部业务权限。创建时不发送邮箱验证码。"
              : locale === "en"
