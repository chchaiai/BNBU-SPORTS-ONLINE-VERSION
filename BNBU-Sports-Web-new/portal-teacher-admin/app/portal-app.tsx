...
> = {
  ...
  admin: {
    ...
    system: {
      title: "系统模式",
      eyebrow: "系统维护",
      description: "在正常和维护模式之间切换；每次变更都写入审计日志。",
    },
    help: {
      title: "帮助中心",
      eyebrow: "内容管理",
...
export function PortalApp() {
  ...
  const [firstPasswordVersion, setFirstPasswordVersion] = useState<number | null>(null);
  const firstPasswordIntent = useRef<{ key: string; input: { currentPassword: string; newPassword: string; confirmPassword: string; expectedVersion: number } } | null>(null);
  ...
  const refreshNotificationBadge = useCallback(async () => {
    if (!role || workspaceMode !== "real") { setHasUnreadNotification(false); return; }
    const epoch = currentApiSessionEpoch();
    try {
      const notices = await request<unknown[]>("/notifications?unreadOnly=true&limit=1");
      if (currentApiSessionEpoch() === epoch) setHasUnreadNotification(Array.isArray(notices) && notices.length > 0);
    } catch { if (currentApiSessionEpoch() === epoch) setHasUnreadNotification(false); }
  }, [role, workspaceMode]);
  useEffect(() => {
    void refreshNotificationBadge();
    if (!role || workspaceMode !== "real") return;
    const interval = window.setInterval(() => void refreshNotificationBadge(), 60000);
    return () => window.clearInterval(interval);
  }, [refreshNotificationBadge, role, workspaceMode]);
  ...
  const submitLogin = async () => {
    ...
    try {
      ...
      if (security.mustChangePassword) {
        setFirstPasswordVersion(security.version);
        firstPasswordIntent.current = null;
        setRecoveryCode("");
        setRecoveryPassword("");
  ...
  useEffect(() => {
    ...
    if (IS_LOCAL_REVIEW_ENTRY && workspaceMode === null && !hasApiSession()) {
      return;
    }
    ...
    const unsubscribe = subscribeSystemMaintenance(() => {
      if (!cancelled) {
        // A denied business request must not erase an already loaded notice.
        setSystemModeStatus(current => current.mode === "MAINTENANCE" && current.announcement
          ? current : blockedSystemModeStatus());
        void refresh();
      }
    });
    return () => {
  ...
  const returnToLogin = () => {
    ...
    if (firstPasswordVersion !== null) clearApiSession();
    setFirstPasswordVersion(null);
    firstPasswordIntent.current = null;
    setRecoveryCode("");
    setRecoveryPassword("");
    setRecoveryPasswordConfirmation("");
  ...
  const submitFirstPassword = async () => {
    if (recoveryBusy || firstPasswordVersion === null) return;
    ...
    try {
      ...
      if (security.mustChangePassword) {
        const input = { currentPassword: recoveryCode, newPassword: recoveryPassword,
          confirmPassword: recoveryPasswordConfirmation, expectedVersion: security.version };
        const previous = firstPasswordIntent.current;
        if (!previous || JSON.stringify(previous.input) !== JSON.stringify(input)) {
          firstPasswordIntent.current = { key: crypto.randomUUID(), input };
        }
        const intent = firstPasswordIntent.current!;
        await changeOwnPassword(intent.input, intent.key);
      }
      if (epoch !== currentApiSessionEpoch()) return;
      const current = await getMe();
      if (epoch !== currentApiSessionEpoch()) return;
      const user = workspaceUserFromCurrent(current);
      if (!user) throw new Error("账号资料不完整，请联系管理员。");
      firstPasswordIntent.current = null;
      setFirstPasswordVersion(null);
      setRecoveryStep(null);
      setRecoveryCode("");
      ...
      if (epoch === currentApiSessionEpoch() || !hasApiSession()) setRecoveryError(toUserFacingError(error, locale));
    } finally {
      setRecoveryBusy(false);
    }
  };
  ...
  const resetPassword = async () => {
    ...
    try {
      ...
      setAccount(recoveryRole === "TEACHER" ? recoveryAccount.trim() : "");
  ...
  if (teacherModeGate || preAuthModeGate) {
    return (
      <PortalMaintenancePage
        ...
        allowAdminAccess={
          preAuthModeGate &&
          !["TEACHER", "STUDENT"].includes(apiSessionRole() ?? "") &&
          ...
          <section className="login-layout">
            ...
            >
              ...
              {!recoveryStep ? (
                ...
                <PasswordRecovery
                  firstPassword={firstPasswordVersion !== null}
                  step={recoveryStep}
                  ...
                  onResetPassword={() => void (firstPasswordVersion !== null ? submitFirstPassword() : resetPassword())}
                  onOpenAssistance={() => {
                    setRecoveryError(null);
      ...
      >
          ...
          <PageHeader
            ...
            actions={
              <>
                ...
                <ThemeControl theme={theme} onChange={handleThemeChange} />
                <button
                  className="icon-button workspace-notification-button"
                  aria-label="通知"
                  type="button"
                  onClick={() =>
                    workspaceMode === "real" ? setModal("notifications") : setToast(
                ...
                >
                  ...
                  {workspaceMode === "real" ? hasUnreadNotification && <span /> : role === "admin" && adminContext.notificationCount > 0 ? (
                    <span />
                  ) : null}
                </button>
              </>
        ...
        {modal && (
          <Modal
            ...
            onNotificationNavigate={(route,target) => { setNotificationTarget(target); setModal(null); navigateTo(route); }}
            onNotificationChanged={() => void refreshNotificationBadge()}
            role={role}
            mode={workspaceMode}
            user={displayUser}
            locale={locale}
  ...
  return (
    <main className="portal-maintenance-shell" aria-live="polite">
      <section className="portal-maintenance-card" role="status">
        ...
        <p className="portal-maintenance-kicker">BNBU SPORTS</p>
        <h1>
          {checking ? "正在确认系统状态" : status.announcement?.titleZh || "系统维护中"}
          <small>{checking ? "Checking system status" : status.announcement?.titleEn || "System maintenance"}</small>
        </h1>
        <p>
          {checking
            ? "正在读取服务端最新运行状态，确认完成前所有业务入口保持关闭。"
            : status.announcement?.bodyZh || "普通学生与教师业务暂不可用，请等待授权管理员恢复系统。"}
        </p>
        <p lang="en">
          {checking
            ? "All business entry points remain closed until the latest server status is confirmed."
            : status.announcement?.bodyEn || "Student and teacher features are temporarily unavailable until an authorised administrator restores service."}
        </p>
        {!checking && (
          <div className="portal-maintenance-meta">
            <span>预计恢复时间：{formatLocalRecoveryTime(status.announcement?.estimatedRecoveryAt, 'zh') || "请留意管理员后续通知"}</span>
            <span lang="en">Estimated recovery: {formatLocalRecoveryTime(status.announcement?.estimatedRecoveryAt, 'en') || "watch for an administrator update"}</span>
  ...
  return (
    <div className="password-recovery" aria-live="polite">
      <div className="recovery-head">
        ...
        <h1 id="login-title">{firstPassword ? "首次登录设置密码" : titles[step]}</h1>
      </div>
      {step === "identify" && (
        <form
      ...
      {step === "reset" && (
        ...
        >
          <p>
            {firstPassword ? "请使用当前临时密码设置个人密码，完成后进入工作台。" : "恢复请求已由后端受理。请输入邮件验证码和新密码；成功后，该账号在所有设备上的旧登录状态将失效。"}
          </p>
          {expiresAt && (
          ...
          <FormField
            label={firstPassword ? "当前临时密码" : "邮件验证码"}
            required
            controlId="recovery-code"
            hint={firstPassword ? "请输入管理员提供的临时密码" : locale === "en" ? "Enter a 4–10 digit verification code" : "请输入 4–10 位数字验证码"}
            error={userFacingFieldError(error, firstPassword ? "currentPassword" : "verificationCode", "code")}
          >
            <input
              id="recovery-code"
              ...
              value={code}
              onChange={(event) =>
                onCodeChange(firstPassword ? event.target.value : event.target.value.replace(/\D/g, "").slice(0, 10))
              }
              inputMode={firstPassword ? "text" : "numeric"}
              autoComplete={firstPassword ? "current-password" : "one-time-code"}
              placeholder={firstPassword ? "请输入当前临时密码" : "请输入 4–10 位数字验证码"}
              minLength={firstPassword ? undefined : 4}
              maxLength={firstPassword ? undefined : 10}
              aria-describedby={error ? "recovery-error" : undefined}
              autoFocus
            />
          </FormField>
          ...
          >
            ...
            {busy ? "正在验证…" : firstPassword ? "保存密码并进入工作台" : "验证并重置密码"}
          </button>
        </form>
      )}
      {step === "assistance" && (
  ...
  return (
    ...
    >
      ...
      >
        <div className="modal-head">
          <div className="account-modal-profile">
            ...
            <div className="account-modal-copy">
              <h2 id="modal-title">
                {notifications ? (locale === "en" ? "My notifications" : "我的通知") : view === "password"
        ...
        {notifications ? <PortalNotifications role={role} locale={locale} onNavigate={onNotificationNavigate} onChanged={onNotificationChanged}/> : view === "profile" ? (
          <>
