...
function RuleDialog({ locale, tableKey, rule, close }: { locale: AdminLocale; tableKey: string; rule?: EnduranceRule; close: () => void }) {
  ...
  const submit = async () => {
    const result = await run(key, () => saveEnduranceRule(form, state ?? undefined), adminCopy(locale, "rule_saved"));
    if (result) close();
  };
  return (
...
function EndurancePanel({ locale }: { locale: AdminLocale }) {
  ...
  const confirmDelete = async () => {
    if (!deleting) return;
    const result = await run(deleteKey, () => deleteEnduranceRule(deleting.id, state, deleting.tableVersion), adminCopy(locale, "rule_deleted"));
...
export function AdminRules({ locale }: { locale: AdminLocale }) {
  return (
    <div className="admin-page-stack">
      ...
      <AdminReviewServices locale={locale} templates />
    </div>
  );
}
