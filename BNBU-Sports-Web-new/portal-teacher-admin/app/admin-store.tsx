...
export function AdminStoreProvider({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      if (mode === "demo") {
        ...
        const [health, enduranceRules] = await Promise.all([refreshHealth(), loadRealEnduranceRules()]);
        const current = stateRef.current;
        if (current) commitState({ ...current, health, enduranceRules });
      }
    } catch (loadFailure) {
  ...
  useEffect(() => {
    ...
    const load =
      mode === "demo"
        ? loadAdminState()
        : Promise.all([refreshHealth(), loadRealEnduranceRules()]).then(([health, enduranceRules]) => ({
