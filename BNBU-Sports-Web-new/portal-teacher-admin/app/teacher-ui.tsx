  ...
  return (
    <div className="progress-cell">
      <div className="progress-cell-copy">
        <b>{available ? `${value.toFixed(1)} / ${target.toFixed(1)}h` : "—"}</b>
        <span>{available ? `${percent}%` : "—"}</span>
      </div>
      <div className="progress-track" aria-label={available ? `完成进度 ${percent}%` : "进度暂不可用"}>
        <i style={{ width: `${available ? percent : 0}%` }} />
      </div>
