...
import {request,toUserFacingError} from './api-client';
...
export function CourseHistorySettings({classSectionId}:{classSectionId:string}) {
 ...
 useEffect(()=>{let live=true;void request<Settings>(`/class-sections/${classSectionId}/history-settings`).then(data=>{if(live)setValue(data);}).catch(error=>{if(live)setMessage(toUserFacingError(error).message);});return()=>{live=false;};},[classSectionId]);
 ...
 }catch(error){setMessage(toUserFacingError(error).message);}finally{setBusy(false);}}
 ...
 </div><p className="course-ops-note">可包含入班前的运动，但不能补今天或未来日期。补卡须在常规提交截止前提交，按实际运动日期计算每日、每周次数。</p>
