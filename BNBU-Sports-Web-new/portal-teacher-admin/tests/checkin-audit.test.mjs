...
test('teacher pending queue excludes AI, technical and student supplement stages while preserving invalid history', () => {
  assert.equal(isTeacherReviewQueueRecord({auditStatus:'pending',workflowStage:'PENDING_TEACHER'}),true);
  assert.equal(isTeacherReviewQueueRecord({auditStatus:'invalid',workflowStage:'INVALID'}),true);
  assert.equal(isTeacherReviewQueueRecord({auditStatus:'pending'}),true);
  for(const workflowStage of ['PENDING_AI','TECHNICAL','AWAITING_SUPPLEMENT','VALID']) {
    assert.equal(isTeacherReviewQueueRecord({auditStatus:'pending',workflowStage}),false);
  }
  assert.equal(isTeacherReviewQueueRecord({auditStatus:'valid',workflowStage:'VALID'}),false);
});
...
test("single-record display accepts authoritative whole minutes and historical awards", () => {
  ...
  assert.equal(toCreditedDurationHours(30), 0.5);
  assert.equal(toCreditedDurationHours(45), 0.75);
  assert.equal(toCreditedDurationHours(31), 31 / 60);
  assert.equal(toCreditedDurationHours(90), 1.5);
  for (const value of [-1, NaN, Infinity, 30.5]) assert.equal(toCreditedDurationHours(value), null);
});
...
test("maps backend sport and two-state review enums", () => {
  ...
  assert.equal(checkin.submittedAt, "2026-08-15T10:00:00.000Z");
});
...
test("teacher workspace exposes invalid records and direct correction wording", async () => {
  ...
  assert.match(workspace, /退回补证和判无效需选择公开原因/);
  assert.match(workspace, /当前没有待处理或无效记录/);
