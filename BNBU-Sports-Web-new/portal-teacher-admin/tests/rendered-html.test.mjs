...
test("course workspaces do not expose an ended course business state", async () => {
  ...
  // V8.1 keeps closed-course continuations and historical facts in teacher scope.
  assert.doesNotMatch(teacherData, /item\.status !== "CLOSED" && item\.status !== "ARCHIVED"/);
  assert.doesNotMatch(adminCourses, /value: "CLOSED"/);
  assert.match(adminCourses, /await loadCourseDirectory\(\)/);
  const directoryApi = await readFile(new URL("../app/admin-course-directory-api.ts", import.meta.url), "utf8");
  assert.match(directoryApi, /status: 'UPCOMING' \| 'ACTIVE'/);
  assert.match(directoryApi, /\/admin\/course-directory/);
});
