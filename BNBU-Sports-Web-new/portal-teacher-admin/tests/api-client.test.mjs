...
test('portal starts a fresh refresh intent after a definitive maintenance denial', async () => {
  const keys = []; let refreshed = false;
  await withFetch(async (input, init = {}) => {
    const url = String(input);
    if (url.endsWith('/auth/password-login')) return Response.json({ data: authSession('maintenance') });
    if (url.endsWith('/auth/refresh')) {
      keys.push(init.headers['Idempotency-Key']);
      if (keys.length === 1) return Response.json({ code: 'SYSTEM_MAINTENANCE' }, { status: 503 });
      refreshed = true;
      return Response.json({ data: authSession('restored') });
    }
    ...
    await assert.rejects(api.request('/maintenance-refresh'), error => error.code === 'SYSTEM_MAINTENANCE');
    assert.deepEqual(await api.request('/maintenance-refresh'), { ok: true });
...
test("admin audit queries send date filters to the Backend and collect every download page", async () => {
  ...
  await withFetch(async (input, init = {}) => {
    ...
    if (url.includes("/admin/audit-events?")) {
      ...
      return Response.json({
        data: {items:[{ id: cursor ? "audit-2" : "audit-1",source:cursor?'V81':'FOUNDATION' }],nextCursor:cursor?null:'next-audit-page',timezone:'Asia/Shanghai'},
        meta: {},
      });
    }
    throw new Error(`Unexpected request: ${url}`);
    ...
    assert.deepEqual(rows.map(({ id }) => id), ["FOUNDATION:audit-1", "V81:audit-2"]);
    assert.equal(calls.length, 2);
    const first = new URL(calls[0].url, "https://local.invalid").searchParams;
    assert.equal(first.get("action"), "CLIENT_ERROR_REPORTED");
    assert.equal(first.get("startDate"), "2026-08-25");
    assert.equal(first.get("endDate"), "2026-08-25");
