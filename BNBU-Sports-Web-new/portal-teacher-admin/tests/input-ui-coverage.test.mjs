...
test("read-only course filters and review, roster and exemption controls keep accessible labels", async () => {
  ...
  assert.match(adminCourses, /await loadCourseDirectory\(\)/);
  assert.doesNotMatch(adminCourses, /fetchEnrollments|fetchExerciseRecords/);
...
test("runtime log downloads use the real server export flow and never synthesize CSV", async () => {
  ...
  assert.match(service, /requestRuntimeLogArchive[\s\S]*\/admin\/runtime-archives/);
  assert.match(service, /ARCHIVE_SIZE_MISMATCH/);
  assert.match(service, /ARCHIVE_HASH_MISMATCH/);
  ...
  assert.match(subadmins, /await requestSubadminIdentity\(/);
  assert.match(subadmins, /await verifySubadminIdentity\(/);
  assert.match(subadmins, /await createSubadminAccount\(\{ identityChallengeId: verified\.id/);
});
