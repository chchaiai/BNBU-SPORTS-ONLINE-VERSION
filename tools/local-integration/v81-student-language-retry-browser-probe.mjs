...
try {
  ...
  await page.getByText(/^(我的|Profile)$/).waitFor();
  ...
  stage='DISMISS_GUIDE';
  await page.reload();
  await page.getByText(/^(运动指引|Activity guide)$/).waitFor();
  await page.getByRole('button',{name:/^(跳过|Skip)$/}).click();
  stage='OPEN_SETTINGS';
  await page.getByText(/^(我的|Profile)$/).click();
  await page.locator('[data-action="profile.openSettings"]').click();
  if(await page.locator('html').getAttribute('lang')==='en'){
    const reset=page.waitForResponse(r=>new URL(r.url()).pathname==='/api/v1/me/preferences'&&r.request().method()==='PATCH');
    await page.locator('[data-action="profile.language"][data-value="zh"]').click();assert.equal((await reset).status(),200);
    await page.waitForFunction(()=>document.documentElement.lang==='zh-CN');
  }
  stage='SAVE_ENGLISH_LOST_RESPONSE';
  let committed, firstKey, retryKey;
  page.on('pageerror',error=>console.log(JSON.stringify({check:'PREFERENCE_PAGE_ERROR',type:error.name,message:error.message.slice(0,200)})));
  await page.route('**/api/v1/me/preferences',async route=>{
    if(route.request().method()!=='PATCH')return route.continue();
    if(!committed){
      firstKey=route.request().headers()['idempotency-key'];
      const response=await route.fetch();assert.equal(response.status(),200);
      committed=(await response.json()).data;console.log(JSON.stringify({check:'PREFERENCE_COMMITTED_RESPONSE_ABORT',version:committed.version}));await route.abort('failed');
  ...
  assert.equal(preferences.version,committed.version);assert.equal(firstKey,retryKey);
  await page.waitForFunction(()=>document.documentElement.lang==='en');
  await page.unroute('**/api/v1/me/preferences');
  await page.evaluate(async()=>{const {localStore}=await import('/student/js/store.js');localStore.setLanguage('zh');document.documentElement.lang='zh-CN';});
  console.log(JSON.stringify({check:'STUDENT_LANGUAGE_LOST_RESPONSE_SAME_VERSION',result:'PASS',version:preferences.version}));
