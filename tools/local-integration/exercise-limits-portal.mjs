...
const output='.local/exercise-limits-portal';fs.mkdirSync(output,{recursive:true});
...
try{
 ...
 await page.goto('http://127.0.0.1:4275/');await page.locator('#login-account').fill(fixture.adminEmail);await page.locator('#login-password').fill(fixture.password);await page.getByRole('button',{name:'登录',exact:true}).click();
 ...
 await maximum.fill('2');await teacher.locator('#course-target-course-hours').fill('10');await teacher.locator('#course-target-other-hours').fill(String(Number(target)/60-10));
 ...
 const rules=(await result.json()).data;assert.equal(rules.maximumMinutes,2);assert.equal(rules.courseTarget+rules.generalTarget,Number(target));
