...
const previous=process.env.DEMAND_BROWSER_REUSE==='1'?JSON.parse(await fs.readFile('.local/demand-six-browser-state.json','utf8')):null;
let fixture,student;
...
const env={...foundationEnvironment(databaseUrl,53001),ACCESS_TOKEN_TTL:'900',REFRESH_TOKEN_IDLE_TTL:'1800',
  LOG_LEVEL:'error',PUBLIC_ORGANIZATION_CODE:'BNBU-TEST',
...
await fs.writeFile('.local/demand-six-browser-state.json',JSON.stringify({fixture,student,organizationCode:organization.organizationCode,baseUrl:'http://127.0.0.1:53000'},null,2));
