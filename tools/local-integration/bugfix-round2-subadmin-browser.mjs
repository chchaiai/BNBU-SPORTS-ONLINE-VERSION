...
try {
  ...
  const suffix = randomUUID(), account = 'browser-' + suffix, email = process.env.V81_SHARED_STUDENT_EMAIL==='1' ? state.accounts.student.email : account + '@example.test';
  ...
  let code;
  let creationEmailChallenges=0;
  page.on('request',request=>{if(new URL(request.url()).pathname==='/api/v1/admin/subadmin-identity-challenges')creationEmailChallenges++;});
  stage='VERIFY_CREATE';
  const createCalls=[];
  if(process.env.V81_DROP_SUBADMIN_CREATE_RESPONSE==='1'){
    await page.route('**/api/v1/admin/subadmins',async route=>{
      if(route.request().method()!=='POST'){await route.continue();return;}
      const response=await route.fetch();assert.equal(response.status(),201);
      const result=(await response.json()).data;
      createCalls.push({id:result.id,key:route.request().headers()['idempotency-key']});
      if(createCalls.length===1)await route.abort('connectionfailed');
      else await route.fulfill({response});
    });
  }
  await page.getByRole('button', { name: '创建分管理员', exact: true }).click();
  if(process.env.V81_DROP_SUBADMIN_CREATE_RESPONSE==='1'){
  ...
  if(process.env.V81_DROP_SUBADMIN_CREATE_RESPONSE==='1'){
    assert.equal(createCalls.length,2);assert.deepEqual(createCalls[0],createCalls[1]);
    console.log(JSON.stringify({check:'REAL_SUBADMIN_COMMITTED_CREATE_LOST_RESPONSE_SAME_KEY_REPLAY',result:'PASS'}));
  }
  await page.getByRole('row').filter({ hasText: account }).waitFor();
  assert.equal(creationEmailChallenges,0);
  console.log(JSON.stringify({check:'SUBADMIN_DIRECT_CREATE_NO_EMAIL_CODE',result:'PASS',sharedStudentEmail:process.env.V81_SHARED_STUDENT_EMAIL==='1'}));
  await page.reload();
  await page.getByRole('button', { name: '分管理员设置', exact: true }).click();
  ...
  await page.getByRole('button', { name: '分管理员设置', exact: true }).click();
  await row.getByRole('button', { name: '停用', exact: true }).waitFor();
  console.log(JSON.stringify({ check: 'BROWSER_DIRECT_CREATE_AND_STATUS_RELOAD', result: 'PASS' }));
  ...
  console.log(JSON.stringify({ check: 'BROWSER_SUBADMIN_DIRECT_CREATE_RELOAD_DISABLE_ENABLE', result: 'PASS', syntheticIdentity: true }));
} catch (error) {
