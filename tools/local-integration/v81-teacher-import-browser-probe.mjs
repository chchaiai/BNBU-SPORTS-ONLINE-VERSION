...
try{
 ...
 await teacher.goto('http://localhost:3300/');await teacher.getByLabel('学校邮箱').fill(accounts[0].email);await teacher.locator('#login-password').fill(password);
 await teacher.getByRole('button',{name:'登录',exact:true}).click();
 await teacher.getByText(/首次登录.*修改|修改初始密码/).first().waitFor();
