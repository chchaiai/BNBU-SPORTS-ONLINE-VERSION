...
try{
 const page=await browser.newPage({viewport:{width:1440,height:1000}});page.setDefaultTimeout(60000);await page.goto(origin);
 ...
 await loaded(row);await row.screenshot({path:'.local/teacher-previews-'+label+'-list.png'});
 await page.getByRole('button',{name:'相册',exact:true}).click();await row.scrollIntoViewIfNeeded();await loaded(row);assert.equal(await row.locator('.proof-thumbnail-real').first().evaluate(e=>getComputedStyle(e,'::before').content),'none');await row.screenshot({path:'.local/teacher-previews-'+label+'-album.png'});
