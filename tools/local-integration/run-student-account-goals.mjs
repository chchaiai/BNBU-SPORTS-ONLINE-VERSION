...
const database='bnbu_self_delete_'+Date.now();
...
const processEnv={...process.env,DATABASE_URL:url,MIGRATION_DATABASE_URL:url,ACCOUNT_GOALS_TEST_DATABASE:url};
mkdirSync('evidence/student-account-goals-20260920',{recursive:true});
...
writeFileSync('evidence/student-account-goals-20260920/local-migration.log',migration.stdout+ migration.stderr);
if(migration.status!==0)throw Error('Isolated database migration failed; see local-migration.log');
const result=spawnSync(process.execPath,['--import','tsx','../tools/local-integration/student-account-goals-http.mjs'],{cwd:'backend',env:processEnv,encoding:'utf8'});
writeFileSync('evidence/student-account-goals-20260920/http-tests.log',result.stdout+result.stderr);
