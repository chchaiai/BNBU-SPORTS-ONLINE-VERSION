...
try{
 ...
 if(process.env.BNBU_EXPECT_CONTINUITY==='1') {
  ...
  await page.waitForFunction(()=>window.loadingProbeApp.ui.checkin.drafts.some(d=>d.type==='video'));
 }
 ...
 if(process.env.BNBU_EXPECT_CONTINUITY==='1') {
  ...
  const cdp=await context.newCDPSession(page);await cdp.send('Page.setWebLifecycleState',{state:'frozen'});await new Promise(r=>setTimeout(r,2500));await cdp.send('Page.setWebLifecycleState',{state:'active'});
  ...
  await page.screenshot({path:'.local/basic-20260911/continuity-'+(cloud?'cloud':'local')+'.png',fullPage:true,animations:'disabled'});
  ...
  fs.writeFileSync('.local/basic-20260911/continuity-'+(cloud?'cloud':'local')+'-record.json',JSON.stringify({recordId:record.id,sessionId:before.id,expectedHours:0}));
