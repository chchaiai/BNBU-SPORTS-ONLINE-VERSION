...
try{
 ...
 await page.screenshot({path:'evidence/photo-capture-20260916/camera-captured.png',fullPage:true});
 ...
 await page.locator('#checkin-description').fill('Synthetic real camera photo upload acceptance');
 ...
 assert.ok(labels.some(label=>label.includes('上传')));
 ...
 await page.screenshot({path:'evidence/photo-capture-20260916/camera-submitted.png',fullPage:true});
 await fs.writeFile('evidence/photo-capture-20260916/camera-result.json',JSON.stringify({status:'PASS',completionStatusCleared:true,syntheticCamera:true,realElapsedSeconds:62,realMinioPuts:puts,submitStatus:response.status(),labels},null,2));
