...
try {
  ...
  stage='LOGOUT';
  const revoke=page.waitForResponse(r=>new URL(r.url()).pathname==='/api/v1/auth/logout'&&r.request().method()==='POST');
  await page.getByRole('button',{name:'退出登录',exact:true}).click();
  await page.getByRole('alertdialog').getByRole('button',{name:'退出登录',exact:true}).click();
  const response=await revoke;assert.equal(response.status(),200);
  const bearer=response.request().headers().authorization;
  assert.ok(bearer);
  const oldSession=await page.request.get('http://127.0.0.1:4274/api/v1/me',{headers:{Authorization:bearer}});
  assert.equal(oldSession.status(),401);
  console.log(JSON.stringify({check:'STUDENT_SERVER_LOGOUT',result:'PASS',status:response.status(),oldSessionStatus:oldSession.status()}));
  stage='RELOAD_LOGGED_OUT';
  await page.reload();
  await page.getByText('邮箱验证码登录',{exact:true}).waitFor();
  assert.equal(await page.getByText('我的',{exact:true}).count(),0);
  console.log(JSON.stringify({check:'STUDENT_LOGOUT_REVOKES_SERVER_SESSION_AND_LOCAL_LOGIN',result:'PASS',oldSessionStatus:401}));
}catch(error){console.error(JSON.stringify({check:'STUDENT_LOGOUT',result:'FAIL',stage,type:error.name,pageText:diagnosticPage?(await diagnosticPage.locator('body').innerText()).slice(0,1400):'',message:error.message.slice(0,350),emailRequests:apiResponses.filter(item=>item.path.includes('email-verification-challenges')),failedRequests:apiResponses.filter(item=>item.status>=400)}));process.exitCode=1;}
