...
const uuidv7=createRequire(new URL('../../backend/package.json',import.meta.url))('uuid').v7,dir='/workspace/.browser-state',state=JSON.parse(fs.readFileSync(dir+'/state.json')),file=dir+(process.env.BNBU_CLOSURE_MULTI==='1'?'/closure-memberships-multi.json':'/closure-memberships.json');
...
try{assert.equal((await prisma.$queryRaw`SELECT current_database() AS name`)[0].name,'v81_browser_test');
 if(!fs.existsSync(file)){
  ...
  await prisma.$transaction(async tx=>{await tx.classSection.create({data:{...source,id:sectionId,classCode:process.env.BNBU_CLOSURE_MULTI==='1'?'SYNTH-CLOSE-MULTI':'SYNTH-CLOSE-MEMBERS',displayName:'Synthetic closure memberships',version:1,createdAt:new Date(),updatedAt:new Date()}});
  ...
  });const student=await seedExerciseSessionStudent(prisma,{...state.fixture,teacherAActiveSectionId:sectionId},randomUUID().slice(0,8).toUpperCase(),'ACTIVE',false);const peers=[];if(process.env.BNBU_CLOSURE_MULTI==='1')for(let i=0;i<2;i++)peers.push(await seedExerciseSessionStudent(prisma,{...state.fixture,teacherAActiveSectionId:sectionId},randomUUID().slice(0,8).toUpperCase(),'ACTIVE',false));fs.writeFileSync(file,JSON.stringify({sectionId,student,peers}),{mode:0o600});
 }
