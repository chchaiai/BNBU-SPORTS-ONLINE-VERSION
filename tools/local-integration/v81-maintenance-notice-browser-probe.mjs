...
try{
 ...
 await teacher.goto('http://localhost:3300/');await teacher.getByLabel('学校邮箱').fill(state.accounts.teacher.email);await teacher.locator('#login-password').fill(state.accounts.teacher.password);await teacher.getByRole('button',{name:'登录',exact:true}).click();await teacher.getByRole('button',{name:'课程管理',exact:true}).waitFor();
 ...
 await student.getByText(notice.bodyZh,{exact:true}).waitFor();await student.getByText(notice.titleZh,{exact:true}).waitFor();await student.getByText('预计恢复时间：'+notice.estimatedRecoveryAt,{exact:true}).waitFor();
 ...
 await teacher.getByText('预计恢复时间：'+notice.estimatedRecoveryAt,{exact:true}).waitFor();
