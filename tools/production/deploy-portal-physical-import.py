...
work = Path('/home/ubuntu/bnbu-portal-physical-import-20260913')
previous = base / 'releases/portal-admin-goal-20260913'
release = base / 'releases/portal-physical-import-20260913'
tag = 'bnbu-portal-production:physical-import-20260913'
gate = json.loads((work / 'validation.json').read_text())
assert gate['baseSource'] == 'a46fbe41f44b0724c00ee6d91944f5fdfc58cdf0'
assert gate['checks'] == {'build': 'PASS', 'focusedTests': '39/39'}
assert gate['changedSourceFiles'] == ['app/admin-i18n.ts', 'app/admin-overview.tsx', 'app/admin-subadmins.tsx', 'app/language.tsx', 'app/own-password-panel.tsx', 'app/portal-app.tsx', 'app/teacher-workspace.tsx', 'app/physical-import-panel.tsx', 'app/physical-result-form.tsx']
...
def switch(target):
    link = base / 'current-portal-physical-import-tmp'
...
try:
    ...
    switch(release)
    print(json.dumps({'check': 'PORTAL_PHYSICAL_IMPORT_DEPLOYMENT', 'result': 'PASS', 'release': str(release), 'previous': str(previous), 'imageId': image_id, 'baseSource': gate['baseSource'], 'patches': gate['patches'], 'rollbackExercised': False}))
except Exception:
    if (base / 'current').resolve() != previous:
        switch(previous)
    start(previous)
    print(json.dumps({'check': 'PORTAL_PHYSICAL_IMPORT_DEPLOYMENT', 'result': 'FAILED_ROLLED_BACK'}))
    raise
