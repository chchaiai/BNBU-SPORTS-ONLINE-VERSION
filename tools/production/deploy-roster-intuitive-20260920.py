...
WORK=Path('/home/ubuntu/bnbu-roster-intuitive-20260920')
RELEASE=BASE/'releases/roster-intuitive-20260920'
...
def switch(target):
 ...
 link=BASE/'current-roster-intuitive-tmp';assert not link.exists() and not link.is_symlink()
...
assert gate['checks']=={'unit':16,'httpAndBrowser':5,'typecheck':'PASS','build':'PASS','focusedLint':'PASS'}
...
if sys.argv[1]=='--prepare':
 ...
 run(['docker','tag',gate['basePortalImage'],'bnbu-portal-roster-intuitive-base:20260920'])
 tag='bnbu-portal-production:roster-intuitive-20260920'
 run(['docker','build','--pull=false','-t',tag,str(WORK)])
 image=out(['docker','image','inspect','--format','{{.Id}}',tag])
 candidate='bnbu-roster-intuitive-candidate'
