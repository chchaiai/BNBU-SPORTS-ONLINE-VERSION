"""Deploy the validated student region display overlay on the pinned Hong Kong release."""
import hashlib
import json
import os
...
WORK = Path('/home/ubuntu/bnbu-region-20260914')
assert os.geteuid() == 0
assert sys.argv[1:] in [['--prepare'], ['--apply'], ['--rollback']]
gate = json.loads((WORK / 'validation.json').read_text())
assert len(gate['sourceCommit']) == 40
assert gate['checks'] == {'focusedTests': 2, 'studentSmoke': 87, 'typecheck': 'PASS', 'build': 'PASS'}
assert gate['previous'] == 'login-admin-d34e3e38-20260914'
assert gate['release'] == 'student-region-20260914'
...
if sys.argv[1] == '--prepare':
    ...
    candidate = 'bnbu-region-portal-candidate'
