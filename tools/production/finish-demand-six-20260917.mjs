...
const f=JSON.parse(await readFile('/acceptance/demand-six-20260917.json','utf8'));
try{
 const ids=[f.fixture.organizationId,f.fixture.isolationOrganizationId];
 const orgs=await db.organization.findMany({where:{id:{in:ids}}});assert.equal(orgs.length,2);assert.ok(orgs.every(o=>/^(BNBU|ISOLATION)-TEST-SIX17-/.test(o.organizationCode)));
