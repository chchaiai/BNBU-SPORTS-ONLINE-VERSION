...
assert os.geteuid()==0
action=sys.argv[1];assert action in ['prepare','verify','finish']
work=Path('/home/ubuntu/bnbu-ai-review-20260917');release=Path('/opt/bnbu-sports-production/current').resolve();assert release.name in ['ai-review-20260917','ai-review-ui-20260917','ai-review-contract-20260917']
directory=work/'acceptance';directory.mkdir(mode=0o700,exist_ok=True);os.chown(directory,10001,10001)
if action=='prepare':assert not (directory/'ai-review-20260917.json').exists(),'Fixture already exists'
if action=='verify':assert not (directory/'result.json').exists(),'Acceptance already completed'
...
script={'prepare':'prepare-ai-review-20260917.mjs','verify':'verify-ai-review-20260917.mjs','finish':'finish-ai-review-20260917.mjs'}[action]
...
if action=='prepare':subprocess.run(['install','-o','ubuntu','-g','ubuntu','-m','0600',str(directory/'ai-review-20260917.json'),str(work/'ai-review-private.json')],check=True)
