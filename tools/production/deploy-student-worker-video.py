...
previous=base/'releases/student-receipt-20260913'
release=base/'releases/student-worker-video-20260913'
work=Path('/home/ubuntu/bnbu-student-worker-video-20260913')
assert (base/'current').resolve()==previous and not release.exists()
gate=json.loads((work/'validation.json').read_text())
assert gate['sourceCommit'].startswith('cfc916c9')
...
try:
    ...
    print(json.dumps({'check':'STUDENT_WORKER_VIDEO_DEPLOYMENT','result':'PASS','release':release.name,'previous':previous.name,'sourceCommit':gate['sourceCommit'],'files':gate['files']}))
except Exception:
    switch(previous)
    raise
