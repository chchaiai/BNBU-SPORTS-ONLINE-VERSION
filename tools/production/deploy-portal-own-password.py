...
work = Path('/home/ubuntu/bnbu-portal-own-password-20260913')
previous = base / 'releases/student-local-player-20260913'
release = base / 'releases/portal-own-password-20260913'
tag = 'bnbu-portal-production:own-password-20260913'
...
def switch(target):
    link = base / 'current-portal-own-password-tmp'
...
try:
    ...
    switch(release)
    print(json.dumps({'check': 'PORTAL_OWN_PASSWORD_DEPLOYMENT', 'result': 'PASS', 'release': str(release), 'previous': str(previous), 'imageId': image_id, 'baseSource': gate['baseSource'], 'patches': gate['patches'], 'rollbackExercised': False}))
except Exception:
    if (base / 'current').resolve() != previous:
        switch(previous)
    start(previous)
    print(json.dumps({'check': 'PORTAL_OWN_PASSWORD_DEPLOYMENT', 'result': 'FAILED_ROLLED_BACK'}))
    raise
