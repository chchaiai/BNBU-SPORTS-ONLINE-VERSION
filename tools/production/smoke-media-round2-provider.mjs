...
try{
  const columns=await db.$queryRaw`SELECT table_name,column_name FROM information_schema.columns
    WHERE table_schema='public' AND (table_name='student_profiles' AND column_name IN ('date_of_birth','region_code','other_region_name')
  ...
  const missing=await db.$queryRaw`SELECT count(*)::int AS count FROM exercise_records e
    JOIN v81_course_rules r ON r.class_section_id=e.class_section_id AND r.published_at IS NOT NULL
    LEFT JOIN v81_record_rule_snapshots s ON s.record_id=e.id WHERE s.record_id IS NULL`;
  assert.equal(missing[0].count,0);
  results.push({check:'CLOUD_FOLLOWUP_SCHEMA_AND_EXISTING_RECORD_SNAPSHOTS',result:'PASS',missingSnapshots:0});
  ...
  sources[1].bytes=Buffer.concat([sources[1].bytes,Buffer.alloc(1208)]);
  ...
  console.log(JSON.stringify({check:'MEDIA_ROUND2_CLOUD_PROVIDER',result:'PASS',checks:results}));
