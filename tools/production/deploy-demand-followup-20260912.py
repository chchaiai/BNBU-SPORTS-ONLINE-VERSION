...
def main():
    ...
    work = Path('/home/ubuntu/bnbu-demand-followup-20260912')
    base = Path('/opt/bnbu-sports-production')
    previous = base / 'releases/demand-20260912'
    release = base / 'releases/demand-followup-20260912'
    ...
    for service in ['backend', 'portal', 'migrator']:
        image = f'bnbu-{service}-production:demand-followup-20260912'
    ...
    for service in ['BACKEND', 'PORTAL', 'MIGRATOR']:
        content, count = re.subn(rf'(?m)^{service}_IMAGE=.*$',
                                f'{service}_IMAGE=bnbu-{service.lower()}-production:demand-followup-20260912', content)
        assert count == 1
    ...
    def switch(target):
        link = base / 'current-demand-followup-tmp'
    ...
    try:
        ...
        result = {'check': 'DEMAND_FOLLOWUP_DEPLOYMENT', 'result': 'PASS', 'sourceCommit': gate['sourceCommit'],
        ...
        print(json.dumps({'check': 'DEMAND_FOLLOWUP_DEPLOYMENT', 'result': 'FAILED_REQUIRES_FORWARD_FIX',
