...
previous=base/'releases/student-worker-video-20260913'
release=base/'releases/student-local-player-20260913'
work=Path('/home/ubuntu/bnbu-student-local-player-20260913')
assert (base/'current').resolve()==previous and not release.exists()
gate=json.loads((work/'validation.json').read_text())
assert gate['sourceCommit'].startswith('c7566129')
assert set(gate['files'])=={'screens/checkin.js','app.js','local-video-player.js'}
...
try:
    switch(release)
    for name,digest in gate['files'].items():
        with urllib.request.urlopen('https://www.student.bnbusports.cn/student/js/'+name+'?acceptance=live-camera',timeout=20) as response:
            assert response.status==200 and hashlib.sha256(response.read()).hexdigest()==digest
    print(json.dumps({'check':'STUDENT_LOCAL_PLAYER_DEPLOYMENT','result':'PASS','release':release.name,'previous':previous.name,'sourceCommit':gate['sourceCommit'],'files':gate['files']}))
except Exception:
    switch(previous)
    raise
