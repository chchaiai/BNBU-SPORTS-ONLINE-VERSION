...
try{
 ...
 fixture=await seedFoundationFixture(db,'-MEMBER13-'+suffix);
 const student=await seedExerciseSessionStudent(db,fixture,'L'+suffix);
 await db.studentProfile.update({where:{id:student.studentId},data:{gender:'MALE',version:{increment:1}}});
 ...
 writeFileSync('/acceptance/membership-20260913.json',JSON.stringify({fixture,student,studentSession,teacherToken:teacher,adminToken:admin,teacher:{email:teacherUser.primaryEmail,password},admin:{email:adminUser.primaryEmail,password},createdAt:new Date().toISOString()}),{mode:0o600,flag:'wx'});
