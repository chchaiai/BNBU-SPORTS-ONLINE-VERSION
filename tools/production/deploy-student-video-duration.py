...
previous=base/'releases/student-recognition-copy-20260913'
release=base/'releases/student-video-duration-20260913'
work=Path('/home/ubuntu/bnbu-student-video-duration-20260913')
assert (base/'current').resolve()==previous and not release.exists()
gate=json.loads((work/'validation.json').read_text())
assert gate['sourceCommit'].startswith('4eae442ecebe46bbaad3f781042e78c88296d464')
...
try:
    switch(release)
    for name,digest in gate['files'].items():
        with urllib.request.urlopen('https://www.student.bnbusports.cn/student/js/'+name+'?acceptance=video-duration',timeout=20) as response:
            assert response.status==200 and hashlib.sha256(response.read()).hexdigest()==digest
    print(json.dumps({'check':'STUDENT_VIDEO_DURATION_DEPLOYMENT','result':'PASS','release':release.name,'previous':previous.name,'sourceCommit':gate['sourceCommit'],'files':gate['files']}))
except Exception:
    switch(previous)
    raise
