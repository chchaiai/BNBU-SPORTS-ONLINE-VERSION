"""Publish a static student media fix on a pinned release with automatic rollback."""
from pathlib import Path
import hashlib,json,os,shutil,subprocess,sys,urllib.request
BASE=Path('/opt/bnbu-sports-production');WORK=Path('/home/ubuntu/bnbu-media-errors-20260920');RELEASE=BASE/'releases/media-errors-20260920'
...
assert previous==BASE/'releases/portal-mobile-20260920'
...
def switch(target):
 ...
 link=BASE/'current-media-errors-tmp';assert not link.exists() and not link.is_symlink();link.symlink_to(target);os.replace(link,BASE/'current')
def http(url):
...
assert gate['checks']=={'tests':95,'smoke':87,'mobileBrowser':'PASS'}
assert (BASE/'current').resolve()==previous and not RELEASE.exists()
assert set(gate['files'])=={'js/api.js','js/photo-originals.js','js/screens/checkin.js'}
...
try:
 ...
 for name,digest in gate['files'].items():
  for suffix in ['', '?verify=media-errors-20260920']:
