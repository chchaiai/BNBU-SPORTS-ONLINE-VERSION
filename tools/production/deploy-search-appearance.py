"""Deploy the validated search appearance overlay on the pinned Hong Kong release."""
import hashlib
import json
import os
...
WORK = Path('/home/ubuntu/bnbu-search-20260914')
assert os.geteuid() == 0
assert sys.argv[1:] in [['--prepare'], ['--apply'], ['--rollback']]
gate = json.loads((WORK / 'validation.json').read_text())
assert len(gate['sourceCommit']) == 40
assert gate['checks'] == {'studentSmoke': 87, 'typecheck': 'PASS', 'build': 'PASS'}
assert gate['previous'] == 'course-targets-20260914'
assert gate['release'] == 'search-20260914'
...
if sys.argv[1] == '--prepare':
    ...
    candidate = 'bnbu-search-portal-candidate'
