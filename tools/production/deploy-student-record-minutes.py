...
previous=base/'releases/student-video-duration-20260913'
release=base/'releases/student-record-minutes-20260913'
work=Path('/home/ubuntu/bnbu-student-record-minutes-20260913')
assert (base/'current').resolve()==previous and not release.exists()
gate=json.loads((work/'validation.json').read_text())
assert gate['sourceCommit'].startswith('5a30c01e6b08e69893dda6d43fefc02dc0b1f281')
...
try:
    ...
    print(json.dumps({'check':'STUDENT_RECORD_MINUTES_DEPLOYMENT','result':'PASS','release':release.name,'previous':previous.name,'sourceCommit':gate['sourceCommit'],'files':gate['files']}))
except Exception:
    switch(previous)
    raise
