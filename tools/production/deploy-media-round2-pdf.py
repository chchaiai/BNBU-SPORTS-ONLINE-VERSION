...
previous = base / 'releases/media-round2-20260912'
release = base / 'releases/media-round2-pdf-20260912'
assert (base / 'current').resolve() == previous and not release.exists()
gate = json.loads((work / 'pdf-patch.json').read_text())
with tarfile.open(work / 'pdf-patch.tar') as tar:
...
link = base / 'current-media-round2-pdf-tmp'
...
result = dict(check='MEDIA_ROUND2_PDF_DEPLOYMENT', result='PASS', sourceCommit=gate['sourceCommit'], release=str(release), previous=str(previous), filesVerified=len(contents))
(work / 'pdf-deployment-result.json').write_text(json.dumps(result, indent=2))
