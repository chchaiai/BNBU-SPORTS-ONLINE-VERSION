...
try{
 ...
 fixture=await seedFoundationFixture(db,'-CLOSE-'+suffix);
 ...
 const template=await request(admin,'/rule-templates',{displayName:'Course closure cloud acceptance',expectedVersion:0});
 ...
 const modeResponse=await fetch('https://www.student.bnbusports.cn/api/v1/admin/review-services/manual-mode/'+fixture.teacherAActiveSectionId,{headers:{authorization:`Bearer ${admin}`}});assert.ok(modeResponse.ok);const mode=(await modeResponse.json()).data;if(!mode.enabled)await request(admin,'/admin/review-services/manual-mode',{classSectionId:fixture.teacherAActiveSectionId,enabled:true,reason:'Isolated course closure acceptance',expectedVersion:mode.version});
 const teacherUser=await db.user.findUniqueOrThrow({where:{id:fixture.teacherUserId}});
 writeFileSync('/acceptance/closure-checkin.json',JSON.stringify({fixture,sectionId:fixture.teacherAActiveSectionId,student,peers,studentSession,teacher:{email:teacherUser.primaryEmail,password},createdAt:new Date().toISOString()}),{mode:0o600,flag:'wx'});
 console.log(JSON.stringify({check:'CLOSURE_CLOUD_FIXTURE_READY',result:'PASS',organizationId:fixture.organizationId,classSectionId:fixture.teacherAActiveSectionId,studentId:student.studentId,teacherId:fixture.teacherUserId,sessionSetup:'isolated synthetic fixture; SMTP not tested'}));
