"""Deploy the validated administrator maintenance access overlay on the pinned Hong Kong release."""
import hashlib
import json
import os
...
WORK = Path('/home/ubuntu/bnbu-admin-maintenance-20260914')
assert os.geteuid() == 0
assert sys.argv[1:] in [['--prepare'], ['--apply'], ['--rollback']]
gate = json.loads((WORK / 'validation.json').read_text())
assert len(gate['sourceCommit']) == 40
assert gate['checks'] == {'http': 'PASS', 'typecheck': 'PASS', 'build': 'PASS', 'portalTests': '33_PASS'}
assert gate['previous'] == 'student-bulk-deletion-20260914'
assert gate['release'] == 'admin-maintenance-20260914'
...
def switch(target):
    link = BASE / 'current-admin-maintenance-tmp'
...
if sys.argv[1] == '--prepare':
    ...
    run(['docker', 'run', '--rm', '--network', 'none', '--entrypoint', 'node', tags['backend'], '--input-type=module', '-e', "const {permitsSystemMode}=await import('./dist/modules/system-mode/system-mode-access.js');if(!permitsSystemMode('MAINTENANCE','ADMIN')||permitsSystemMode('MAINTENANCE','STUDENT'))throw Error('Mode policy');"])
    run(['docker', 'build', '--pull=false', '-t', gate['migratorImage'], str(WORK / 'migrator')])
    candidate = 'bnbu-admin-maintenance-candidate'
    ...
    result = {'result': 'PASS', 'phase': 'PREPARED', 'images': images, 'sourceCommit': gate['sourceCommit'], 'migrationExecuted': False, 'migratorImageId': output(['docker','image','inspect','--format','{{.Id}}',gate['migratorImage']])}
...
assert output(['docker','image','inspect','--format','{{.Id}}',gate['migratorImage']]) == prepared['migratorImageId']
assert not release.exists()
...
for service, tag in tags.items():
    ...
    assert count == 1
content, count = re.subn(r'(?m)^MIGRATOR_IMAGE=.*$', 'MIGRATOR_IMAGE=' + gate['migratorImage'], content)
assert count == 1
...
(WORK / 'backup-result.json').write_text(json.dumps(backup, indent=2) + '\n')
...
try:
    ...
    result = {'result': 'PASS', 'phase': 'DEPLOYED', 'sourceCommit': gate['sourceCommit'], 'previous': str(previous), 'release': str(release), 'images': prepared['images'], 'health': checks, 'studentModuleHashes': 'PASS', 'migrationExecuted': '0078_admin_maintenance_access', 'runtimeConfiguration': 'PRESERVED', 'rollbackExercised': False, 'databaseBackup': backup}
