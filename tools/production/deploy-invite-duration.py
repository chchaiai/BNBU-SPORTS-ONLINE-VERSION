"""Deploy the validated custom course invitation duration overlay on the pinned Hong Kong release."""
import hashlib
import json
import os
...
WORK = Path('/home/ubuntu/bnbu-invite-duration-20260914')
assert os.geteuid() == 0
assert sys.argv[1:] in [['--prepare'], ['--apply'], ['--rollback']]
gate = json.loads((WORK / 'validation.json').read_text())
assert len(gate['sourceCommit']) == 40
assert gate['checks'] == {'http': 'PASS', 'typecheck': 'PASS', 'build': 'PASS', 'portalTests': '14_PASS_2_BASELINE_FAILURES'}
assert gate['previous'] == 'swim-intake-20260914'
assert gate['release'] == 'invite-duration-20260914'
...
def switch(target):
    link = BASE / 'current-invite-duration-tmp'
...
def baseline():
    ...
    assert output(['docker', 'exec', 'bnbu-sports-production-backend-1', 'sha256sum', '/app/dist/modules/course-invites/domain/invite-timing.js']).split()[0] == gate['baselineBackendService']
...
if sys.argv[1] == '--prepare':
    ...
    run(['docker', 'run', '--rm', '--network', 'none', '--read-only', '--tmpfs', '/tmp', '--entrypoint', 'node', tags['backend'], '--input-type=module', '-e', "await import('reflect-metadata');const {resolveInviteExpiry}=await import('./dist/modules/course-invites/domain/invite-timing.js');const now=new Date('2026-09-14T00:00:00Z');if(resolveInviteExpiry({expiresInMinutes:10080},now,new Date('2027-01-01')).getTime()-now.getTime()!==10080*60000)throw Error('Duration');"])
    candidate = 'bnbu-invite-duration-candidate'
