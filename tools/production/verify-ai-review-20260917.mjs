// Mutations are confined to the AI17 synthetic organization created for this acceptance.
...
const f=JSON.parse(await readFile('/acceptance/ai-review-20260917.json','utf8'));assert.match(f.fixture.teacherEmail,/ai17-/);
const checks=[],records=[],requests=[];const origin='https://www.teacher.bnbusports.cn/api/v1';
async function api(path,token,body,expected=200){const requestId=randomUUID();const response=await fetch(origin+path,{method:body?'POST':'GET',headers:{authorization:`Bearer ${token}`,'content-type':'application/json','idempotency-key':randomUUID(),'x-request-id':requestId},...(body?{body:JSON.stringify(body)}:{}),signal:AbortSignal.timeout(40000)});const result=await response.json();requests.push({path,status:response.status,requestId});assert.equal(response.status,expected,JSON.stringify({path,status:response.status,code:result.code,details:result.details,requestId}));return result.data;}
...
try{
 const organization=await db.organization.findUniqueOrThrow({where:{id:f.fixture.organizationId}});assert.match(organization.organizationCode,/^BNBU-TEST-AI17-/);
 ...
 for(const [kind,bytes] of [['IMAGE',image],['VIDEO',video],['IMAGE',image]]){
  ...
  const draft=await api('/exercise-records',f.studentSession.accessToken,{sessionId,creditType:'GENERAL',sportType:'RUNNING',description:`AI17 synthetic ${kind} acceptance`,clientRequestId:randomUUID()},201);
  const submitted=await api(`/exercise-records/${draft.id}/submit`,f.studentSession.accessToken,{mediaIds:[upload.mediaId],expectedVersion:draft.version});
  records.push({id:draft.id,mediaId:upload.mediaId,kind});assert.equal(submitted.workflowStage,'PENDING_TEACHER');assert.equal(submitted.creditedDurationSeconds,0);
  const result=await waitFor(()=>api(`/exercise-records/${draft.id}`,f.teacherToken),v=>['SUCCEEDED','FAILED'].includes(v.aiReview?.status));
  assert.equal(result.aiReview.status,'SUCCEEDED',JSON.stringify(result.aiReview));assert.equal(result.workflowStage,'PENDING_TEACHER');assert.equal(result.currentReview.result,'PENDING');assert.equal(result.creditedDurationSeconds,0);
  ...
  checks.push({kind,recordId:draft.id,ai:result.aiReview,teacherDecisionStillPending:true,studentAdviceHidden:true});
 }
