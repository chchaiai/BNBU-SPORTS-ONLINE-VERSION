...
try{const state=JSON.parse(readFileSync('/acceptance/student-deletion-checkin.json','utf8')),orgs=[state.fixture.organizationId,state.fixture.isolationOrganizationId];
 for(const id of orgs){const org=await db.organization.findUniqueOrThrow({where:{id}});assert.ok(org.organizationCode.includes('-ERASE-'));}
 if(process.argv[2]==='--reopen-teacher'){const user=await db.user.findUniqueOrThrow({where:{id:state.fixture.teacherUserId}});assert.equal(user.status,'DISABLED');assert.equal(user.organizationId,state.fixture.organizationId);await db.user.update({where:{id:user.id},data:{status:'ACTIVE'}});console.log(JSON.stringify({check:'STUDENT_DELETION_TEACHER_REOPENED',result:'PASS'}));}else{
 ...
 console.log(JSON.stringify({check:'STUDENT_DELETION_FIXTURE_DISABLED',result:'PASS',disabled:disabled.count,recordsRetained:before}));}
