...
work = Path('/home/ubuntu/bnbu-portal-password-labels-20260913')
previous = base / 'releases/portal-own-password-20260913'
release = base / 'releases/portal-password-labels-20260913'
tag = 'bnbu-portal-production:password-labels-20260913'
...
assert gate['changedSourceFiles'] == ['app/admin-i18n.ts', 'app/admin-overview.tsx', 'app/admin-subadmins.tsx', 'app/language.tsx', 'app/own-password-panel.tsx', 'app/portal-app.tsx']
...
def switch(target):
    link = base / 'current-portal-password-labels-tmp'
...
try:
    ...
    switch(release)
    print(json.dumps({'check': 'PORTAL_PASSWORD_LABELS_DEPLOYMENT', 'result': 'PASS', 'release': str(release), 'previous': str(previous), 'imageId': image_id, 'baseSource': gate['baseSource'], 'patches': gate['patches'], 'rollbackExercised': False}))
except Exception:
    if (base / 'current').resolve() != previous:
        switch(previous)
    start(previous)
    print(json.dumps({'check': 'PORTAL_PASSWORD_LABELS_DEPLOYMENT', 'result': 'FAILED_ROLLED_BACK'}))
    raise
