"""Deploy the validated administrative class display removal overlay on the pinned Hong Kong release."""
import hashlib
import json
import os
...
WORK = Path('/home/ubuntu/bnbu-class-display-final-20260914')
assert os.geteuid() == 0
assert sys.argv[1:] in [['--prepare'], ['--apply'], ['--rollback']]
gate = json.loads((WORK / 'validation.json').read_text())
assert len(gate['sourceCommit']) == 40
assert gate['checks'] == {'lint': 'PASS', 'typecheck': 'PASS', 'build': 'PASS'}
assert gate['previous'] == 'class-display-20260914'
assert gate['release'] == 'class-display-final-20260914'
previous = BASE / 'releases' / gate['previous']
release = BASE / 'releases' / gate['release']
tags = {service: f'bnbu-{service}-production:{gate["release"]}' for service in ['portal']}
...
if sys.argv[1] == '--prepare':
    ...
    candidate = 'bnbu-class-display-candidate'
