...
try{
 ...
 fixture=await seedFoundationFixture(db,'-ERASE-'+suffix);
 ...
 const peers=[];for(let i=0;i<2;i++)peers.push(await seedExerciseSessionStudent(db,fixture,'C'+i+suffix,'ACTIVE',false));
 ...
 const template=await request(admin,'/rule-templates',{displayName:'Student deletion cloud acceptance',expectedVersion:0});
 ...
 const modeResponse=await fetch('https://www.student.bnbusports.cn/api/v1/admin/review-services/manual-mode/'+fixture.teacherAActiveSectionId,{headers:{authorization:`Bearer ${admin}`}});assert.ok(modeResponse.ok);const mode=(await modeResponse.json()).data;if(!mode.enabled)await request(admin,'/admin/review-services/manual-mode',{classSectionId:fixture.teacherAActiveSectionId,enabled:true,reason:'Isolated student deletion acceptance',expectedVersion:mode.version});
 ...
 writeFileSync('/acceptance/student-deletion-checkin.json',JSON.stringify({fixture,sectionId:fixture.teacherAActiveSectionId,student,peers,studentSession,admin:{email:fixture.adminEmail,password},teacher:{email:teacherUser.primaryEmail,password},createdAt:new Date().toISOString()}),{mode:0o600,flag:'wx'});
 console.log(JSON.stringify({check:'STUDENT_DELETION_CLOUD_FIXTURE_READY',result:'PASS',organizationId:fixture.organizationId,classSectionId:fixture.teacherAActiveSectionId,studentId:student.studentId,teacherId:fixture.teacherUserId,sessionSetup:'isolated synthetic fixture; SMTP not tested'}));
