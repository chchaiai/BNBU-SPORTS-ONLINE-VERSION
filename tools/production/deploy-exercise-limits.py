...
base=Path('/opt/bnbu-sports-production');work=Path('/home/ubuntu/bnbu-exercise-limits-20260912')
previous=base/'releases/bug-20260912';release=base/'releases/exercise-limits-20260912'
...
for service in ['backend','portal','migrator']:
 image='bnbu-'+service+'-production:exercise-limits-20260912'
...
for service in ['BACKEND','PORTAL','MIGRATOR']:
 content,count=re.subn(rf'(?m)^{service}_IMAGE=.*$',f'{service}_IMAGE=bnbu-{service.lower()}-production:exercise-limits-20260912',content);assert count==1
...
try:
 ...
 # Drain/stop the old application before changing the schema. No new capped sessions
 # can be created before validation, so an application rollback remains compatible.
 ...
 raise
# Once writes resume, new session snapshots are authoritative. Later problems need
# a forward correction or a separately reviewed compatibility rollback, not old code.
site.write_text(oldNginx);run(['nginx','-t']);run(['systemctl','reload','nginx'])
result=dict(check='EXERCISE_LIMITS_DEPLOYMENT',result='PASS',sourceCommit=gate['sourceCommit'],release=str(release),previous=str(previous),staticFilesVerified=len(gate['staticFiles']),images=gate['images'],writesResumed=True)
