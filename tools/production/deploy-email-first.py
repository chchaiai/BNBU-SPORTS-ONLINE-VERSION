"""Deploy the validated email-first course joining overlay on the pinned Hong Kong release."""
import hashlib
import json
import os
...
WORK = Path('/home/ubuntu/bnbu-email-first-20260914')
assert os.geteuid() == 0
...
assert gate['previous'] == 'maintenance-navigation-20260914'
assert gate['release'] == 'email-first-20260914'
...
def switch(target):
    link = BASE / 'current-email-first-tmp'
...
if sys.argv[1] == '--prepare':
    ...
    run(['docker', 'run', '--rm', '--network', 'none', '--read-only', '--tmpfs', '/tmp', '--entrypoint', 'node', tags['backend'], '--input-type=module', '-e', "await import('reflect-metadata');for(const name of ['modules/client-capabilities/client-authentication.service','modules/join-capabilities/application/join-capabilities.service','modules/enrollments/application/qr-join.service'])await import('./dist/'+name+'.js');"])
