...
try{
 ...
 fixture=await seedFoundationFixture(db,'-AI17-'+suffix);
 const student=await seedExerciseSessionStudent(db,fixture,'L'+suffix);
 await db.classSection.update({where:{id:fixture.teacherAActiveSectionId},data:{isEnrollmentOpen:true,checkInWindowMode:'AVAILABLE',checkInStartDate:new Date('2026-08-01T00:00:00Z'),submissionDeadlineAt:new Date('2027-01-23T15:59:59Z'),checkInEndDate:new Date('2027-01-23T00:00:00Z'),dailyStartTime:new Date('1970-01-01T00:00:00Z'),dailyEndTime:new Date('1970-01-01T23:59:59Z')}});
 await db.studentProfile.update({where:{id:student.studentId},data:{majorName:'JC'}});
 const password=randomBytes(32).toString('base64url');
 await db.user.updateMany({where:{organizationId:{in:[fixture.organizationId,fixture.isolationOrganizationId]}},data:{passwordHash:await hash(password,{type:argon2id})}});
 ...
 async function request(token,path,body){const r=await fetch('https://www.student.bnbusports.cn/api/v1'+path,{method:'POST',headers:{authorization:`Bearer ${token}`,'content-type':'application/json','idempotency-key':randomUUID()},body:JSON.stringify(body)});const value=await r.json();assert.ok(r.ok,JSON.stringify({path,status:r.status,code:value.code,details:value.details,requestId:value.requestId}));return value.data;}
 const template=await request(admin,'/rule-templates',{displayName:'AI review advisory cloud acceptance',expectedVersion:0});
 ...
 await db.user.update({where:{id:fixture.adminUserId},data:{passwordHash:await hash(password,{type:argon2id})}});
 const teacherUser=await db.user.findUniqueOrThrow({where:{id:fixture.teacherUserId}});
 writeFileSync('/acceptance/ai-review-20260917.json',JSON.stringify({fixture,student,studentSession,teacherToken:teacher,adminToken:admin,accounts:{teacher:{email:teacherUser.primaryEmail,password},admin:{email:fixture.adminEmail,password}},createdAt:new Date().toISOString()}),{mode:0o600,flag:'wx'});
 console.log(JSON.stringify({check:'AI_REVIEW_CLOUD_FIXTURE_READY',result:'PASS',organizationId:fixture.organizationId,classSectionId:fixture.teacherAActiveSectionId,studentId:student.studentId,teacherId:fixture.teacherUserId,sessionSetup:'isolated synthetic fixture; SMTP not tested'}));
