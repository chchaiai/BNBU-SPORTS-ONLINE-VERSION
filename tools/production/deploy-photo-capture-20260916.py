...
assert os.geteuid()==0
base=Path('/opt/bnbu-sports-production');work=Path('/home/ubuntu/bnbu-photo-capture-20260916')
previous=base/'releases/demand-20260916-final';release=base/'releases/photo-capture-20260916'
...
for name,item in gate.items():
 assert name in ['js/photo-originals.js','js/screens/checkin.js']
...
def switch(target):
 link=base/'current-photo-fix-tmp';assert not link.exists() and not link.is_symlink();link.symlink_to(target);os.replace(link,base/'current')
try:
 switch(release)
 for name,item in gate.items():
  with urllib.request.urlopen('https://www.student.bnbusports.cn/student/'+name+'?verify=photo-capture-20260916',timeout=25) as response:assert response.status==200 and hashlib.sha256(response.read()).hexdigest()==item['after']
