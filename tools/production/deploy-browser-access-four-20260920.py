...
BASE=Path('/opt/bnbu-sports-production');WORK=Path('/home/ubuntu/bnbu-browser-access-four-20260920');RELEASE=BASE/'releases/browser-access-four-20260920'
...
assert previous==BASE/'releases/browser-gate-20260920'
...
def switch(target):
 ...
 link=BASE/'current-browser-access-four-tmp';assert not link.exists() and not link.is_symlink();link.symlink_to(target);os.replace(link,BASE/'current')
def http(url):
...
assert gate['checks']=={'tests':100,'smoke':87,'browserScenarios':8}
...
try:
 ...
 for name,digest in gate['files'].items():
  for suffix in ['', '?verify=browser-access-four-20260920']:
