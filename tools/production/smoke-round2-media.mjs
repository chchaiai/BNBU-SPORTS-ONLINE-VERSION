...
try{
 fixture=await seedFoundationFixture(db,'-ROUND2-'+Date.now().toString(36).toUpperCase());
 ...
 const workflow=await request(teacher,`/exercise-records/${record.id}/workflow`);
 assert.equal(workflow.stage,'PENDING_TEACHER');
 const publicComment='Synthetic round2 teacher public comment';
 const reviewInput={action:'VALID',publicComment,expectedVersion:workflow.version},reviewKey=randomUUID();
 const reviewed=await request(teacher,`/exercise-records/${record.id}/v81-reviews`,reviewInput,reviewKey);
 assert.equal(reviewed.stage,'VALID');assert.equal(reviewed.publicComment,publicComment);
 assert.deepEqual(await request(teacher,`/exercise-records/${record.id}/v81-reviews`,reviewInput,reviewKey),reviewed);
 const ownResult=await request(own,`/exercise-records/${record.id}`);
 assert.equal(ownResult.currentReview.publicComment,publicComment);
 assert.equal((await request(own,`/exercise-records/${record.id}/workflow`)).publicComment,publicComment);
 assert.equal(ownResult.creditedDurationSeconds,0);
 const directory=await request(admin,'/admin/course-directory');
 const row=directory.rows.find(item=>item.id===fixture.teacherAActiveSectionId);
 assert.ok(row);assert.equal(row.completedStudents,0);assert.equal(row.completionRate,0);
 const teacherEmail=`round2-${randomUUID()}@example.test`,employeeId=`R${randomUUID().replaceAll('-','').slice(0,20)}`;
 const csv=`employee_id,name,email,college\n${employeeId},Synthetic Round2 Teacher,${teacherEmail},Synthetic`;
 const preview=await request(admin,'/admin/teacher-imports/preview',{csv});assert.equal(preview.canCreate,true);
 const batchInput={csv,previewToken:preview.previewToken,initialPassword:'Initial7!'+randomUUID()},batchKey=randomUUID();
 const batch=await request(admin,'/admin/teacher-imports/confirm',batchInput,batchKey);assert.equal(batch.createdCount,1);
 assert.deepEqual(await request(admin,'/admin/teacher-imports/confirm',batchInput,batchKey),batch);
 const duplicates=await request(admin,'/admin/teacher-imports/preview',{csv});assert.equal(duplicates.canCreate,false);
 assert.ok(duplicates.rows[0].errors.includes('EMAIL_EXISTS'));assert.ok(duplicates.rows[0].errors.includes('EMPLOYEE_ID_EXISTS'));
 const security=await db.v81AccountSecurity.findUnique({where:{userId:batch.accounts[0].userId}});assert.equal(security.mustChangePassword,true);
 results.push({check:'DEFAULT_MANUAL_REVIEW_PUBLIC_COMMENT_STUDENT_READBACK_REPLAY_ZERO_SHORT_CREDIT',status:'PASS'});
 results.push({check:'COURSE_DIRECTORY_ZERO_COMPLETED_TEACHER_CREATE_REPLAY_DUPLICATE_AND_DATABASE',status:'PASS'});
 // Exercise both category targets through the real certification lifecycle.
 const certificationBytes=await sharp({create:{width:9,height:9,channels:3,background:'#337766'}}).png().toBuffer();
 const certificationUpload=await request(own,'/media-uploads',{enrollmentId:student.enrollmentId,businessPurpose:'EXEMPTION_APPLICATION',mediaType:'IMAGE',mimeType:'image/png',fileSizeBytes:certificationBytes.length,captureSource:'FILE_PICKER',declaredContentSha256:createHash('sha256').update(certificationBytes).digest('hex'),durationSeconds:null});
 const certificationPut=await fetch(certificationUpload.uploadUrl,{method:'PUT',headers:{...certificationUpload.requiredHeaders,...(origin.startsWith('http:')?{host:new URL(certificationUpload.uploadUrl).host}:{})},body:certificationBytes});
 assert.equal(certificationPut.status,200);
 await request(own,`/media-uploads/${certificationUpload.uploadSessionId}/confirm`,{etag:certificationPut.headers.get('etag').replaceAll('"','')});
 for(let attempt=0;attempt<45;attempt++){
   const media=await request(own,`/media/${certificationUpload.mediaId}`);
   if(media.uploadStatus==='AVAILABLE')break;
   assert.notEqual(media.uploadStatus,'FAILED');await delay(500);
 }
 const application=await request(own,'/exemption-applications',{enrollmentId:student.enrollmentId,applicationType:'EXERCISE_CHECK_IN',applicationSubtype:'SCHOOL_TEAM',organizationName:'Synthetic round2 team',reason:'Synthetic certification boundary acceptance',mediaIds:[certificationUpload.mediaId]});
 const submittedApplication=await request(own,`/exemption-applications/${application.id}/submit`,{expectedVersion:application.version});
 const approved=await request(teacher,`/exemption-applications/${application.id}/review`,{expectedVersion:submittedApplication.version,decision:'APPROVE',courseMinutes:600,generalMinutes:0,publicComment:'Synthetic first category only'});
 const directoryRow=async()=> (await request(admin,'/admin/course-directory')).rows.find(item=>item.id===fixture.teacherAActiveSectionId);
 assert.equal((await directoryRow()).completedStudents,0);
 const adjusted=await request(teacher,`/activity-certification-applications/${application.id}/recognition-allocation-revisions`,{expectedVersion:approved.version,reason:'Synthetic both categories completed',courseMinutes:600,generalMinutes:600});
 const completed=await directoryRow();assert.equal(completed.completedStudents,1);assert.equal(completed.completionRate,100/completed.currentMembers.students);
 await seedExerciseSessionStudent(db,fixture,'A'+randomUUID().slice(0,6).toUpperCase(),'ACTIVE',false);
 await seedExerciseSessionStudent(db,fixture,'R'+randomUUID().slice(0,6).toUpperCase(),'REMOVED',false);
 const mixed=await directoryRow();assert.equal(mixed.completedStudents,1);assert.equal(mixed.currentMembers.students,completed.currentMembers.students+1);assert.equal(mixed.completionRate,100/mixed.currentMembers.students);
 await request(teacher,`/activity-certification-applications/${application.id}/revoke`,{expectedVersion:adjusted.version,reason:'Synthetic certification reversal'});
 assert.equal((await directoryRow()).completedStudents,0);
 assert.ok(directory.rows.some(item=>item.courseTargetSeconds===null&&item.completionRate===null));
 results.push({check:'COMPLETION_REQUIRES_BOTH_CATEGORIES_INCLUDES_VALID_CERTIFICATION_EXCLUDES_REMOVED_AND_REVERSES',status:'PASS'});
