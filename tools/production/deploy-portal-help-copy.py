...
work = Path('/home/ubuntu/bnbu-portal-help-copy-20260913')
previous = base / 'releases/portal-grade-layout-20260913'
release = base / 'releases/portal-help-copy-20260913'
tag = 'bnbu-portal-production:help-copy-20260913'
...
assert gate['changedSourceFiles'] == ['app/admin-i18n.ts', 'app/admin-overview.tsx', 'app/admin-subadmins.tsx', 'app/language.tsx', 'app/own-password-panel.tsx', 'app/portal-app.tsx', 'app/teacher-workspace.tsx', 'app/physical-import-panel.tsx', 'app/physical-result-form.tsx', 'app/physical-import-panel.css', 'app/admin-help.tsx']
...
def switch(target):
    link = base / 'current-portal-help-copy-tmp'
...
try:
    ...
    switch(release)
    print(json.dumps({'check': 'PORTAL_HELP_COPY_DEPLOYMENT', 'result': 'PASS', 'release': str(release), 'previous': str(previous), 'imageId': image_id, 'baseSource': gate['baseSource'], 'patches': gate['patches'], 'rollbackExercised': False}))
except Exception:
    if (base / 'current').resolve() != previous:
        switch(previous)
    start(previous)
    print(json.dumps({'check': 'PORTAL_HELP_COPY_DEPLOYMENT', 'result': 'FAILED_ROLLED_BACK'}))
    raise
