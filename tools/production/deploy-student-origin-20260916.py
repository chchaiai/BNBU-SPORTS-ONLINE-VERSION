...
work = Path('/home/ubuntu/bnbu-student-origin-20260916')
previous = base / 'releases/course-settings-final-20260915'
release = base / 'releases/student-origin-20260916'
tag = 'bnbu-portal-production:student-origin-20260916'
gate = json.loads((work / 'validation.json').read_text())
assert gate['checks'] == {'focusedTests': '2/2', 'typecheck': 'PASS', 'build': 'PASS'}
...
assert output(['docker', 'image', 'inspect', '--format', '{{.Id}}', 'bnbu-portal-production:course-settings-final-20260915']) == gate['basePortalImage']
for name, digest in gate['baselineStudent'].items():
    assert hashlib.sha256((previous / 'web' / name).read_bytes()).hexdigest() == digest
...
candidate = 'bnbu-student-origin-candidate'
run(['docker', 'run', '--rm', '-d', '--name', candidate, '--network', 'none', '--read-only', '--tmpfs', '/tmp', '--cap-drop', 'ALL', '--security-opt', 'no-new-privileges', '--memory', '512m', tag])
try:
    for _ in range(60):
        if output(['docker', 'inspect', '--format', '{{.State.Health.Status}}', candidate]) == 'healthy':
            break
        time.sleep(1)
    else:
        raise RuntimeError('Candidate health timeout')
    assert output(['docker', 'exec', candidate, 'sha256sum', '/app/dist/server/index.js']).split()[0] == gate['files']['dist/server/index.js']
finally:
...
def switch(target):
    link = base / 'current-student-origin-tmp'
...
try:
    ...
    for name in gate['baselineStudent']:
        with urllib.request.urlopen('https://www.student.bnbusports.cn/' + name + '?release=student-origin-20260916', timeout=20) as response:
            data = response.read()
            assert hashlib.sha256(data).hexdigest() == gate['files']['web/' + name], name
            assert '地域' not in data.decode('utf-8') and '生源地' in data.decode('utf-8')
    result = {'result':'PASS', 'release':str(release), 'previous':str(previous), 'imageId':image_id, 'sourceCommit':gate['sourceCommit'], 'backendUnchanged':True, 'verifiedPublicAssets':len(assets), 'studentModuleHashes':'PASS', 'candidateHealthy':True, 'migrationExecuted':False}
