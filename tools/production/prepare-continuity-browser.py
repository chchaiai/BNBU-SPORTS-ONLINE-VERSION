...
for script in ['prepare-continuity-student.mjs']:
    subprocess.run([
        ...
        '-v', str(work / 'production-smoke-helpers.mjs') + ':/app/production-smoke-helpers.mjs:ro',
        '--entrypoint', 'node', 'bnbu-backend-production:27f9f25a-round2', '/app/' + script,
    ], check=True)
subprocess.run(['install', '-o', 'ubuntu', '-g', 'ubuntu', '-m', '600',
                str(work / 'long-acceptance/continuity.json'),
                str(work / 'continuity-private.json')], check=True)
