...
archives=[work/'teacher-cascade-combined.tar']
...
previous=base/'releases/c33f037b-teacher-cascade'
release=base/'releases/teacher-preview-20260911'
gate=json.loads((work/'teacher-preview-validation.json').read_text())
assert gate['status']=='PASS' and gate['sourceCommit']=='bcafdd612bc5f31ef51b2afe83fa5956319c29f7'
assert (base/'current').resolve()==previous and not release.exists()
def digest(stream):
    result=hashlib.sha256()
    for chunk in iter(lambda:stream.read(1024*1024),b''):result.update(chunk)
    return result.hexdigest()
delta=work/'teacher-preview-delta.tar.gz'
with delta.open('rb') as source:assert digest(source)==gate['deltaSha256']
combined=work/'teacher-preview-combined.tar'
...
for service in ['portal']:
    tag=f'bnbu-{service}-production:teacher-preview-20260911'
...
for service,old in [('PORTAL','c33f037b-teacher-cascade')]:
    before=f'{service}_IMAGE=bnbu-{service.lower()}-production:{old}'
    assert content.count(before)==1
    content=content.replace(before,f'{service}_IMAGE=bnbu-{service.lower()}-production:teacher-preview-20260911')
...
try:
    ...
    switch(release)
    print(json.dumps({'check':'TEACHER_PREVIEW_DEPLOYMENT','result':'PASS','release':str(release),'images':gate['images']}))
except Exception:
    ...
    print(json.dumps({'check':'TEACHER_PREVIEW_DEPLOYMENT','result':'FAILED_APPLICATION_ROLLED_BACK','database':'No database migration performed'}))
    raise
