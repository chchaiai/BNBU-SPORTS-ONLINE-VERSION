...
def main():
    ...
    work = Path('/home/ubuntu/bnbu-demand-20260912')
    base = Path('/opt/bnbu-sports-production')
    previous = base / 'releases/course-delete-20260911'
    release = base / 'releases/demand-20260912'
    ...
    for service in ['backend', 'portal', 'migrator']:
        image = f'bnbu-{service}-production:demand-20260912'
    ...
    for service in ['BACKEND', 'PORTAL', 'MIGRATOR']:
        content, count = re.subn(rf'(?m)^{service}_IMAGE=.*$',
                                f'{service}_IMAGE=bnbu-{service.lower()}-production:demand-20260912', content)
        assert count == 1
    ...
    try:
        ...
        result = {'check': 'DEMAND_DEPLOYMENT', 'result': 'PASS', 'sourceCommit': gate['sourceCommit'],
        ...
        print(json.dumps({'check': 'DEMAND_DEPLOYMENT', 'result': 'FAILED_APPLICATION_ROLLED_BACK',
                          'database': 'Forward migrations retained; no destructive automatic database restore'}))
        raise
