...
try{
 ...
 fixture=await seedFoundationFixture(db,'-OCR13-'+suffix);
 const student=await seedExerciseSessionStudent(db,fixture,'L'+suffix);
 const secondStudent=await seedExerciseSessionStudent(db,fixture,'B'+suffix);
 await db.studentProfile.update({where:{id:student.studentId},data:{studentNumber:'9900000001',fullName:'TEST ALPHA',gender:'FEMALE',version:{increment:1}}});
 await db.studentProfile.update({where:{id:secondStudent.studentId},data:{studentNumber:'9900000002',fullName:'TEST BETA',gender:'MALE',version:{increment:1}}});
 ...
 await db.user.updateMany({where:{id:{in:[fixture.teacherUserId,fixture.adminUserId]}},data:{passwordHash:await hash(password,{type:argon2id})}});
 ...
 const template=await request(admin,'/rule-templates',{displayName:'OCR 20260913 cloud acceptance',expectedVersion:0});
 ...
 const adminUser=await db.user.findUniqueOrThrow({where:{id:fixture.adminUserId}});
 writeFileSync('/acceptance/ocr-triplatform-20260913.json',JSON.stringify({fixture,student,studentSession,teacherToken:teacher,adminToken:admin,teacher:{email:teacherUser.primaryEmail,password},admin:{email:adminUser.primaryEmail,password},createdAt:new Date().toISOString()}),{mode:0o600,flag:'wx'});
 console.log(JSON.stringify({check:'OCR_CLOUD_FIXTURE_READY',result:'PASS',organizationId:fixture.organizationId,classSectionId:fixture.teacherAActiveSectionId,studentId:student.studentId,teacherId:fixture.teacherUserId,sessionSetup:'isolated synthetic fixture; SMTP not tested'}));
