...
previous=base/'releases/portal-required-20260913'
release=base/'releases/student-recognition-copy-20260913'
work=Path('/home/ubuntu/bnbu-student-recognition-copy-20260913')
assert (base/'current').resolve()==previous and not release.exists()
gate=json.loads((work/'validation.json').read_text())
assert gate['sourceCommit'].startswith('365b93aa')
assert set(gate['files'])=={'i18n.js','screens/profile.js'}
...
try:
    switch(release)
    for name,digest in gate['files'].items():
        with urllib.request.urlopen('https://www.student.bnbusports.cn/student/js/'+name+'?acceptance=365b93aa',timeout=20) as response:
            assert response.status==200 and hashlib.sha256(response.read()).hexdigest()==digest
    print(json.dumps({'check':'STUDENT_RECOGNITION_COPY_DEPLOYMENT','result':'PASS','release':release.name,'previous':previous.name,'sourceCommit':gate['sourceCommit'],'files':gate['files']}))
except Exception:
    switch(previous)
    raise
