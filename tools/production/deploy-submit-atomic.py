"""Deploy atomic exercise submission fix with rollback."""
import hashlib
import json
import os
...
work = Path('/home/ubuntu/bnbu-submit-atomic-20260913')
previous = base / 'releases/portal-permission-20260913'
release = base / 'releases/submit-atomic-20260913'
tag = 'bnbu-backend-production:submit-atomic-20260913'
gate = json.loads((work / 'validation.json').read_text())
assert gate['baseSource'] == 'a46fbe41f44b0724c00ee6d91944f5fdfc58cdf0'
assert gate['checks'] == {'build': 'PASS', 'focusedTests': '12/12'}
assert gate['changedSourceFiles'] == ['modules/exercise-records/application/exercise-records.service.js']
...
assert output(['docker', 'image', 'inspect', '--format', '{{.Id}}', 'bnbu-backend-production:utc-20260913']) == gate['baseImage']
...
content, count = re.subn(r'(?m)^BACKEND_IMAGE=.*$', 'BACKEND_IMAGE=' + tag, environment.read_text())
assert count == 1
...
def switch(target):
    link = base / 'current-submit-atomic-tmp'
...
try:
    ...
    switch(release)
    print(json.dumps({'check': 'SUBMIT_ATOMIC_DEPLOYMENT', 'result': 'PASS', 'release': str(release), 'previous': str(previous), 'imageId': image_id, 'baseSource': gate['baseSource'], 'patches': gate['patches'], 'rollbackExercised': False}))
except Exception:
    if (base / 'current').resolve() != previous:
        switch(previous)
    start(previous)
    print(json.dumps({'check': 'SUBMIT_ATOMIC_DEPLOYMENT', 'result': 'FAILED_ROLLED_BACK'}))
    raise
