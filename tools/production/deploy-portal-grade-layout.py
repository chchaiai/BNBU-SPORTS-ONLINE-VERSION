...
work = Path('/home/ubuntu/bnbu-portal-grade-layout-20260913')
previous = base / 'releases/portal-grade-workflow-20260913'
release = base / 'releases/portal-grade-layout-20260913'
tag = 'bnbu-portal-production:grade-layout-20260913'
...
def switch(target):
    link = base / 'current-portal-grade-layout-tmp'
...
try:
    ...
    switch(release)
    print(json.dumps({'check': 'PORTAL_GRADE_LAYOUT_DEPLOYMENT', 'result': 'PASS', 'release': str(release), 'previous': str(previous), 'imageId': image_id, 'baseSource': gate['baseSource'], 'patches': gate['patches'], 'rollbackExercised': False}))
except Exception:
    if (base / 'current').resolve() != previous:
        switch(previous)
    start(previous)
    print(json.dumps({'check': 'PORTAL_GRADE_LAYOUT_DEPLOYMENT', 'result': 'FAILED_ROLLED_BACK'}))
    raise
