...
try {
  const result = await db.$transaction(async tx => {
    ...
    const now = new Date();
    const fixtures = [];
    for (const [name, ageMinutes] of [['lateTransferInside24h', 40], ['lateTransferOutside24h', 1450]]) {
    ...
    return { check: 'SYNTHETIC_SWIM_TRANSFER_EXPIRY_FIXTURES', createdAt: now, organizationId,
