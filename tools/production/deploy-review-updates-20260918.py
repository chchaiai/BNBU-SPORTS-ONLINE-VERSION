"""Deploy pinned demand 1-6 artifacts, retaining database, limits and an application rollback."""
...
work=Path('/home/ubuntu/bnbu-review-updates-final-20260918/bundle');base=Path('/opt/bnbu-sports-production')
gate=json.loads((work/'validation.json').read_text());previous=base/'releases'/gate['release'];release=base/'releases/review-updates-final-20260918'
...
def switch(target):
 link=base/'current-review-updates-tmp';assert not link.exists() and not link.is_symlink();link.symlink_to(target);os.replace(link,base/'current')
def start(target):
...
if sys.argv[1]=='--prepare':
 ...
 for s in ['backend','portal','migrator']:
  run(['docker','tag',gate['baseImages'][s],f'bnbu-{s}-review-updates-final-base:20260918'])
  tag=f'bnbu-{s}-production:review-updates-final-20260918'
 ...
 candidate='bnbu-review-updates-portal-candidate'
...
try:
 ...
 for item in gate['web']:
  with urllib.request.urlopen('https://www.student.bnbusports.cn/student/'+item['path']+'?verify=review-updates-20260918',timeout=30) as r:assert r.status==200 and hashlib.sha256(r.read()).hexdigest()==item['after']
 ...
 result={'result':'PASS','previous':str(previous),'release':str(release),'images':images,'backup':backup,'migration':'0085_history_duration_limit','configurationAndBudgetUnchanged':True,'runtimeHashes':len(gate['delta']),'publicStudentHashes':len(gate['web']),'health':'PASS'}
