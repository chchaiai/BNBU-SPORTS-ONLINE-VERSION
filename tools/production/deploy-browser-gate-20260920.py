...
BASE=Path('/opt/bnbu-sports-production');WORK=Path('/home/ubuntu/bnbu-browser-gate-20260920');RELEASE=BASE/'releases/browser-gate-20260920'
...
assert previous==BASE/'releases/media-errors-20260920'
...
def switch(target):
 ...
 link=BASE/'current-browser-gate-tmp';assert not link.exists() and not link.is_symlink();link.symlink_to(target);os.replace(link,BASE/'current')
def http(url):
...
assert gate['checks']=={'tests':99,'smoke':87,'browserScenarios':8}
assert (BASE/'current').resolve()==previous and not RELEASE.exists()
assert set(gate['files'])=={'js/app.js','js/student-device.js'}
...
try:
 ...
 for name,digest in gate['files'].items():
  for suffix in ['', '?verify=browser-gate-20260920']:
