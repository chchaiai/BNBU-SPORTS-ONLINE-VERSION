...
try{
 ...
 fixture=await seedFoundationFixture(db,'-DEMAND16-'+suffix);
 ...
 await db.studentProfile.update({where:{id:student.studentId},data:{majorName:'未分流'}});
 ...
 const template=await request(admin,'/rule-templates',{displayName:'Demand 20260916 cloud acceptance',expectedVersion:0});
 ...
 writeFileSync('/acceptance/demand-20260916.json',JSON.stringify({fixture,student,studentSession,teacherToken:teacher,adminToken:admin,accounts:{teacher:{email:teacherUser.primaryEmail,password},admin:{email:fixture.adminEmail,password}},createdAt:new Date().toISOString()}),{mode:0o600,flag:'wx'});
 console.log(JSON.stringify({check:'DEMAND_CLOUD_FIXTURE_READY',result:'PASS',organizationId:fixture.organizationId,classSectionId:fixture.teacherAActiveSectionId,studentId:student.studentId,teacherId:fixture.teacherUserId,sessionSetup:'isolated synthetic fixture; SMTP not tested'}));
