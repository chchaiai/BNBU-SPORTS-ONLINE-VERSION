...
await loadRuntimeSecrets(process.env);const config=validateEnvironment(process.env).RUNTIME_CONFIG,db=new PrismaService(config),organizationId='01a096c2-20a2-706b-8c69-802f67dee12c',sectionId=process.argv[2],out='/tmp/ocr-closing-fixture-20260913.json';assert.ok(!existsSync(out));
try {
 const section=await db.classSection.findUniqueOrThrow({where:{id:sectionId},include:{teacher:true}});assert.equal(section.organizationId,organizationId);assert.equal(section.displayName,'Synthetic closing settlement 20260913');assert.equal(await db.enrollment.count({where:{classSectionId:sectionId}}),0);
 ...
 await db.$transaction(async tx=>{
   ...
   await tx.studentProfile.create({data:{id:student.studentId,organizationId,userId:student.userId,studentNumber:'9900000003',fullName:'TEST GAMMA',gender:'MALE',gradeYear:2026,status:'ACTIVE',createdAt:now,updatedAt:now}});
