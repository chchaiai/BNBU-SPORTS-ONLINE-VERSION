...
if sys.argv[1] == '--prepare':
    ...
    with (WORK / 'build.log').open('w') as log:
        subprocess.run(['docker','build','--network=host','--pull=false','-t',TAG,str(WORK / 'backend')],check=True,stdout=log,stderr=subprocess.STDOUT)
        subprocess.run(['docker','build','--network=host','--pull=false','-t',MIGRATOR,str(WORK / 'migrator')],check=True,stdout=log,stderr=subprocess.STDOUT)
