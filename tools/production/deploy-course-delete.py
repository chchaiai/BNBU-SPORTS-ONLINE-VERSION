...
archives=[work/'teacher-media-combined.tar']
...
previous=base/'releases/teacher-media-20260911'
release=base/'releases/course-delete-20260911'
gate=json.loads((work/'course-delete-validation.json').read_text())
assert gate['status']=='PASS' and gate['sourceCommit']=='78246a79fa5ac178f44f61249ace370b0df88694'
assert (base/'current').resolve()==previous and not release.exists()
def digest(stream):
    result=hashlib.sha256()
    for chunk in iter(lambda:stream.read(1024*1024),b''):result.update(chunk)
    return result.hexdigest()
delta=work/'course-delete-delta.tar.gz'
with delta.open('rb') as source:assert digest(source)==gate['deltaSha256']
combined=work/'course-delete-combined.tar'
...
for service in ['backend','portal','migrator']:
    tag=f'bnbu-{service}-production:course-delete-20260911'
...
with tarfile.open(work/'course-delete-static.tar') as static:
    expected={'student/js/app.js','student/js/screens/checkin.js'}
    assert set(static.getnames())==expected
    for name in expected:
        target=release/'web'/name
        assert hashlib.sha256(target.read_bytes().replace(b'\r\n',b'\n')).hexdigest()==gate['staticBefore'][name]
        content=static.extractfile(name).read()
        assert hashlib.sha256(content).hexdigest()==gate['staticAfter'][name]
        target.write_bytes(content)
env=release/'.env';content=env.read_text()
for service,old in [('BACKEND','c33f037b-teacher-cascade'),('PORTAL','teacher-media-20260911'),('MIGRATOR','e4bd83eb-student-delete')]:
    before=f'{service}_IMAGE=bnbu-{service.lower()}-production:{old}'
    assert content.count(before)==1
    content=content.replace(before,f'{service}_IMAGE=bnbu-{service.lower()}-production:course-delete-20260911')
...
try:
    ...
    switch(release)
    print(json.dumps({'check':'COURSE_DELETE_DEPLOYMENT','result':'PASS','release':str(release),'images':gate['images']}))
except Exception:
    ...
    print(json.dumps({'check':'COURSE_DELETE_DEPLOYMENT','result':'FAILED_APPLICATION_ROLLED_BACK','database':'Additive migration retained; no automatic data restore'}))
    raise
