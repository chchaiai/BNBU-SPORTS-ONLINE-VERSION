...
work = Path('/home/ubuntu/bnbu-portal-permission-20260913')
previous = base / 'releases/portal-health-20260913'
release = base / 'releases/portal-permission-20260913'
tag = 'bnbu-portal-production:permission-copy-20260913'
...
def switch(target):
    link = base / 'current-portal-permission-tmp'
...
try:
    ...
    switch(release)
    print(json.dumps({'check': 'PORTAL_PERMISSION_COPY_DEPLOYMENT', 'result': 'PASS', 'release': str(release), 'previous': str(previous), 'imageId': image_id, 'baseSource': gate['baseSource'], 'patches': gate['patches'], 'rollbackExercised': False}))
except Exception:
    if (base / 'current').resolve() != previous:
        switch(previous)
    start(previous)
    print(json.dumps({'check': 'PORTAL_PERMISSION_COPY_DEPLOYMENT', 'result': 'FAILED_ROLLED_BACK'}))
    raise
