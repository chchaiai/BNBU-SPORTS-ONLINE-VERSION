...
const result={check:'CLOUD_UTC_THREE_ROLE_READ_REGRESSION',observedAt:new Date().toISOString(),organizationId:state.fixture.organizationId,scope:'Read routes and student role denials; not full mutation or UI acceptance',passed:checks.filter(c=>c.pass).length,total:checks.length,checks};
fs.writeFileSync('evidence/ocr-triplatform-20260913/cloud-utc-read-regression.json',JSON.stringify(result,null,2)+'\n');
