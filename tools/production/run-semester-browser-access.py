...
assert os.geteuid()==0
base=Path('/opt/bnbu-sports-production');assert (base/'current').resolve().name in ['public-note-locale-20260913','semester-header-20260913']
...
assert (directory/'semester-history-fixture.json').exists()
assert (directory/'semester-history-student.json').exists()
assert (directory/'semester-history-roster.json').exists()
assert sys.argv[1:] in [['prepare'],['activate'],['disable']]
...
for host,target in [('/etc/bnbu-sports-production/secrets/runtime.json','/run/secrets/runtime.json'),('/etc/bnbu-sports-production/secrets/tencentdb-ca-chain.pem','/run/secrets/tencentdb-ca-chain.pem'),('/home/ubuntu/semester-create-only.mjs','/app/semester-create-only.mjs'),('/home/ubuntu/semester-browser-access.mjs','/app/semester-browser-access.mjs')]:args+=['-v',host+':'+target+':ro']
args+=['-v',str(directory)+':/acceptance','--entrypoint','node',image,'/app/semester-browser-access.mjs']
