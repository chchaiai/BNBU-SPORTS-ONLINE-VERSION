...
work = Path('/home/ubuntu/bnbu-semester-header-20260913')
previous = base / 'releases/public-note-locale-20260913'
release = base / 'releases/semester-header-20260913'
tag = 'bnbu-portal-production:semester-header-20260913'
...
assert gate['changedSourceFiles'] == ['app/admin-semesters.tsx', 'app/admin-workspace.tsx']
...
assert output(['docker', 'image', 'inspect', '--format', '{{.Id}}', 'bnbu-portal-production:invite-ui-final-20260913']) == gate['baseImage']
...
try:
    ...
    switch(release)
    print(json.dumps({'check': 'SEMESTER_HEADER_DEPLOYMENT', 'result': 'PASS', 'release': str(release), 'previous': str(previous), 'imageId': image_id, 'baseSource': gate['baseSource'], 'patches': gate['patches'], 'rollbackExercised': False}))
except Exception:
    if (base / 'current').resolve() != previous:
        switch(previous)
    start(previous)
    print(json.dumps({'check': 'SEMESTER_HEADER_DEPLOYMENT', 'result': 'FAILED_ROLLED_BACK'}))
    raise
