...
work=Path('/home/ubuntu/bnbu-demand-six-20260917/bundle');base=Path('/opt/bnbu-sports-production')
gate=json.loads((work/'validation.json').read_text());previous=base/'releases'/gate['release'];release=base/'releases/demand-six-20260917'
...
def switch(target):
 link=base/'current-demand-six-tmp';assert not link.exists() and not link.is_symlink();link.symlink_to(target);os.replace(link,base/'current')
def start(target):
...
if sys.argv[1]=='--prepare':
 ...
 for s in ['backend','portal']:
  run(['docker','tag',gate['baseImages'][s],f'bnbu-{s}-demand-six-base:20260917'])
  tag=f'bnbu-{s}-production:demand-six-20260917'
 ...
 candidate='bnbu-demand-six-portal-candidate'
...
try:
 ...
 for item in gate['web']:
  with urllib.request.urlopen('https://www.student.bnbusports.cn/student/'+item['path']+'?verify=demand-six-20260917',timeout=30) as r:assert r.status==200 and hashlib.sha256(r.read()).hexdigest()==item['after']
 ...
 result={'result':'PASS','previous':str(previous),'release':str(release),'images':images,'backup':backup,'migration':'NONE','configurationAndBudgetUnchanged':True,'runtimeHashes':len(gate['delta']),'publicStudentHashes':len(gate['web']),'health':'PASS'}
