...
const result={check:'CLOUD_POST_SUBMIT_FIX_THREE_ROLE_READ_REGRESSION',productionRelease:'submit-atomic-20260913',observedAt:new Date().toISOString(),organizationId:state.fixture.organizationId,scope:'Read routes and student role denials; not full mutation or UI acceptance',passed:checks.filter(c=>c.pass).length,total:checks.length,checks};
fs.writeFileSync('evidence/ocr-triplatform-20260913/cloud-submit-atomic-read-regression.json',JSON.stringify(result,null,2)+'\n');
