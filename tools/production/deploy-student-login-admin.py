"""Deploy the validated student-login/admin overlay on the pinned Hong Kong release."""
import hashlib
import json
import os
...
WORK = Path('/home/ubuntu/bnbu-login-admin-20260914')
assert os.geteuid() == 0
assert sys.argv[1:] in [['--prepare'], ['--apply'], ['--rollback']]
gate = json.loads((WORK / 'validation.json').read_text())
assert gate['sourceCommit'].startswith('d34e3e38')
assert gate['checks'] == {'http': 17, 'unitContract': 25, 'studentSmoke': 87, 'typecheck': 'PASS', 'build': 'PASS'}
assert gate['previous'] == 'semester-header-20260913'
assert gate['release'] == 'login-admin-d34e3e38-20260914'
...
def baseline():
    ...
    for name, digest in gate['baselineBackend'].items():
        assert output(['docker', 'exec', 'bnbu-sports-production-backend-1', 'sha256sum', '/app/dist/' + name]).split()[0] == digest
...
if sys.argv[1] == '--prepare':
    ...
    script = "await import('reflect-metadata');await import('./dist/modules/users/users.service.js');await import('./dist/modules/users/users.dto.js');await import('./dist/modules/client-capabilities/client-authentication.service.js');console.log('IMPORTS_PASS')"
    assert output(['docker', 'run', '--rm', '--network', 'none', '--read-only', '--tmpfs', '/tmp', '--cap-drop', 'ALL', '--security-opt', 'no-new-privileges', '--entrypoint', 'node', tags['backend'], '--input-type=module', '-e', script]) == 'IMPORTS_PASS'
    candidate = 'bnbu-login-admin-portal-candidate'
...
try:
    ...
    invalid = json.loads(http('https://www.student.bnbusports.cn/api/v1/auth/student-sign-in-codes/verify', 401, {'challengeId': str(uuid.uuid4()), 'code': '000000', 'deviceId': 'deployment-nonexistent-challenge'}))
    assert invalid['code'] == 'AUTH_VERIFICATION_CODE_INVALID'
    for service in tags:
        assert output(['docker', 'inspect', '--format', '{{.Image}}', f'bnbu-sports-production-{service}-1']) == prepared['images'][service]
    result = {'result': 'PASS', 'phase': 'DEPLOYED', 'sourceCommit': gate['sourceCommit'], 'previous': str(previous), 'release': str(release), 'images': prepared['images'], 'health': checks, 'studentModuleHashes': 'PASS', 'invalidChallenge': '401 AUTH_VERIFICATION_CODE_INVALID', 'migrationExecuted': False, 'runtimeConfiguration': 'PRESERVED', 'rollbackExercised': False}
