...
assert Path('/opt/bnbu-sports-production/current').resolve().name=='ocr-full-20260913'
...
assert image=='bnbu-backend-production:ocr-full-20260913'
assert len(sys.argv)==1
script='prepare-ocr-triplatform.mjs'
subprocess.run(['docker','run','--rm','--network','host','--read-only','--tmpfs','/tmp','--cap-drop','ALL','--security-opt','no-new-privileges','--user','10001:10001','--group-add','10001','--env-file','/opt/bnbu-sports-production/current/production.env','--env-file','/etc/bnbu-sports-production/mail.env',
 ...
 '--entrypoint','node',image,'/app/'+script],check=True)
subprocess.run(['install','-o','ubuntu','-g','ubuntu','-m','0600',str(directory/'ocr-triplatform-20260913.json'),str(work/'ocr-triplatform-20260913-private.json')],check=True)
