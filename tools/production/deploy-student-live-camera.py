...
previous=base/'releases/student-record-minutes-20260913'
release=base/'releases/student-live-camera-20260913'
work=Path('/home/ubuntu/bnbu-student-live-camera-20260913')
assert (base/'current').resolve()==previous and not release.exists()
gate=json.loads((work/'validation.json').read_text())
assert gate['sourceCommit'].startswith('0d5bf3a9')
assert set(gate['files'])=={'screens/checkin.js','recorded-video.js'}
...
try:
    ...
    print(json.dumps({'check':'STUDENT_LIVE_CAMERA_DEPLOYMENT','result':'PASS','release':release.name,'previous':previous.name,'sourceCommit':gate['sourceCommit'],'files':gate['files']}))
except Exception:
    switch(previous)
    raise
