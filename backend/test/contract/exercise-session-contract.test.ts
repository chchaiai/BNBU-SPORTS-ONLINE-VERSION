...
describe('Stage 14 ExerciseSession contract', () => {
  ...
  it('matches the operation registry, keeps no Session list, nullable active recovery, and five formal states', async () => {
    const root = await openapi();
    const paths = object(root.paths);
    let count = 0;
    for (const pathItem of Object.values(paths)) {
    ...
    assert.equal(count, Object.keys(operationPolicies).length);
