...
// @ts-expect-error JavaScript integration probe shared with local runtime checks.
import { probeEnduranceHttp } from '../../../tools/local-integration/v81-endurance-http-probe.mjs';
...
describe('V81 endurance rule administration HTTP E2E', () => {
  ...
  it('preserves rule revisions, rejects invalid bands and enforces concurrent versions and revoked permissions', async () => {
    const adminToken = await login(fixture.adminEmail);
    const teacherToken = await login(fixture.teacherEmail);
    const student = await seedExerciseSessionStudent(prisma, fixture, 'END');
    ...
    await probeEnduranceHttp({ prisma, fixture, baseUrl: `${baseUrl}/api/v1`, request: businessRequest,
      adminToken, teacherToken, token });
  });
});
